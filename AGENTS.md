...
| HWPX 픽스처 추가 | `Tests/CoreHwpTests/HwpxFixtures/<id>/` (`openHwpx` 사용, 별도 루트라 위 스윕 핀에는 영향 없음 — `HwpxFixtures/README.md`의 생성 정책 참조). 대신 **뷰어 계층 핀**이 걸린다: manifest에 `sourceHwpFixture`(HWP 쌍)와 `pageCount`+`pageCountSource`를 채운다 — `Tests/HwpKitTests/HwpxFixtureRenderTests.swift`가 파싱 가능한 픽스처 전부에 쪽수 핀과 출처를 **등식으로 강제**하므로 빠뜨리면 빨개진다 (`expectedError` 픽스처만 예외). 유실 가드용 하한도 함께 올린다 — `HwpxFixtureRenderTests`에 여덟 곳(픽스처 수 1 + 비교 쌍 6 + 가시 텍스트 1), `HwpxFixtureCopyParityTests`·`HwpxFixtureManifestTests`·`HwpxHwpEquivalenceTests`·`HwpParagraphNumberingFixtureTests`에 각 한 곳이다 (정확한 현재 값은 `rg 'count\) >= ' Tests/`로 확인한다 — 여기 숫자를 적으면 곧 낡는다) |
...
- **계약**: 결정적 순서 — 컨테이너(구역·문단·컨트롤) 순회는 문서 순서,
  ...
  정상 픽스처 전수(전 35종 중 expectedError 4종 제외 31종)에서 복구 kind 0건, 실저장본 진단은
...
| `FixtureBaselineAnchorTests` | 정합성 (그려진 베이스라인 = 문서 자신의 줄 캐시 `lineLocation + baselineDistance`) | **문서에 적힌 값** (소스 상수) | ✓ 상시 |

넷째 줄은 좌표를 기록하지 않고 **성질**을 검사한다 (#95) — 위 셋은 전부 "안
바뀜"만 증명해서, 각주가 본문을 덮는 렌더가 헌법주석 362쪽에서 기준선으로 굳어
있어도 아무도 빨개지지 않았다. 겹침 예산은 #165(각주 이어짐, 한글 12.30 PDF
1,030쪽 전수 대조 — `Sources/HwpKitCore/AGENTS.md`)로 **전 픽스처 0**이 됐다 —
쪽수·총 침범·최대 침범 세 축 모두다. 예산이었을 때의 교훈은 남는다: 쪽수 하나로는
오독한다 — #95는 362 → 368쪽으로 **늘리면서** 총 침범을 14,707 → 6,917pt로
줄였다 (각주가 제 페이지로 돌아와 집계 대상 페이지는 늘고, 마지막 조각에 몰렸던
스택은 흩어진다). 각주 영역 **상단**이 본문 상단 위로 올라가지 않는 것과 각주가
...
프레임 **하단** 초과도 이제 0이다 — 절대 캐시 모드의 스택이 본문 아래 자리에
맞춰 바닥 정렬되고 안 들어가는 각주는 나뉘거나 다음 쪽으로 가기 때문이다. 남는
경우는 빈 쪽에도 안 들어가는 분할 지점 없는 각주를 진행 보장으로 싣는 것뿐이라
(코퍼스 0건) 예산으로 두되 0이다. **문서가 불변식이라고 적은 것을 가드가 절반만
검사하면 그 절반은 없는 것과 같다** — #95 리뷰가 이 구멍을 상단만 보던 가드에서
찾아냈다.
...
다섯째 줄은 기준선을 우리가 기록하지 않는다 (#178) — 오라클이 **문서 자신의 줄
캐시**(`PARA_LINE_SEG`)라 "안 바뀜"이 아니라 "한글이 적어 둔 자리인가"에 답한다.
한글이 그 자리에 실제로 그린다는 것은 PDF 내보내기의 벡터 베이스라인으로 확인했고
(합성 스윕 34줄·`CharShape` 12줄·`noori` 본문 18줄), 앵커가 글꼴 지표의 함수가
아니므로 결정론 resolver에서도 같은 값이라 CI에서 돈다.

CI ✓인 여섯 스위트가 도는 근거는 전부 `HwpFontResolver.testDeterministic`
하나다 — 폰트 조회 세 축(시스템 등록 폰트·한컴 번들·문서 대체 글꼴)을 모두
닫아 설치 폰트와 무관하게 같은 CTFont가 나온다. `HwpxFixtureRenderTests`도
쪽수 두 축(manifest 대조·HWP 쌍 대조)과 쪽 크롬 텍스트 축(HWP 쌍 대조, #135)·
차트 블록 수 축(HWP 쌍 대조, #134)에서 그 resolver를 쓰고, 같은 클래스의 가시
텍스트 축만 기본 resolver다 — 좌표가
...
가드 다섯 층"의 넷째 줄과 같은 교훈이다: 가드가 "레이어가 올바른가"만 보면
...
가드 축도 위 다섯 층과 다르다 — **기준선이 없고 두 경로를 맞대 본다**
...
타임아웃도 이 API로 대체됐다 — 즉 **렌더 가드가 이제 이 프로덕션 코드를
...
뒤집힌다 — 한글.app도 이 장식만 클립보드 RTF에서 뺀다, 2026-09-09 실측),
...
3. **검증 (계층별)**
   - `swift test` — 환경 의존 2종(렌더 해시·fidelity)은 자동 skip. 커밋된
     ...
     이유가 사라진다 — 유일한 예외는 위 "렌더 가드 다섯 층"의 정합성 수정 사례),
...
opt-in — **커밋된** 기준선을 쓰는 스위트는 CI에서 상시 돈다 (위 "렌더 가드 다섯 층").
