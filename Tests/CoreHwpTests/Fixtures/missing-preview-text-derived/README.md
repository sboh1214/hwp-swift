...
- `document.hwp`: Hancom Office HWP for macOS 12.30.0 build 6382가 저장한
  `plain-text-minimal` HWP를 기반으로, OLE directory entry의 `PrvText` stream
  name만 같은 길이의 `XrvText`로 바꾼 파생 fixture.
- 목적: `PrvText` stream이 이름 기준으로 없을 때 `CoreHwp`가 crash나 필수 stream
  오류 없이 기본 `HwpPreviewText` raw payload(`[0x0D, 0x00, 0x0A, 0x00]`)를
...
2. 파일 bytes에서 UTF-16LE `PrvText` directory entry 이름 한 건만 찾아 `XrvText`로
   교체한다. stream payload와 다른 OLE 구조는 변경하지 않는다.
3. `manifest.json`의 `previewTextLength`를 `1`,
   `previewTextRawPayloadLength`를 `4`, `previewTextPrefixBytes`와
   `previewTextSuffixBytes`를 `[13, 0, 10, 0]`으로 둔다.
