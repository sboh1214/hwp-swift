...
문단 k(1부터)의 베이스라인은 줄 캐시대로 107.7 + 16k(PDF 실측 123.72·139.80…)다.
아래 '해석' 열의 이동은 **베이스라인 기준·위가 양수**이고, 선 y는 쪽 위에서부터라
위 방향 이동은 y에서 뺀 값이다. 선의 중심 y:

| k | 조합 | 선 y | 해석 (베이스라인 기준, 위 양수) |
|---|---|---:|---|
| 1 | 위 첨자 취소선 | 117.12 | 옮겨진 베이스라인(4.32 위) + 0.35 × 6.36pt = 6.60 위 |
| 2 | 아래 첨자 취소선 | 138.72 | 옮겨진 베이스라인(1.20 아래) + 0.35 × 6.36pt = 1.08 위 |
...
선 두께는 11개 전부 0.36pt(0.04em = 0.4pt의 0.12pt 장치 양자화, 첨자 run도 본문 두께)다.
첨자 글리프는 6.36pt로 위 첨자가 4.32~4.44pt 위(표본에 따라 장치 한 단위 차), 아래
첨자가 1.20pt 아래에 놓인다 (함초롬바탕·Apple SD 산돌고딕 Neo 두 글꼴, 10·20pt에서
같은 비율).
