...
- `memo`: Hancom Office HWP for macOS 12.30.0 build 6382로 생성.
  ...
  raw payload, structured parameter 보존을 실제 fixture로 검증.
...
- `underline-above`: `plain-text-minimal` 사본에 글자 모양 › 확장 › 밑줄 위치
  '위쪽'을 적용해 Hancom Office HWP for macOS 12.30.0 build 6446으로 2026-09-05에
...
- `legacy-common-control-property`: 기존 HWP 5.0.2.2 샘플에서 object description 길이
  ...
  manifest 및 실제 fixture로 검증. `FixtureCorruptionStabilityTests`의
...
2026-06-15 기준 local Downloads folder의 HWP 후보와 한컴오피스 앱 번들 preset
...
검증합니다. 2026-06-16에는 repo fixture, local Downloads folder,
...
`/Applications/Hancom Office HWP.app/Contents/Resources`, local Downloads folder
...
실제 파싱 결과로 확인합니다.
2026-06-19에는 local Documents/Downloads/Desktop folders와 temporary directory의
현재 HWP 후보 17개를 package-internal
...
local Downloads/Documents/Desktop folders와
...
후보는 새로 발견되지 않았습니다.
같은 날 personal CloudStorage folder에서 `.hwp`/`.hwt` 후보 1,446개를
...
같은 날 repository root, local Downloads folder, personal CloudStorage folder 아래
`.hwp`/`.hwt` 후보 1,518개를 다시
...
entry를 동시에 rename하는 `OptionalStreamStabilityTests`로 두 preview stream
...
local Downloads folder와 한컴오피스 앱 번들 `Contents/Resources`의 HWP/HWT 후보
...
빈 HWP를 temporary output file로 저장한 뒤 값을
...
현재 repo fixture, local Downloads/Documents/Desktop folders, 한컴오피스 앱 번들의
`.hwp`/`.hwt` 361개를
...
2026-06-20에 repo fixture, local Downloads folder,
...
## 상위 README 이관: fixture 기준

최상위 README를 간결하게 유지하기 위해 fixture 정책, coverage 표, 공개 sample scan
기록을 이 하위 문서로 이관했습니다.
...
| memo | 검증 중: 한컴오피스 생성 문서의 `MEMO/...` parameter field를 memo control로 분류하고 raw payload, structured parameter 보존 | `memo` |
