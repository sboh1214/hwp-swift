...
1. `/Applications/한컴오피스 한글.app`(bundle
   ...
   `line-shapes`·`script-decorations`)은 `.hwp`와 `.hwpx`를 **같은 편집 세션에서
   연달아** 저장해야 두 파일이 같은 문서가 된다. 한글 GUI로 만들기 어려운 조합은
   합성 HWPX를 한글로 열어 두 형식으로 저장해도 된다(`line-shapes` — 17종 선 모양을
   네 자리에 모두 실은 문서, `script-decorations` — 첨자 × 장식선 × 글자 위치 조합
   11문단; 각각 `Fixtures/<id>/README.md`의 절차).
