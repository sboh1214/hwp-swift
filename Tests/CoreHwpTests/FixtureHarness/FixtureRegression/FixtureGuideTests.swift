...
private func assertHancomAutomationPolicy(in guide: String) {
    ...
    expect(guide).to(contain("2026-08-28 재확인"))
    expect(guide).to(contain("/Applications/한컴오피스 한글.app"))
    expect(guide).to(contain("12.30.0` / `6446"))
    expect(guide).to(contain("한글 표준 문서 (*.hwpx)"))
