...
import XCTest
    ...
    final class HwpMeasuredLineFragmentCacheTests: XCTestCase {
        ...
        /// 접혀 상자에 들어간다; 앞 조각은 측정 전진량이라 표식이 있다.
        ...
        /// 쪽 경계 흐름 분할: 본문 24pt라 캐시 높이 32pt 문단이 쪽보다 커 줄 단위로 나뉜다 —
        /// 앞 조각 30자(표식), 뒤 조각 30자 + `끝`(무표식·17pt·한 줄).
        func testFlowSplitKeepsTheCachedRemainderFragmentUnmarked() async throws {
            ...
            expect(fragments.map(\.text.length)) == [30, 31]
            Self.expectCacheGate(fragments: fragments)
        }

        /// 표 행 분할: 셀 문단은 캐시 높이(`preferCachedHeight`)로 재어 rect 높이가 32pt이고
        /// 행은 마지막 줄 간격을 뺀 26pt(#160)다 — 본문 24pt라 빈 쪽에도 안 들어가 줄 하나
        /// (표식)와 나머지 둘(무표식)로 나뉜다.
        ...
        func testColumnBalanceKeepsTheCachedRemainderFragmentUnmarked() async throws {
            ...
            let section = HwpSynthetic.section(
                firstParagraphControls: [
                    .section(Support.sectionDef(columnWidth: columnWidth * 2 + 10, contentHeight: 60)),
        ...
        /// 비등폭 2단(넓은 단 31자·좁은 단 30자, 간격 10pt) 구역 — 62자 문단은 넓은 단 폭으로
        /// 31자 + 31자 두 줄로 재어진다.
        private static func unevenColumnPaginator(
            ...
            let built = Self.built(paragraph)
            let wide = Support.columnWidth(charactersPerLine: 31, in: built)
            let narrow = Support.columnWidth(charactersPerLine: 30, in: built)
        ...
        private static func expectNarrowSingleLineFold(
            ...
            expect(file: file, line: line, fragments[1].frame.width).to(beCloseTo(narrow, within: 0.05))
        ...
        /// 흐름 분할(`appendLineSliceBlock`)로 비등폭 단에 이월된 **한 줄** 조각(캐시 없음): 본문
        /// 40pt라 넓은 단에 구역 첫 문단과 첫 줄이 들어가고 둘째 줄이 좁은 단으로 간다 —
        /// 좁은 단(6% 이내)에서 렌더러가 다시 줄바꿈하면 두 줄이 되어 한 줄 높이 상자를
        /// 넘치므로 표식 없이 종전 접힘을 둔다.
        func testSingleLineFragmentCarriedToANarrowerColumnKeepsTheFold() async throws {
        ...
        /// 다단 균형 재배치(`balancedBlocks`)로 비등폭 단에 옮겨진 **한 줄** 조각도 같다: 본문
        /// 60pt라 문단이 넓은 단에 다 들어간 뒤 밴드를 닫으며 둘째 줄이 좁은 단으로 옮겨진다.
        func testSingleLineUnitRebalancedIntoANarrowerColumnKeepsTheFold() async throws {
            let paragraph = try HwpSynthetic.textParagraph(String(repeating: "가", count: 62))
