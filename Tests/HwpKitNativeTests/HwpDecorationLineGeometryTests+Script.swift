...
///
/// 선 두께는 네 조합 모두 본문과 같은 0.36pt(= 10pt × 0.04)다.
///
...
extension HwpDecorationLineGeometryTests {
    ...
    /// 글자 위치 30 → 아래 3pt (렌더러 규약 양수 = 위)
    private static let locationShift: CGFloat = -3
    ...
    /// 줄어든 크기의 0.35배로 놓인다. 종전에는 셋이 전부 같은 행이라 첨자 글리프
    /// 아래로 선이 떨어졌다.
    ...
    /// 줄어든 6.7pt 기준(−1.14pt)이라 0.56pt 위였다.
    ...
    /// 첨자 run의 취소선·아래쪽 밑줄 두께는 본문과 같다 (한글: 10pt 첨자 선 0.36pt =
    /// 본문). 빈칸 run 위 선의 열 커버리지 합으로 잰다 — 청록은 본문, 자홍은 첨자.
    func testScriptRunDecorationLinesKeepPreScriptThickness() throws {
        ...
        for below in [false, true] {
            ...
            text.append(NSAttributedString(
                string: "    ",
                attributes: below ? belowUnderlineRun(color: cyan) : strikethroughRun(color: cyan)
            ))
            text.append(NSAttributedString(
                string: "    ",
                attributes: below
                    ? belowUnderlineRun(color: magenta, script: true)
                    : strikethroughRun(color: magenta, script: true)
            ))
            let raster = try render(text: text)
            let plain = try XCTUnwrap(
            ...
            expect(scripted).to(
                beCloseTo(ratio * Self.baseSize, within: 0.05),
                description: below ? "아래 밑줄" : "취소선"
            )
        }
