...
// (`ci.yml`의 iOS 잡이 같은 번들을 돌린다) macOS 전용으로 못박는다.
    ...
    final class FixtureRenderGoldenTests: XCTestCase {
        ...
        ///
        /// **`noori` p3(인덱스 2)는 의도적으로 뺐다** — 붙임 표의 선언 높이가
        /// 627.0pt(62,701 HWPUNIT)이고 이 파일의 한글 캐시도 그 문단에 637.5pt를
        /// 잡아 뒀는데 우리 표 블록은 181.6pt로 그려져, 형상 행이 눌리고 셀 그림이
        /// 아래 행 위로 넘친다. 골든은 "바뀌었나"에만 답하고 "맞나"에는 답하지
        /// 않으므로, 틀린 렌더를 커밋하면 나중의 올바른 수정이 리뷰에서 회귀처럼
        /// 보인다. 표 높이 건이 정리된 뒤 이 목록에 합류시킨다.
