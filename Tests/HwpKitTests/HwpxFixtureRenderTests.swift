...
final class HwpxFixtureRenderTests: XCTestCase {
    ...
    /// 쪽수가 정확히 일치해야 한다.
    ///
    /// 핀은 **파싱 가능한 픽스처 전부**에 강제한다 — 하한(`>= 30`)만 두면
    /// `pageCount` 없는 새 픽스처가 조판 캐시 회귀 가드에서 조용히 빠진다.
    /// 등식만 두면 반대로 픽스처가 전부 유실돼도 0 == 0으로 통과하므로 하한도
    /// 남긴다. `expectedError` 픽스처(암호·배포용·DRM)는 HWP 하니스가 52종 중
    /// 48종만 요구하는 것과 같은 이유로 분모에서 뺀다.
    ...
    /// 선다 (#167). `hp:pageHiding`·`hp:newNum`이 승격돼야 section-marks 쌍의
    /// 1쪽 번호가 감춰지고 2·3쪽이 9·10으로 다시 매겨진다 (#169 — 강등 상태는
    ...
    func testHwpxPageChromeMatchesHwpPairs() async throws {
        ...
        var sectionMarkChrome: [[String]]?
        var pageNumberSkipChrome: [[String]]?
        for fixture in hwpxFixtures where !fixture.hasExpectedError {
            ...
            do {
                ...
                if fixture.id == "section-marks" {
                    sectionMarkChrome = hwpxChrome
                }
                if fixture.id == "section-page-number-skip" {
                    pageNumberSkipChrome = hwpxChrome
                }
            } catch {
                failures.append("[\(fixture.id)] load threw: \(error)")
            }
        ...
        // 2쪽은 새 번호 9, 3쪽은 그 뒤를 이어 10이다.
        expect(sectionMarkChrome) == [[], ["- 9 -"], ["- 10 -"]]
        // 이어서 1 → 홀수 3(2를 건너뜀) → 짝수 4(이미 짝수) → 짝수 6(5를 건너뜀) →
        // 홀수 7(이미 홀수) → 사용자 9 → 이어서 10. 물리 쪽은 7쪽 그대로다 (한글 12.30
        // PDF 내보내기 실측, `Fixtures/section-page-number-skip/README.md`).
        expect(pageNumberSkipChrome) == [
            ["- 1 -"], ["- 3 -"], ["- 4 -"], ["- 6 -"], ["- 7 -"], ["- 9 -"], ["- 10 -"],
        ]
        if !failures.isEmpty {
    ...
    /// 유지된다).
    ///
    /// 힌트 축의 알려진 격차는 `knownHwpxHintGaps`에 **정확히** 핀한다 — 건너뛰지 않는다.
    /// 승격이 들어오면 그 등식이 깨져 표에서 지우게 된다.
    func testHwpxChartBlocksMatchHwpPairs() async throws {
        ...
        for fixture in hwpxFixtures where !fixture.hasExpectedError {
            ...
            do {
                ...
                if let gap = Self.knownHwpxHintGaps[fixture.id] {
                    if hwpxHints != gap.hwpx || hwpHints != gap.hwp {
                        failures.append("[\(fixture.id)] hint gap: \(hwpxHints) / \(hwpHints)")
                    }
                } else if hwpxHints != hwpHints {
    ...
    ///
    /// 라벨 문자는 픽스처 20종의 유일한 실물인 `-`로 한정한다. `□`·`o` 같은
    ...
    /// 미지원 힌트 축의 알려진 HWP↔HWPX 격차 (픽스처 id → 두 포맷의 힌트).
    ///
    /// `hyperlink-click-band`(#233): HWPX 매퍼가 `hp:fieldBegin type="HYPERLINK"`를 typed
    /// `.hyperLink`로 승격하지 않고 강등 컨트롤(`.notImplemented`, 필드 4CC `%hlk`)로 보존해,
    /// 링크 29개마다 "알 수 없음" 힌트가 선다 (HWP 쌍은 typed라 힌트가 없다). 링크 영역도 그래서
    /// HWP 쌍에만 선다. 승격되면 이 항목을 지운다.
    private static let knownHwpxHintGaps: [String: (hwpx: [String], hwp: [String])] = [
        "hyperlink-click-band": (
            hwpx: Array(repeating: "알 수 없음: notImplemented", count: 29),
            hwp: []
        ),
    ]
    ...
    /// 30쌍 중 주석을 가진 문서는 footnote-endnote·inline-table-actual-height·
    ...
    func testHwpxFootnoteBlocksMatchHwpPairs() async throws {
        ...
        for fixture in hwpxFixtures where !fixture.hasExpectedError {
            ...
            do {
                let hwpx = try await loader.load(from: fixture.documentURL)
                let hwp = try await loader.load(from: pair.documentURL)
                let hwpxNotes = Self.footnoteTexts(of: hwpx)
                let hwpNotes = Self.footnoteTexts(of: hwp)
                if hwpxNotes != hwpNotes {
                    failures.append("[\(fixture.id)] hwpx notes \(hwpxNotes) != hwp \(hwpNotes)")
                }
                if Self.footnoteFrames(of: hwpx) != Self.footnoteFrames(of: hwp) {
                    failures.append("[\(fixture.id)] note frames differ from the HWP pair")
                }
                if fixture.id == "footnote-endnote" {
                    noteTexts = hwpxNotes
                }
            } catch {
                failures.append("[\(fixture.id)] load threw: \(error)")
        ...
        if !failures.isEmpty {
            fail("HWP↔HWPX footnote mismatches (\(failures.count)):\n" +
                failures.joined(separator: "\n"))
        }
    }

    /// 책갈피가 **탐색 목록**(`metadata.outline`)에 나와야 한다 (#169의 검증 항목) —
    /// `hp:bookmark`가 승격돼야 `HwpOutlineCollector`가 `.bookmark` 컨트롤에서
    /// 이름을 읽는다. 강등 상태에서는 목록이 비어 HWP 쌍과 갈린다.
    func testHwpxBookmarkOutlineMatchesHwpPairs() async throws {
        let hwpxFixtures = try FixtureRoot.loadAllHwpxFixtures(from: #file)
        let hwpFixtures = try FixtureRoot.loadAllFixtures(from: #file)
        let hwpByID = Dictionary(uniqueKeysWithValues: hwpFixtures.map { ($0.id, $0) })
        let loader = HwpDocumentLoader(fontResolver: .testDeterministic)
        ...
        var failures: [String] = []
        var comparedCount = 0
        var sectionMarkBookmarks: [String]?
        for fixture in hwpxFixtures where !fixture.hasExpectedError {
            guard let pairID = fixture.sourceHwpFixture, let pair = hwpByID[pairID],
                  !pair.hasExpectedError
            else { continue }
            ...
            do {
                let hwpx = try await loader.load(from: fixture.documentURL)
                let hwp = try await loader.load(from: pair.documentURL)
                let hwpxBookmarks = Self.bookmarkTitles(of: hwpx)
                let hwpBookmarks = Self.bookmarkTitles(of: hwp)
                if hwpxBookmarks != hwpBookmarks {
                    failures.append(
                        "[\(fixture.id)] hwpx bookmarks \(hwpxBookmarks)"
                            + " != hwp \(hwpBookmarks)"
                    )
                }
                if fixture.id == "section-marks" {
                    sectionMarkBookmarks = hwpxBookmarks
                }
            } catch {
                failures.append("[\(fixture.id)] load threw: \(error)")
            }
        ...
        expect(comparedCount) >= 30
        // 30쌍 중 책갈피를 가진 문서는 section-marks와 inline-object-marker-size(마커 글자
        ...
        if !failures.isEmpty {
            fail("HWP↔HWPX bookmark outline mismatches (\(failures.count)):\n" +
                failures.joined(separator: "\n"))
        }
    }

    private static func bookmarkTitles(of document: HwpDocument) -> [String] {
        document.metadata.outline.bookmarks.map(\.title)
    }

    private static func footnoteTexts(of document: HwpDocument) -> [[String]] {
    ...
    private static func footnoteFrames(of document: HwpDocument) -> [[String]] {
        document.pages.map { page in
            page.blocks.filter { $0.kind == .footnote }.map {
                String(format: "%.2f,%.2f", $0.frame.origin.x, $0.frame.origin.y)
            }
        }
    }
