...
final class HwpxFixtureRenderTests: XCTestCase {
    ...
    /// 쪽수가 정확히 일치해야 한다.
    ///
    /// 핀은 **파싱 가능한 픽스처 전부**에 강제한다 — 하한(`>= 15`)만 두면
    ...
    /// (#134). 14쌍 중 차트를 가진 문서는 chart뿐이라 나머지는 0 == 0 등식이고,
    ...
    ///
    /// 라벨 문자는 픽스처 14종의 유일한 실물인 `-`로 한정한다. `□`·`o` 같은
    ...
    /// 14쌍 중 주석을 가진 문서는 footnote-endnote 하나뿐이라 나머지는 빈 배열
    ...
    func testHwpxBookmarkOutlineMatchesHwpPairs() async throws {
        ...
        expect(comparedCount) >= 15
        // 15쌍 중 책갈피를 가진 문서는 section-marks뿐이라 나머지는 빈 배열
