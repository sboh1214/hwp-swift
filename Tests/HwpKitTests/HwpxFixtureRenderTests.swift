...
final class HwpxFixtureRenderTests: XCTestCase {
    ...
    /// 쪽수가 정확히 일치해야 한다.
    ///
    /// 핀은 **파싱 가능한 픽스처 전부**에 강제한다 — 하한(`>= 21`)만 두면
    /// `pageCount` 없는 새 픽스처가 조판 캐시 회귀 가드에서 조용히 빠진다.
    /// 등식만 두면 반대로 픽스처가 전부 유실돼도 0 == 0으로 통과하므로 하한도
    /// 남긴다. `expectedError` 픽스처(암호·배포용·DRM)는 HWP 하니스가 41종 중
    /// 37종만 요구하는 것과 같은 이유로 분모에서 뺀다.
    ...
    /// 21쌍 중 쪽 크롬을 가진 문서는 그 넷뿐이라 나머지는 빈 배열 등식이고, 넷은
    ...
    /// (#134). 21쌍 중 차트를 가진 문서는 chart뿐이라 나머지는 0 == 0 등식이고,
    ...
    /// 21쌍 중 주석을 가진 문서는 footnote-endnote 하나뿐이라 나머지는 빈 배열
    ...
    func testHwpxBookmarkOutlineMatchesHwpPairs() async throws {
        ...
        expect(comparedCount) >= 21
        // 21쌍 중 책갈피를 가진 문서는 section-marks뿐이라 나머지는 빈 배열
