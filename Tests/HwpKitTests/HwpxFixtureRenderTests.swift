...
final class HwpxFixtureRenderTests: XCTestCase {
    ...
    /// 쪽수가 정확히 일치해야 한다.
    ///
    /// 핀은 **파싱 가능한 픽스처 전부**에 강제한다 — 하한(`>= 20`)만 두면
    ...
    /// 20쌍 중 쪽 크롬을 가진 문서는 그 넷뿐이라 나머지는 빈 배열 등식이고, 넷은
    ...
    /// (#134). 20쌍 중 차트를 가진 문서는 chart뿐이라 나머지는 0 == 0 등식이고,
    ...
    /// 20쌍 중 주석을 가진 문서는 footnote-endnote 하나뿐이라 나머지는 빈 배열
    ...
    func testHwpxBookmarkOutlineMatchesHwpPairs() async throws {
        ...
        expect(comparedCount) >= 20
        // 20쌍 중 책갈피를 가진 문서는 section-marks뿐이라 나머지는 빈 배열
