...
/// pt). 문단 k(1부터)의 베이스라인은 줄 캐시대로 107.7 + 16k이고 선은 다음 자리다:
///
/// | k | 문단 | 한글 선 y | 기준 |
/// |---|---|---:|---|
/// | 1 | 위 첨자 취소선 | 117.12 | 옮겨진 베이스라인 119.40 + 0.35 × 6.36 |
/// | 2 | 아래 첨자 취소선 | 138.72 | 옮겨진 베이스라인 141.00 + 0.35 × 6.36 |
...
/// 아래 첨자 내림 0.30em vs 0.12em — 이 축은 #179 밖이라 후속 이슈) 절대 좌표 대신
    ...
    func testScriptIndependentLinesMatchHancomCoordinatesInBothFormats() async throws {
        ...
        for hwpx in [false, true] {
            ...
            for pin in pins {
                ...
                // 래스터 양자화 0.25pt + 한글 장치 좌표 0.12pt 양자화.
                expect(center).to(
