...
- **'글자 아래' 밑줄이 한글보다 낮게, 장식선이 큰 글자에서 가늘게 그려지던 것을
  ...
  `0.12pt × round(크기/3)`과 13개 크기 전부 일치) 40pt 글자의 선이 종전의 4배 굵기인
  1.6pt로 그려집니다. 변경 내용 추적 삽입 밑줄은 0.75pt 사각형의 아래 모서리를
  0.35배에 두어 중심이 크기에 비례하지 않던 것을 중심 0.26배·두께 0.065배로 고쳤습니다.
...
- **취소선이 글자 가운데보다 낮게 그려지던 것을 바로잡았습니다** (#136). 종전에는
  ...
  **베이스라인 위 글자 크기의 0.35배**에 놓입니다. 이제 그 비율로 그립니다 —
  ...
  변경 내용 추적의 삭제선은 한글이 조금 낮게(0.29배) 그리므로 분리했습니다.
