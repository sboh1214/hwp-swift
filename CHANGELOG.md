...
- **줄 상자가 링크 글자보다 높은 줄에서 링크 밑줄이나 글자와 밑줄 사이를 눌러도 링크가 열리지
  ...
  **줄 상자 상단부터 그 줄의 줄 간격 몫까지**(다음 줄 상자 상단까지)이고, 문단 위·아래 간격은
  ...
  100%이거나 고정값이 줄 상자보다 작으면 그 줄 아래의 밑줄은 다음 줄 몫입니다. 쪽·문서의 마지막
  본문 줄은 한글이 줄 상자에서 끊는 것과 달리 줄 간격 몫까지 열립니다(그 아래는 빈 자리입니다).
  글자 위치로 옮겨진 글자의 영역은 종전처럼 그 글자가 그려진 자리를 따릅니다. 한글.app이 저장한
  `hyperlink-click-band` 쌍(HWP + HWPX, 2쪽 29개 링크)을 픽스처로 추가해 링크마다 영역을 문서의 줄
  배치 정보와, 클릭 결과를 한글 편집 화면 실측과 대조했습니다. 각주는 문단마다 블록 하나라 그 블록이
  각주의 마지막 항목인지를 알리는 `HwpFootnoteBlock.isNoteEnd`(기본값 `true`)를 더했습니다.
