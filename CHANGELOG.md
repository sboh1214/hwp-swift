...
- **쪽·단 끝에 줄 상자가 들어가는 줄을 줄 간격 때문에 다음 쪽으로 넘기던 문제를 바로잡았습니다**
  ...
  줄 배치 정보가 없는 각주는 마지막 줄의 줄 간격 여분(10pt·160%면 6pt)까지 각주 영역에 넣어 구분선이
  그만큼 위에 그려지고 본문이 먼저 넘어갔고, 다단 구분선은 단 끝 문단의 아래 간격만큼 길게
  그려졌습니다(한글은 둘 다 마지막 줄 상자 바닥에서 끝냅니다). 한글이 저장한 줄 배치 정보로 놓는
  문서는 저장된 쪽 나눔을 그대로 따르므로 바뀌지 않습니다.
...
- **1단 본문에서 문단의 일부 줄이 현재 쪽에 들어갈 수 있어도 문단 전체가 다음 쪽으로 넘어가
  ...
  않고, 나머지 속성(줄 나눔 단위·`lineWrap`)은 조판이 아직 읽지 않습니다. 쪽 끝 적합 판정이
  줄 상자가 아니라 줄 전진량이라 한글보다 한 줄 먼저 넘어가는 경우(#222)와, 첫 줄은 들어가지만
  그 각주까지는 들어가지 않을 때 한글이 줄을 남기고 각주만 다음 쪽으로 넘기는 규칙은 이번
  범위 밖입니다(그 경우 문단을 통째로 옮기는 종전 동작을 유지).
