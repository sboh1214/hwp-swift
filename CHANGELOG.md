...
- **한글 2007 호환 문서의 밑줄·취소선 선 모양은 한컴오피스 한글처럼 글자 크기와 무관한 고정
  무늬로 그립니다** (#227). 한글은 이 모드(HWP의 호환 문서 대상 프로그램 1, HWPX
  `targetProgram="HWP200X"`)에서 점선·원형 점선·여러 줄·물결도 선 두께(고정 0.36pt, #210)처럼
  크기와 무관한 고정 pt로 그립니다 — 위 #191 항목의 글자 크기 비례 무늬를 그대로 쓰면 40pt 긴
  점선의 한 토막이 11.40pt(한글 2.40pt), 2중선 간격이 3.6pt(한글 1.08pt)가 됩니다. 한컴오피스 한글
  12.30이 내보낸 PDF로 16개 모양의 글자 아래 밑줄 5~100pt, 글자 위·가운데 밑줄·취소선 5~80pt와
  크기가 섞인 줄·상대 크기·첨자·글꼴 넷을 합친 표본 493개를 재어 규칙을 확정했습니다 — 대시 단위
  0.48pt(긴 점선 2.40/1.44pt, 점선 0.48/0.72pt, 일점쇄선 4.80/1.44/0.48/1.44pt), 원형 점선은
  지름 1.32pt 원이 3.0pt 간격으로 띠가 자라는 쪽으로 0.24pt 옮겨진 자리, 2중선은 0.36pt 두 줄(중심
  간격 1.08pt), 가는+굵은·굵은+가는·3중선은 4.2pt 띠에 0.96·2.28pt와 0.6·1.8·0.6pt, 물결은
  진폭 2.88pt·획 0.72pt, 2중 물결은 진폭 1.44pt·획 0.36pt·둘째 파 1.08pt 아래입니다. 무늬가 글자
  모양마다 다시 시작하고 한 글자 모양 안에서는 슬롯마다 상대 크기가 달라도 이어지는 것은 한글
  문서와 같습니다. 기하는 `HwpLineShapeGeometry.Scale.hwp200XCharacterLine`과
  `HwpRenderTuning.LineShape`의 `hwp200X*` 상수입니다. `hwp2007-decorations` 쌍(HWP + HWPX)을
  다시 저장해 선 모양 10표본을 더하고 한글 PDF의 조각 길이·선 자리·두께를 픽셀로 잠갔습니다 —
  우리 PDF를 같은 방식으로 읽으면 열 표본 모두 한글과 0.12pt 안입니다. 이 쌍은 이제 2쪽입니다.
...
- **줄 상자가 링크 글자보다 높은 줄에서 링크 밑줄이나 글자와 밑줄 사이를 눌러도 링크가 열리지
  않던 문제를 바로잡았습니다** (#233). 링크 클릭 영역의 세로 범위가 글꼴의 ascent·descent여서,
  10pt 링크와 한 줄에 40pt 문단 끝 문자·기본 크기 40pt 책갈피·높이 40pt '글자처럼 취급' 그림이
  있으면 줄 상자 바닥에 붙는 밑줄(#226)과 그 위 빈칸이 영역 밖이었습니다. 한컴오피스 한글
  12.30.0(macOS) 편집 화면에서 링크를 눌러 잰 규칙으로 바꿨습니다 — 링크가 열리는 세로 범위는
  **줄 상자 상단부터 그 줄의 줄 간격 몫까지**(다음 줄 상자 상단까지)이고, 문단 위·아래 간격은
  어느 줄의 것도 아니며, 표 셀·글상자·각주의 마지막 줄은 줄 상자 바닥에서 끝납니다. 줄 간격이
  100%이거나 고정값이 줄 상자보다 작으면 그 줄 아래의 밑줄은 다음 줄 몫입니다. 다음 문단 첫 줄의
  글자가 제 줄 상자 위로 솟아 앞 줄 링크의 띠를 덮어도 그 자리는 앞 줄 링크가 열립니다 — 한글은 줄
  띠로 판정하고 다음 줄의 글자를 보지 않습니다(상대 크기 200%·150% 줄로 실측). 쪽·문서의 마지막
  본문 줄은 한글이 줄 상자에서 끊는 것과 달리 줄 간격 몫까지 열립니다(그 아래는 빈 자리입니다 —
  줄 간격이 줄 상자보다 작으면 줄 상자까지). 글자 위치로 옮겨진 글자의 영역은 종전처럼 그 글자가
  그려진 자리를 따릅니다. 한글.app이 저장한 `hyperlink-click-band` 쌍(HWP + HWPX, 2쪽 29개
  링크)을 픽스처로 추가해 링크마다 영역을 문서의 줄 배치 정보와, 클릭 결과를 한글 편집 화면
  실측과 대조했습니다. 각주는 문단마다 블록 하나라 그 블록이 각주의 마지막 항목인지를 알리는
  `HwpFootnoteBlock.isNoteEnd`(기본값 `true`)를 더했습니다.
- **같은 줄의 밑줄 높이가 글자 크기마다 달라지고, 상대 크기를 준 글자의 밑줄·취소선이
  ...
  무늬는 한글 문서에서 한 글자 모양 안의 슬롯마다 상대 크기가 달라도 끊기지 않고 이어집니다. 키 큰 개체
  ...
  25.5pt 어긋났습니다. 한글 2007 호환 문서의 실선이 아닌 선 모양은 크기와 무관한 고정 무늬라
  이 항목의 줄 단위 축척을 따르지 않습니다 — Added의 #227 항목.
...
- **한글 2007 호환 문서의 밑줄 위치와 장식선 두께가 한컴오피스 한글 출력과 다르던 문제를
  ...
  픽셀로 잠갔습니다. 크기가 섞인 줄에서 밑줄을 줄 단위로 놓는 축은 위 #226 항목, 실선이 아닌
  선 모양은 Added의 #227 항목에서 다룹니다.
