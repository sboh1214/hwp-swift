...
- **줄글의 베이스라인이 한글보다 1~2pt 낮게 그려지던 것을 바로잡았습니다** (#178).
  ...
  **슬롯이 균일한 것이 관찰되는 문단은 복원식을 타지 않습니다** — 줄 상자 높이·보고 descent·
  줄 원점 간격이 모두 같고 글자처럼 취급 개체가 없으면 CoreText 슬롯도 같으므로, 첫 줄의
  정확한 값을 모든 줄에 씁니다. CoreText가 **양쪽 정렬** 줄의 지표에 강제 줄 높이를 적용하기
  전 값을 담기 때문입니다(하한 15pt·10pt 글자에서 보고 descent 2.2998, 실제 4.0) — 한글
  문단은 기본이 양쪽 정렬이라 이 갈래가 실물을 받습니다. 덤으로 아주 긴 문단이 여러 조판
  프레임으로 갈릴 때도 자리가 같아집니다.
