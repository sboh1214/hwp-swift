...
(2026-07-06 사용자 실측 피드백 반영) 표 셀 안 그림·도형·글상자는 셀 콘텐츠
(표-로컬 rect)로 배치되어 페이지 흐름을 소비하지 않는다 (noori 3쪽 정합).
셀 문단 전부가 라인 캐시로 측정된 셀은 저작된 셀 높이 (표 80)를 신뢰하되,
문단 기준으로 떠 있는 개체 높이를 하한으로 얹는다 — 그 개체는 줄 상자에 없어
캐시에도 저작 높이에도 잡히지 않는다 (#91: 얹지 않으면 noori 3쪽 붙임 표가
선언 627.0pt의 29%인 181.6pt로 접히고 셀 그림이 아래 행들을 덮는다).
