...
- **`DataReader`** — `Data` 위의 cursor. `read(T.Type)`은 정수 폭(1/2/4
  byte)으로 분기하며, 그 외 타입은 `preconditionFailure()`. `readBytes`는
  throw가 아닌 precondition 사용 — 범위 초과는 프로그래머 오류.
...
실제 크기. child는 `level`로 중첩된다 —
`for _ in 0..<level { parent = parent.children.last! }`. root record는
