...
HWP 5.0 바이너리 문서(.hwp)와 HWPX 문서(.hwpx)를 읽는 Swift 파서입니다.

## Overview

CoreHwp는 OLE 컨테이너의 FileHeader·DocInfo·BodyText 등 주요 스트림을
해석해, 한컴이 공개한 ‘한글 문서 파일 형식 5.0’의 레코드 구조를 바탕으로
모델 트리를 구성하는 읽기 전용 파서입니다. HWPX(OWPML, ZIP+XML) 문서는 같은
진입점이 선두 바이트로 자동 감지해 동일한 모델로 변환 파싱하므로, 호출 측은
두 포맷을 구분할 필요가 없습니다. Apple 플랫폼뿐 아니라 Linux에서도
