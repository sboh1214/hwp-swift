...
모양·스타일·구역/쪽 설정·단·표·그림·쪽 번호 위치·머리말/꼬리말(#167)·각주/미주
(#168)·새 번호/쪽 감추기/책갈피/찾아보기 표식(#169)·OLE 개체(내장 차트)·문단
번호와 글머리표 정의(라벨 생성·렌더는 HwpKitCore #153·#154)이고, 그 밖의
요소(도형·수식·홀짝 조정 등)는 실제 4CC를 실은 `.notImplemented`와 합성 tagId(0)의
`unknownRecords`로 강등되어 `parseDiagnostics()`에 보고됩니다. 같은 문서의
HWP↔HWPX 파싱 등가는 `Tests/CoreHwpTests/FixtureHarness/Hwpx/`의
`HwpxHwpEquivalenceTests`가 실물 변환 쌍으로 고정합니다 (쌍 수는 하한으로만 두고
목록은 `HwpxFixtures/`가 정본).
...
| 셀 속성 (표 80) `height` | **저장값이 실제 셀 높이가 아닐 수 있다.** 한컴오피스 한글 12.30 macOS(build 6446)는 `표 > 표 만들기`로 만든 표의 셀 높이를 내용과 무관하게 **위아래 안쪽 여백 합(141 + 141 = 282 HWPUNIT)**으로 저장한다 — 2026-09-07 실측: 2×2 표의 1줄·4줄·2문단·빈 셀 4개 전부 282(HWP·HWPX 동일), 화면 행 높이는 줄 캐시 마지막 줄 아래 + 여백(6082·2882 HWPUNIT ≈ 60.7·29.0pt 실측)이고 표 공통 속성 `height` 8964가 그 합이다. `numbering-sequence` 쌍(같은 앱 저장)도 282. 윈도우 저장본(noori)과 헌법주석은 사용자가 정한 높이(2348·1000 등)를 담되 내용보다 작을 수 있다(헌법주석 s24/p490: 1000 < 2줄 1108 = 표 공통 속성 1108). 셀 문단 줄 캐시의 `lineLocation`은 셀 안에서 문단을 넘어 누적된다(2문단 셀 0·1600). 뷰어는 저작 높이를 `줄 캐시 상자 + 여백`의 하한으로 보정한다 (#160, `Sources/HwpKitCore/AGENTS.md` "캐시 높이 우선") |
