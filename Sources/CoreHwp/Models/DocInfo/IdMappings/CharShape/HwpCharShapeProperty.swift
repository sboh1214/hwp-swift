...
/// 값 이름의 정본은 한컴 공개 OWPML 모델의 `g_UnderlineTypeList`
/// (`ULT_NONE`·`ULT_BOTTOM`·`ULT_CENTER`·`ULT_TOP`)다 — 스펙 표는 값 2를
/// 빠뜨렸다 (#136).
public enum HwpUnderlineType: Int, HwpPrimitive {
    ...
    case under = 1
    /**
     2: 글자 가운데 — 스펙(표 33)의 표에는 빠져 있지만 한컴 공개 OWPML 모델이
     `ULT_CENTER = 2`로 못박은 값이다(pyhwp `LINE_THROUGH`, hwplib `Middle`).
     취소선과 **같은 선**이라 hwplib은 "취소선만"을 취소선 비트 + 이 값으로
     쓰고 읽는다(`StrikeUnderLineMode.OnlyStrike`). `CharShape`·
     `CharShapeProperty` 픽스처의 취소선 견본이 그 조합이고, 한글.app은 HWPX로
     재저장할 때 밑줄 없음(`type="NONE"`) + `<hh:strikeout>`으로 접는다.
     렌더도 취소선과 한 줄로 합류시킨다 (#136 실측).
     */
    case center = 2
