...
└── Ctrl Header/                    # HwpCtrlId로 dispatch되는 컨트롤 payload
...
[`Enums/Ctrl Id/Hwp{Common,Other,Field}CtrlId.swift`](file:///Users/sboh/Repos/hwp-swift/Sources/CoreHwp/Enums/Ctrl%20Id/)와 매칭되어
`HwpCtrlId` enum case로 들어간다. 모델링되지 않은 컨트롤은
`HwpCtrlId.notImplemented(HwpCtrlHeader)`로 떨어진다 — **throw 금지, assert
금지**. 그대로 보존해야 라운드트립이 깨지지 않는다.

새 컨트롤 추가 절차:

1. 4-byte ID를 `Enums/Ctrl Id/`의 알맞은 파일에 추가.
2. `Ctrl Header/` 하위에 payload struct 추가 (하위 record가 있으면 전용
   subdirectory).
...
- 폴더명은 한컴 공개 문서의 절 제목을 그대로 사용한다 — `Ctrl Header/`,
  `SectionDef/`, `Table/`, `Column/`. 변경 금지.
...
- 모르는 컨트롤에 대해 `invalidCtrlId`를 throw — `.notImplemented`로
  떨어뜨릴 것. throw 하면 미모델 컨트롤이 포함된 모든 픽스처가 깨진다.
- 단락 개수 하드코딩 — 구역(section)의 단락 배열은 가변. index 대신 iterate.
- `Ctrl Header/` 밖에 컨트롤 로직 추가 — dispatch는 `HwpParagraph`에,
  payload는 여기에.
