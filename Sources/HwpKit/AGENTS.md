...
- `HwpDocumentLoader(fontResolver:)` — 렌더에 쓸 폰트 해석기 주입 (기본 `HwpFontResolver()`). 커밋된 기준선을 쓰는 렌더 가드가 전부 이 인자 하나에 기대 기기 독립을 얻는다 (`.testDeterministic` — 루트 AGENTS.md "렌더 가드 다섯 층"). 호스트도 재현 가능한 조판이 필요하면 같은 방식으로 고정 resolver를 넘긴다
- `HwpDocumentLoader.load(from:)` — URL / Data / FileWrapper 오버로드. **`.hwp` 와 `.hwpx` 를 구분 없이 받는다** — 포맷 감지는 CoreHwp `HwpFile` 의 선두 바이트 스니핑이라 로더에 포맷 인자가 없다. 확장자로 갈리는 분기를 새로 만들지 말 것 (호스트도 확장자를 볼 필요가 없다). 대신 파일 선택기·드롭에서 `.hwpx` 를 **고를 수 있게** 하는 것은 호스트 몫이다 — 아래 "Sample 앱". 내부적으로 `HwpDocumentActor` 사용. 오류는 `HwpDocumentLoadError`로 매핑 (`CustomStringConvertible` + `LocalizedError` 채택 — 오류 설명은 한국어이므로 호스트가 `error.localizedDescription`을 그대로 표시하면 된다. `presentationBuildFailed`는 하위 파서나 페이지네이터가 보고한 원인을 `reason`에 원문 그대로 보존한다. 새 케이스를 추가하면 `description`도 함께 채우고 `HwpDocumentLoaderTests.testErrorDescriptionsCoverEveryCase`의 배열에도 넣을 것) (#117)
...
**`.hwpx` 를 여는 데 필요한 것은 호스트 배선 세 곳이다.** 로더는 포맷을 모르지만 (위 `load(from:)`) 선택기·드롭은 **콘텐츠 타입**으로 거르므로, 선언하지 않으면 `.hwpx` 가 선택기에서 회색으로 비활성화된다 — 파서가 읽을 수 있는데도 사용자가 고를 수 없다. ① `project.yml` 의 `UTImportedTypeDeclarations` 에 `dev.sboh.hwpx`(확장자 `hwpx`) + `CFBundleDocumentTypes` 항목, ② `fileImporter(allowedContentTypes:)`, ③ 드롭 `acceptedTypes`. **`public.zip-archive` 에는 적합시키지 않는다** — 컨테이너가 ZIP 이어도 그렇게 선언하면 iOS Files 가 `.hwpx` 를 아카이브로 보고 탭 시 압축을 풀고 macOS '다음으로 열기' 후보에 Archive Utility 가 낀다 (시뮬레이터·프로브 앱 실측). docx/epub·한컴 자신의 hwpx UTI 처럼 문서 UTI 는 `public.data` 만 적합시키고, ZIP 판정은 파서의 바이트 스니핑에 맡긴다. 드롭 경로는 그 선언만으로 부족하다: 한글.app 이 깔린 Mac 에서 두 확장자 모두 `com.haansoft.hancomofficeviewer.mac.*` 로 풀려 앱 UTI 에 적합하지 않으므로 (실측), 넓게 받고 **확장자로 검증**하는 기존 정책이 `.hwpx` 까지 그대로 간다 (`DropOpenSupport.openableExtensions` 가 그 집합의 단일 출처이고, 걸린 후보의 실패 사유는 `.notSupportedType` 이다).
...
**iOS 최근 문서 접근은 재부팅을 넘지 못한다** (#126 리뷰) — `.withSecurityScope` 가 iOS 에서 `API_UNAVAILABLE` 이라 옵션 없이 만들 수밖에 없고, 그렇게 만든 북마크가 자동으로 다는 implicit ephemeral security scope 를 SDK 가 **"valid until reboot at the latest"** 로 못박는다 (`NSURL.h` 의 `WithoutImplicitSecurityScope` 항목). 함정은 실패 모양이다: 재부팅 뒤에도 북마크는 **URL 로 멀쩡히 풀리므로** resolve 성공만으로 살아 있다고 보면 눌러도 매번 실패하는 행이 목록에 남는다 (`startAccessingSecurityScopedResource` 의 false 는 판별에 못 쓴다 — 컨테이너 안 URL 도 false 를 준다). 그래서 `RecentDocumentsStore.resolve` 가 범위를 쥔 채 읽기 가능 여부까지 보고 `.inaccessible` 을 따로 낸다. **이 지적은 리뷰에서 두 번 올라왔고, 한 번은 "Apple 이 iOS 에서 일반 북마크를 권장한다"는 포럼 근거로 잘못 기각됐다** — 포럼 서술은 부팅 세션 안에서 참이고 경계를 말하는 것은 위 SDK 문장이다. 기각하려면 그 문장부터 반박할 것.

사이드바 상태는 **직교하는 두 값**이다 — 어느 축을 고르고 있는가(`SidebarMode`)와 지금 보이는가(`Bool`), #76. 불리언을 축마다 두면 "개요와 축소판이 둘 다 켜짐"이라는 없는 상태가 생겨 macOS 인라인 열과 iPhone 시트에서 서로 다르게 깨지지만, 반대로 `SidebarMode?` 하나로 접으면 **감출 때 고른 축이 사라져** 다시 열 때 다른 축이 된다. 개요를 골랐지만 그 문서에 개요가 없으면 축소판으로 **대신 그린다** — 개요가 없는 문서에서 사이드바가 통째로 사라지던 것이 그 축을 추가한 이유이고, 사용자의 선택을 덮어쓰지는 않는다(개요가 있는 문서를 다음에 열면 다시 개요다). 축소판 렌더러(`HwpPageThumbnails`)는 **호스트가** 소유한다: 사이드바 뷰가 소유하면 모드를 토글하거나 시트를 닫을 때마다 그때까지 그린 축소판을 통째로 버린다. 그렇게 오래 사는 만큼 **문서를 버릴 때 `update(document: .empty)`로 교체**해야 한다 (#76 리뷰) — `cancelOutstanding()`은 요청만 끊고 보유는 유지하므로, 새 로드가 첫 스냅샷을 내기 전에 실패하면 옛 문서(쪽·공급자·디코드 이미지·축소판)가 오류 화면 내내 상주한다.
