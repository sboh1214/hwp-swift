...
///
/// **`@State`를 하나도 두지 않는다.** 그래야 `HwpToolsTests` 관례(뷰 인스턴스를
/// 만들어 body 밖 메서드를 직접 부르는 방식)로 검증된다. 질의는 컨트롤러가
/// 소유하고 포커스는 호스트가 준다. (`HwpPageNavigator`의 쪽 번호 입력 초안은
/// 예외다 — 커밋 전 중간 값이 바인딩에 새면 안 되고 그것을 들 컨트롤러가 없다.
/// 판정 로직은 여전히 초안을 인자로 받는 메서드라 같은 관례로 검증된다.)
///
/// ```swift
