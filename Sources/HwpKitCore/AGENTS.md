...
- **자리 차지 표는 문단 글줄 앞 띠가 우선이다** (#161, `appendBandedTableIfNeeded`).
  ...
  **대상은 `topAndBottom` 하나다** — `consumesFlow`로 게이트하면 어울림(`square`)과
  ...
  규칙이 성립하는지 실측한 적이 없다.
  **저작 문단 위 간격은 띠가 아니다** — 절대 캐시 배치는 `beforeGap`을 커서로 소비하지
  않고 캐시 줄 위치에 이미 담고 있어서, 빼지 않으면 문단이 의도한 여백에 표가 들어간다.
  띠는 그 간격 **뒤**에서 시작한다(`authoredSpacingTop`, 산식은 `authoredBeforeGap`
  하나를 흐름 배치와 공유한다). 간격이 있는 문단에서 표가 간격 앞·뒤 어디에 놓이는지는
  미실측이고, 실물(`numbering-sequence`)은 간격 0이다.
