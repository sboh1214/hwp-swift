...
- **흐름 배치 문단의 자리 차지 표는 글줄 앞에 흘린다** (#190, `appendTablesPrecedingText`).
  ...
  확정하면 1·2·9). **표보다 앞선 쪽 장식(머리말·꼬리말·쪽 번호 위치·감추기)은 표를
  놓기 전에 등록한다**(`registerPageChromePreceding` → `chromeRegisteredBeforeText`, 글줄 뒤
  방출은 그 서수를 건너뛴다) — 종전 순서(서수 순 등록 뒤 표 흐름)를 지켜 표가 넘긴 첫 쪽부터
  장식이 실린다(PR 리뷰: 쪽 번호 컨트롤 뒤 10행 표에서 첫 쪽 번호가 빠지고 둘째 쪽이 `- 2 -`).
  표 뒤의 장식은 종전대로 글줄 뒤 방출이 등록한다. **여백이 빈 단마저 소진하면 여백을
  ...
  **빈 단 높이**(`currentColumnFrame.height`, 이 쪽의 각주 예약을 빼지 않은 값)이고 —
  ...
  이월한 쪽의 예약이 여백까지 못 담으면 남은 조각은 여백 없이 흘린다.
