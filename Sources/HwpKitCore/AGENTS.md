...
1000·850·1200, 표·도형·바깥 여백(20 + 7 + 3 → 3000)·상대 크기 50% 마커·셀 안도 같고 줄 간격 고정 30 →
...
글자와 개체가 함께 내려갔고(K3 17pt·K7 25.5pt), 한글이 저장한 캐시보다 높게 잰 줄이 낡은 캐시로
판정돼 버려지면서 뒤 문단까지 누적됐다. MS 워드 호환 문서는 원래 마커 글꼴을 상자에서 뺐다 (#194,
`msWordSpacingBase`).
...
(`HwpPaginator.combinedAttributedString`)은 문단마다 자기 스타일이 붙어 있으므로 줄 자신의
...
표 셀·글상자·머리말/꼬리말 안은 남았다), 남은 것은 **개체를 실은 마커 run의 글자 모양이 본문보다 클 때의 줄 상자**(한글은 그 크기를 `vertsize`에 넣지
않고 비율 여분 기준에만 넣는다 — 10pt 글 + 40pt 마커의 20pt 그림 줄이 `vertsize` 20·`spacing` 24;
폭 0 마커(책갈피)의 40pt는 `vertsize`에도 든다, #217)이다. ⑤ 상자보다 큰 글자의 잉크가 블록 프레임 위로 나가는데 최상위 텍스트 블록의 히트 자격에는
...
글자 크기이고 표를 담는 줄)에서는 발동하지 않는다 — 헌법주석 페이지 절단 유지. 캐시의 쪽 절단점은
