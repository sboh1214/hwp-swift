...
- **각주/미주도 표 셀·글상자와 같은 컨테이너다** (#94). `HwpFootnoteBlock`이
  ...
  **표 셀은 반대다** (R48): 셀 생산자 (`HwpTableLayout`) 가 평면·정렬 키를 채우지
  않아 전부 기본값이므로, 셀 페인터 (`walkTable`) 는 셀 표를 정렬에서 빼고 모든
  개체 **뒤에** 그린다 — 셀 히트도 그 순서를 따라 표를 최상단으로 먼저 본다.
