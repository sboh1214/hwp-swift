...
판정하고, 줄 간격 여분과 문단 아래 간격은 쪽·단 아래로 넘쳐도 된다. 방출 높이(블록 높이·커서
...
- **절대 캐시 모드의 각주는 본문 아래 자리에 한글의 이어짐 규칙으로 싣는다** (#165,
  ...
  **없는** 각주(흐름 조판 문서·캐시를 버린 HWPX)와 개체를 담아 낡은·쪽 걸침 캐시 대신 CT 높이를 지키는
  각주 — 도 같다: CT 프레임의 마지막 줄 상자 아래 몫(`HwpParagraphMeasurer.Result.trailingGap`)을
  뺀다(#222, 한글 12.30 실측 `probes/222` FN: 10pt·160% 각주 한 줄의 상자 바닥이 본문 하단에 닿고
  영역은 8.5 + 5.67 + 10 = 24.17). 종전에는 0을 빼 영역이 6pt 커졌고 구분선이 그만큼 위였다. ④ 통째로 안 들어가는 각주는
