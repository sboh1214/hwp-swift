...
- **절대 캐시 모드의 각주는 본문 아래 자리에 한글의 이어짐 규칙으로 싣는다** (#165,
  ...
  **개체를 담은 각주는 나누지 않는다** (`NoteMeasurement.carriesObjects`): 개체는 앵커
  ...
  (전 경계를 보간하면 같은 줄 수인 문단의 조각까지 줄 중간에서 갈린다). 그래도 앞 조각이 글을
  다 그렸으면 남은 캐시 줄은 넘기지 않는다 (`appendHead` — 빈 블록을 싣지 않는다).
  **이월의 비용은 쪽 수에 비례해야 한다** (PR 리뷰, `HwpFootnoteLayout.SourceLayout`):
  ...
  문단까지다 (문단 안 리셋만 보면 문단 경계에서 갈리는 각주가 남은 문단 전부를 예약한다). 결과 200개 3.66s·800개 13.0s·1,600개 25.9s (선형). **각주 하나의
