...
- 장식 선의 세로 위치와 **두께**는 **글꼴 지표가 아니라 글자 크기 비례**다 (#136·#176
  실측: 한글.app 12.30 PDF 내보내기, 5~100pt·함초롬바탕/돋움·Apple SD·HY울릉도M 네
  글꼴에서 같은 값). 취소선 +0.35em · 글자 위 밑줄 +0.87em · 글자 아래 밑줄 −0.17em ·
  두께 0.04em(한글 PDF는 0.12pt 장치 단위로 `0.12 × round(크기/3)`)이
  `HwpRenderTuning.Text`에 있고 하한은 없다(5pt는 0.2pt). 밑줄 종류 raw 2(글자
  가운데)는 취소선과 같은 선으로 합류시킨다
- **변경 추적 표시선의 상수(삭제선 +0.29em·삽입 밑줄 −0.26em·두께 0.065em)는 변경
