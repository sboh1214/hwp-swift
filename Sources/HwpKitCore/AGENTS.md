...
- **절대 캐시 모드의 각주는 본문 아래 자리에 한글의 이어짐 규칙으로 싣는다** (#165,
  ...
  구분선이 커진 글자 위에 그어진다, PR 리뷰) − 구분선 위·아래 여백. 선 두께는 안 센다 — 선은
  ...
  `0, 0, 1172`) 이 있고 첫 줄이 들어가면 그 앞 몫만 싣고 나머지를 다음 쪽 **첫** 각주로
  잇는다 (`Input.placedLineCount`, 구분선은 다시 긋고 번호는 반복하지 않으며 이어지는
  줄은 내어쓰기 자리 — `continuationFragment`). 앞 조각은 다른 분할 경로(표 셀·다단
