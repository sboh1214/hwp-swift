...
- **입력 쪽 일도 유계다.** 형식 분해(`HwpNumberingFormat.pattern`)는 부를 때마다
  형식 전체를 다시 훑으므로 정의·수준마다 한 번만 하고 메모한다
  (`HwpNumberingPatternCache` — 안 그러면 65,535단위 형식 × 20,000 문단 ≈ 13억
  스칼라를 출력 상한이 걸리기 **전에** 쓴다). 순회는 `HwpPaginator.init`의 동기
  경로라 조판의 쪽 단위 지연·문단 단위 취소 관찰 밖이다 — 그래서 걷는 문단 수에
  `maximumVisitedParagraphs`(500,000 — 항목 상한은 번호 문단에서만 줄어들어 번호
  없는 문단 수백만 개는 막지 못한다)를 두고, 256문단마다 `Task.isCancelled`를 봐
  취소된 로드는 걷다 만다(둘 다 `isTruncated`). init에 두는 이유는 #154의 조판
  문자열 조립·복사·접근성이 actor 밖 동기 코드라 `nonisolated let`으로 읽어야
  해서다 — 옮기려면 그 동기 접근부터 다시 설계해야 한다.
