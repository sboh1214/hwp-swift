...
1120 = 16 × 0.7인데 우리는 600, #206). 마지막 글자 모양이 다른 문단에서만 드러난다. ② 글상자·표
셀·쪽 번호 크롬 안 텍스트는 컨테이너 자체의 세로 자리가 한글과 달라 (noori 1쪽 제목 글상자
−5.35pt·표 셀 −1.3 ~ −2.6pt·쪽 번호 −31.7pt) 앵커를 맞춘 뒤 그 격차가 제 크기로 드러났다
(#193). ③ MS Word 호환 문서 (`targetProgram` 2) 는 줄 상자 모델 자체가 다르다 —
...
간격)를 배치와 같은 순서로 쌓고 마지막 줄의 줄 간격만 뺀 값, 즉 배치가 그리는 마지막
줄 상자의 아래)보다 작으면 그 값으로 올린다 (#160). 한컴오피스 한글 12.30 macOS는
