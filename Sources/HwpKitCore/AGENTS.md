...
(`HwpObjectAnchorGeometry.inlineAnchorOrigin`)도 이 모델이다: 개체 바닥을 그 줄의 앵커에 두되
개체가 앵커 위 공간보다 크면 상자 상단에 붙는다 — 개체가 상자를 정한 줄(코퍼스의 전부)은
개체 상단 = 상자 상단이고 렌더가 그 줄 글자를 그리는 자리와 같은 기준이다. 헌법주석 665쪽
...
읽는다 (아래 장식 항목의 #194 절). ④ 상자보다
**작은** 글자처럼 취급 개체의 세로 자리는 한글 실측 전이다 (#195 — 지금은 개체 바닥을 앵커에
둔다). ⑤ 상자보다 큰 글자의 잉크가 블록 프레임 위로 나가는데 최상위 텍스트 블록의 히트 자격에는
