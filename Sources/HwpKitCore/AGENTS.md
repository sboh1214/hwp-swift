...
구역 — 첫 정의 문단에서 카운터를 다시 비우지 않는다). 시작 번호 해석은 CoreHwp
`HwpNumbering.continuesPreviousList`·`startingNumber(forLevel:)`(정의 전체 시작
번호 0 = 앞 목록/앞 구역에 이어, ≥1 = 새 번호; 수준별 값은 65,535로 접는다 —
근거는 그 파일의 doc-comment)이고, 카운터(`HwpNumberingCounter`)는 **종류마다
따로** 돈다 — 개요는 **구역 시작**에서, 번호 매기기는 **정의가 바뀌는 번호
문단**에서 새 정의가 새 번호면 비우고 이어 매기기면 잇는다. 번호 매기기 카운터는
**구역 시작을 보지 않는다**(도움말의 "앞 번호 목록에 이어" = 문서 순서로 가장
가까운 번호 문단을 잇는다). 상위 수준을 매기면 하위는 비워지고, 비워진 수준을
형식이 참조하면 시작 번호를 보인다(한글 대화상자 미리보기 규약). `^n`·`^N`은
1수준부터 그 수준까지를 각 수준의 번호 모양으로 잇는다
(`HwpNumberingLabelFormatter` — `outline-numbering` 둘째 정의의 9·10수준으로
한글.app 실측 문자열 `I.가.1.가.1.가.①.㉮.ㄱ)`·`ㄱ.I0)`을 핀). 표 셀·글상자·각주·
미주·머리말/꼬리말 안 문단(`childParagraphs(of:)`가 여는 컨테이너 전부)도 품은
문단 뒤에 같은 카운터로 센다(실물 대조 전 — 아래). 오라클 셋: 헌법주석의 **생성
목차**(구역 0 문단 50-377, 1수준 표제 280개의 `I.`… 가 조문(구역)마다 다시
시작 — 41개 구역 정의 중 첫 것만 시작 번호 0이고 40개가 1; 2-7수준 1,664개는
외부 근거 없이 스냅샷으로만 잠근다), 한글.app 저장본 `outline-numbering`의
PrvImage 라벨, 1,944개 전체의 커밋 스냅샷
`Tests/HwpKitCoreTests/ParagraphNumberingSnapshots/`(`RECORD_NUMBERING_SNAPSHOTS=1`).
**한글.app 실측이 남은 것** (규칙은 도움말 추론이고 실물은 모순되지 않을 뿐이다):
① 시작 번호 0인 정의가 번호 문단·구역 **뒤에** 올 때 실제로 잇는지(실물의 0은
전부 첫 구역·첫 목록이라 "구역·정의가 바뀌면 늘 다시 셈"과 구분되지 않는다 —
현재 규칙을 가르는 실측), ② 새 번호 N이 정의 전체 시작 번호와 수준별 시작 번호 중
어디에 적히는지(둘 중 큰 값을 써 두 인코딩을 모두 덮는다), ③ 같은 정의를 두
구역이 가리킬 때의 재시작, ④ "새 번호 목록 시작"이 새 정의를 만드는지·같은 새
번호 정의로 되돌아올 때 다시 세는지, ⑤ 개요와 번호 매기기가 같은 정의를 가리킬
때 카운터를 공유하는지, ⑥ 컨테이너(표 셀·글상자·각주·머리말/꼬리말) 안 문단이
본문 카운터를 잇는지, ⑦ 2수준 없이 3수준이 올 때 다수준 형식이 보이는 값,
⑧ 도움말의 셋째 방식 "이전 번호 목록에 이어"의 저장 형식(구분 못 함 — 이어
매기기로 읽힌다). 2026-09-06 GUI 접근이 거부돼 그 실측과 번호 매기기·시작 번호
지정 픽스처 쌍은 미뤘다.
