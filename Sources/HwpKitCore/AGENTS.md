...
⑥ 전진량과 별개로 **1단 본문 문단은 쪽 끝에서 줄 단위로 나뉘지 않고 통째로 다음 쪽으로
간다** (`placeFlowParagraph`는 문단이 한 쪽보다 클 때만 분할) — 한글은 들어가는 줄까지 쪽 끝에
둔다 (#207, 합성 실측: 3줄 문단의 첫 줄만 들어가는 자리에서 한글은 첫 줄을 남기고 우리는
문단 전체를 옮겨 뒤 문단이 16pt 밀린다).
