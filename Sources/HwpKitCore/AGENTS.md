...
- **절대 캐시 모드의 각주는 본문 아래 자리에 한글의 이어짐 규칙으로 싣는다** (#165,
  ...
  구분선 여백·길이는 **쪽**의 몫이라 그대로 현재 구역 모양을 쓴다. 경계가 줄 중간에 떨어지면 그 줄의 앞부분을
  ...
  **개체를 담은 각주는 나누지 않는다** (`NoteMeasurement.carriesObjects`): 개체는 앵커
  ...
  (`hasFloatingObject` 상위집합 술어) 하고, **예약도 같은 범위를 본다** — 개체를 담은
  각주의 문단은 그 문단에 개체가 없어도 캐시 빠른 길(`measuredFootnoteTextHeight`)로 가지
  않는다 (가면 예약이 캐시 합, 배치가 CT 높이로 갈린다). 여러 문단짜리 각주의 뒤 문단이 0에서 다시 시작하면 그
