...
- 가드: `HwpHyperlinkClickBandTests`(합성 11건 — 이슈의 세 표본·줄 경계·문단 간격·고정/100%·음수
  ...
  3건 — 뒤 블록의 프레임 위 글자 claim: 상대 크기·Menlo ascent·각주 문단, 개체 칠은 claim 유지),
  ...
  되돌리면 합성 11건 중 10건(흐름 스태커 판정만 남음)·claim 3건·픽스처 2건 전부, 목록 끝을
  무시하면(`.end` = 전진량) 합성 4건·픽스처 2건, `.unknown`을 전진량만으로 두면 합성 1건·픽스처 띠
  1건, claim 양보를 끄면 claim 3건·픽스처 히트·#200 테스트가 빨개진다.
