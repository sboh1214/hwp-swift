...
- **선 모양(표 25)은 `HwpLineShapeGeometry` 한 곳이 채우기 경로로 푼다** (#191, 한글 12.30
  ...
  **축척이 두 갈래다**: 글자선은 글자 크기(대시 단위 0.057em·2중선 띠 0.12em·그 밖 여러 줄
  ...
  0.8 진폭 아래 / 테두리 (t/4, 3t/4)로 옮겨 겹친다. 물결은 45° 지그재그이고 꼭짓점 사이
  0.12pt 평탄(`waveVertexFlat`)은 한글의 600dpi 정수 좌표 부산물인데 주기를 맞추려 둔다.
  ...
  - **테두리는 셀 모서리에 중심**(양쪽으로 t/2, 종전 셀 안쪽 띠가 아니다)이고 공유 모서리는
    ...
    남는다). 가로 변은 이웃 세로 변이 있는 끝만 그 폭의 절반 연장, 세로 변은 연장 없음,
    여러 줄은 부속선마다 바깥쪽에서의 거리만큼 물러나 모서리에서 겹상자 (`HwpBorderSet.Edge`).
    히트 띠(`EdgeGeometry.band`)는 점선·물결의 빈 자리도 든다. 페인트는 변마다 `.drawPath`
    하나 — 조각이 많은 모양도 명령 하나다.
  - 단 구분선은 밴드가 닫힐 때(`closeColumnBand`·`cacheCurrentPage`, `dividersEmitted`로 한
    번) 단 사이 간격 중앙에 `.shape`(`.pageChrome`) 블록으로 낸다. 세로 범위는 밴드 첫 줄
    위에서 가장 긴 단의 마지막 줄 **글상자 아래**(밴드 바닥이 본문이면 `bandTrailingLineSpacing`
    을 뺀다, 표면 그대로)까지이고 둘째 단이 비어도 그린다.
