...
**작은** 글자처럼 취급 개체의 세로 자리는 바깥 상자를 글자로 보는 위 규칙으로 고쳤다 — 남은 것은
내용이 저작 높이보다 키운 표(한글은 실제 높이의 0.85, 우리는 예약한 저작 높이 기준, #218)와
**개체를 실은 마커 run의 글자 모양이 본문보다 클 때의 줄 상자**(한글은 그 크기를 `vertsize`에 넣지
...
- treatAsChar + 문단 라인에 U+FFFC 앵커 존재: 그 줄 위치에 인라인 배치.
  ...
  **표도 포함** (`appendInlineAnchoredTable` — noori 보도자료 표 실측: 캐시 줄
  높이 = 표 높이 + 위·아래 바깥 여백, 표는 왼쪽·위쪽 여백만큼 안 — #193). **개체가 자기
...
- **조각의 줄 앵커** (#164): 조각 블록은 `appendBlock(anchorLines:)`로 **조각 기준**
  ...
  **같은 사본**을 준다 (`placedFragment`). 다시 풀 때는 **마커 한 글자씩** 훑는다 —
  ...
  (종이·쪽·절대). 각주 해석기(`forFootnoteArea`)처럼 기준을 **고정**하는 처방은 여기
