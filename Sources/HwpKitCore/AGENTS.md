...
- **페이지에 걸친 문단의 각주는 조각(run)마다 그 페이지에 수집한다** (#95).
  ...
  **각주·미주 안쪽은 예외의 예외다**: 그 노트는 이 조각이 곧바로 배치하므로
  안쪽 노트도 같은 조각이 걷는다 (미루면 참조와 갈리고 번호 순서가 밀린다).
  그래서 마지막 조각의 전수 순회는 **노트 컨테이너의 자식을 건너뛴다** — 안
  그러면 앞 조각이 이미 걷은 것을 또 세어 번호가 어긋난다.
  가드: `HwpFootnoteFragmentAttributionTests`(11종 — 캐시 좌표가 그려진 조각과
  어긋나는 문단, 컨테이너 안 각주가 그중 둘이다). 코퍼스에서는 둘 다 잠재였다
  (렌더 해시·겹침 실측 무변화).
