...
- 키 큰 인라인 개체 줄에서 글자 아래 밑줄이 개체 하단(= 줄 상자 바닥)에 남는 실물(공공누리)은
  ...
  두 번 셌다 (10pt 글자 + 40pt 개체 줄: 한글 −6.12~−6.24pt, 종전 −7.70pt). 글자 위 밑줄은 반대쪽
  가장자리(상자 상단 = 개체 윗변)에 붙는다 — 한글 12.30: 10pt 위 밑줄 + 40pt 그림·표 줄
  +34.20pt(종전 +8.70pt). 옛 보정에서 얻은 교훈 둘은 규칙에 그대로 녹아 있다: 아래쪽 보정을
