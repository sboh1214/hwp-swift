...
- **자리 차지 표는 문단 글줄 앞 띠가 우선이다** (#161, `appendBandedTableIfNeeded`).
  ...
  **대상은 `topAndBottom` 하나다** — `consumesFlow`로 게이트하면 어울림(`square`)과
  ...
  규칙이 성립하는지 실측한 적이 없다.
  **저작 문단 간격은 띠가 아니다** — 절대 캐시 배치는 `beforeGap`을 커서로 소비하지
  않고 캐시 줄 위치에 이미 담고 있어서, 빼지 않으면 문단이 의도한 여백에 표가 들어간다.
  띠는 그 간격 **뒤**에서 시작한다: 이 문단의 위 간격(`authoredSpacingTop`, 산식은
  `authoredBeforeGap` 하나를 흐름 배치와 공유)과 **앞 문단의 아래 간격**
  (`precedingSpacingBottom`/`authoredAfterGap`) 둘 다다 — 절대 캐시 블록 높이
  (`absoluteRunBlockHeight`)는 줄 위치·높이·줄 간격만으로 만들어져 아래 간격을 담지
  않으므로 그 몫이 `entry.top`과 캐시 줄 사이에 남는다(밴드가 열리면 앞 문단이 다른
  밴드의 것이라 0으로 되돌린다). **앞 문단의 몫은 그 문단이 캐시로 배치됐을 때만
  뺀다** — 흐름 배치는 `paragraphHeight`(= `totalHeight`, CT가 `paragraphSpacing`으로
  담는다)로 커서를 전진시켜 아래 간격을 **이미 소비**했으므로, 또 빼면 같은 여백이 두
  번 빠져 유효한 띠가 좁다고 오판되고 표가 글줄 뒤로 밀려 뒷 문단을 덮는다(절대 캐시
  문서에도 캐시 없는 문단이 있다 — `track-changes` s0p0이 실물이다).
  ...
  **앞선 표가 글줄 자리로 갔으면 띠를 닫는다**(`bandClosed`) — 띠에 못 든 표가 흐름으로
  ...
  흐름 커서는 `max(원래 자리, 표 아래 + 여백)`으로 되돌린다. 흐름 모드(캐시 없음)는
  규칙이 미실측이라 손대지 않는다 — 그쪽 간격은 문단 위 간격이라 작은 표가 우연히
  걸릴 수 있다. 2007 계열 저장본은 표를 글줄 **뒤**에 두는데(헌법주석 구역 22 문단
  201·구역 25 문단 45) 간격이 요구량에 못 미쳐 자연히 이 경로를 안 탄다.
