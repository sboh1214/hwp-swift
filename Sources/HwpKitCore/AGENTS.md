...
아래 "쪽 끝 줄 단위 분할" 절. 남은 격차: 쪽 끝 적합 판정이 줄 상자가 아니라 전진량이라 여분
때문에 줄 하나가 한글보다 먼저 넘어간다 (#222); 첫 줄과 그 각주가 함께 안 들어가면 한글은
...
- **절대 캐시 모드의 각주는 본문 아래 자리에 한글의 이어짐 규칙으로 싣는다** (#165,
  ...
  예약 `measuredFootnoteHeight(isNoteEnd:)`도 같은 산식). ④ 통째로 안 들어가는 각주는
