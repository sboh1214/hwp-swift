...
- **배치 ascent를 `CTLineGetTypographicBounds`의 ascent에서 가져오면 안 된다.** CT는 슬롯의
  여분을 baseline **위**에 넣으면서 그 몫을 줄 객체에 되돌려 주지 않고 (Helvetica 10pt 자연
  조판: 보고 7.70 vs 배치 9.70), 못박힌 청크에서는 클램프한 줄에 자연값과 클램프값을 섞어
  보고한다 (noori 15pt·170% 문단: 같은 25.5pt 슬롯의 세 줄이 16.05·16.05·**18.0**을 보고하고
  배치는 전부 18.0). **배치값을 직접 재는 방법**은 프레임 높이를 줄여 줄이 떨어지는 임계를
...
- **줄 사이 간격은 복원에서 걷어내되, 줄 간격 하한·상한을 적용한 값을 쓴다**
  ...
  **원시 `lineSpacingAdjustment`가 아니라 유효 간격이다** — CT는 그 값을
  ...
  넣어도 걸리지 않는다. **줄 간격 하한 0은 미설정으로 본다** — CT는 명시적 0에서도 음수 간격을
  그대로 적용한다 (실측: 하한 0 명시 + 간격 −6에서 슬롯 12.0 → 6.0). 문단 아래·위 간격은 이
  상한에 걸리지 않고 (상한 0에도 문단 간격이 남는다) 하한은 문단 경계에도 더해진다
  (하한 9 + 문단 간격 10 → 19).

  **음수 간격은 두 곳에서 갈린다.** 상자 사이로 옮기는 몫 (`interlineGap`) 은 **양수만**이다 —
  CT는 음수 문단 간격을 무시하고 (실측: −6·−2에서 슬롯 불변), 음수 줄 간격은 슬롯을 그만큼 줄여
  이미 반영하므로 (상자 상단 = 슬롯 상단) 또 옮기면 두 번 세어진다. 반면 **슬롯 하한
  (`minimumSlot`) 은 음수 몫을 반영해야 한다** — 하한 20pt에 간격 −6을 주면 CT 슬롯이 20 → 14로
  ...
  **남는 잔차 둘**(전부 실측): ① 상한·하한이 실제로 걸리면서 CT가 글꼴에 따라 갖는
...
- **하한만·상한만은 못박은 것이 아니다** (`pinnedLineHeight` = `min == max`이고 둘 다 양수).
  ...
  둘째 줄이 첫 줄 baseline보다 **위로** 올라간다 (합성 실측: 172.8 대신 120.5). **상한만**도
  같다 — 상한은 그 아래 높이를 전혀 건드리지 않으므로 무해한 상한
  ...
  **남는 잔차**: 못박힌 문단에 **글자처럼 취급 개체**가 있고 그 블록에 다른 높이로 못박은 문단이
  ...
  상자를 달리 잡는다 — 즉 줄별이 **모델로는 맞다**. 그런데 한글의 전진량은 상자 + 간격
  (13.52·11.72) 이고 우리는 CT 슬롯 (줄별 14.17·13.00, 못박은 것으로 보면 13.00·13.00) 이라
  둘째·셋째 줄의 |오차| 합이 1.28 → 2.58pt로 늘었다. 못박은 쪽이 이 문단에서 우연히 가까울
  뿐이고, 그 대가로 첫 줄에 개체가 있는 같은 종류의 문단에서 baseline이 뒤집힌다. 성질
  (단조증가·개체 상자 안) 을 지키고 남은 수치는 전진량 축 (#180) 에 맡기는 쪽을 택했다.
- 청크 이월은 baseline이 아니라 **상자 상단과 배치 ascent**를 넘긴다 (`resume`). ascent를
  넘기지 않으면 다음 청크가 **새 프레임의 첫 슬롯 특례**(`floor`)를 기준으로 복원해 그 몫이
  경계마다 되풀이된다 — 실측: Hiragino Sans 하한 10pt 문단이 예산 20에서 33.6pt, Helvetica가
  12.0pt 어긋났고 이월 ascent를 넘기면 예산 13·20·40 전부 0.000pt다. 미완 줄이 없는 이월
  (rescue·경계 일치) 은 그 청크 **마지막 줄**의 ascent를 넘긴다 (같은 문단이 이어지는 흔한
  ...
  **재개한 첫 줄의 슬롯 하한도 그 자리의 것이다.** 이월 ascent를 쓴 줄은 앞 청크에서 간격이
  ...
  예산 20에서 328.5pt, 30에서 310.5pt). 이월 값을 **버린** 줄은 그 프레임의 진짜 첫 줄이므로
  하한 그대로다.

  **그 하한의 간격은 앞 줄의 문단이 정하므로 이월이 실어 온다** (`ResumedSlot.spacing`).
  하한은 그 줄 **자신의** 문단, 간격은 **앞 줄의** 문단이고 (`minimumSlot`의 index > 0 갈래와
  같은 규약) 두 값은 문단 경계에서 갈린다 — 2026-09-13 임계 실측 (Helvetica 10pt·폭 70·하한 20
  두 문단·첫 문단 4줄): 간격 −6 → 0에서 슬롯이 `20·14·14·14·[14]·20·20·20`, 0 → −6에서
  `20·20·20·20·[20]·14·14·14`, 4 → −4에서 `20·24·24·24·[24]·16·16·16`으로 대괄호로 표시한
  경계 줄(5번째)이 전부 **앞 문단**의 간격을 따른다. 앞 줄은 다음 청크에 없으니 재개 줄 자신의 값을 쓰던 동안
  문단 경계가 이월에 걸리면 하한이 6pt 높아 경계 줄부터 6.0pt 아래로 밀렸다 (예산 18·20·24·28·44,
  하한이 문단마다 다른 20/−6 → 30/0·30/−6 → 20/0도 같다). 전체 조판은 임계 실측과 정확히 같다.
  넘길 간격은 그 청크가 **마지막으로 놓은 줄**(`keepCount − 1`)의 것이다 — 버린 줄을 이월하는
  길에서도 그 줄의 앞 줄이 곧 이 줄이다 (`dropped == keepCount`). **이월 전진량도 같은 값을
  쓴다** (`fallbackLineAdvance`) — `interlineGap`이 상자 사이에 남기는 값과 같아야 하는데 원시
  `lineSpacingAdjustment`를 쓰고 있었다 (실측, Helvetica 10pt·폭 70 10줄 문단·예산 14: 상한 4에
  ...
  **남는 잔차 (이 축)**: 이월을 **버리면** 그 프레임의 `floor`를 쓰는데 그 값도 앞 줄이 넘긴
  ...
  −2 → −6에서 8.0pt), 그 프레임의 `floor`를 쓰는 쪽도 같은 14를 내므로 **이월을 버리는 것으로는
  고쳐지지 않는다** — 그 자리의 ascent는 `하한 − 아래 몫`으로 다시 세워야 한다. 전진량 축
  (#180·#192·#198) 과 같은 성격이라 그쪽에 둔다.
...
delegate 분기보다 먼저 읽어야 마커만 있는 줄 (자리 차지 개체 앵커·필드 표식·메모 앵커는
폭 0 delegate를 단다) 의 상자가 0이 되지 않는다.

**남은 격차 (다른 축)**: ① 줄 **전진량**은 아직 상자 = 글자 크기 모델이 아니다 —
`.marginOnly`·`.atLeast` 문단은 여러 줄일 때 CT 자연 높이로 전진해 한글보다 넓고
(합성 실측: 여백만 0·40pt 문단의 둘째 줄이 12.2pt 아래 — #180. 그중 0.3pt는 CT가 **첫 줄
슬롯만 그만큼 크게** 잡는 몫이다: 같은 내용의 첫 줄 배치 ascent가 10.00, 뒤 줄이 9.70이고
임계 실측 슬롯도 12.30 대 12.00이다. 못박힌 문단에는 없다. 코퍼스에서 이 몫이 드러나는 곳은
`legacy-common-control-property` 147쪽 표본 중 **두 쪽**이다 — 470쪽 각주는 줄마다 상자가
달라 (캐시 `h=1080·900·900`) 복원이 이기고 (|오차| 합 2.58 → 1.96pt), 666쪽 본문 문단은 첫
두 줄 상자가 같아 이 몫만 남는다 (25.46 → 27.26pt, 줄당 +0.18). 둘 다 그 문단의 줄당 1.0pt
...
여백만 v → 상자 + v이고 상자는 언제나 글자 크기다 (스윕 실측, #180 코멘트).
**한글의 줄 상자는 줄마다 그 줄의 최대 글자 크기다** — 문단 최대가 아니다 (실측: 10pt 45자 +
40pt 12자 한 문단의 `vertsize`가 줄별로 1000·4000·4000). 우리는 문단 단위로 강제하므로
(`maxFontSize`) 크기가 섞여 줄바꿈되는 문단은 줄 **상자 높이**부터 어긋난다 — 앵커는 줄별로
맞으므로 각 문단 첫 줄은 정확하고 (그 상자 상단이 블록 상단이다) 오차는 종전 73.3pt에서
61.0pt로 줄지만, 둘째 줄부터는 상자 상단이 문단 단위 전진량으로 타일돼 남는다. 앵커를 청크
첫 줄 값으로 고정하면 그 문단 오차만 48.0pt로 더 작아지는데 **그 길로 가지 않는다** — 한글은
줄마다 자기 상자 앵커를 쓰고 (위 실측), 코퍼스 정합성도 줄별이 낫다 (PrvImage MAE 합계
0.0499 → 0.0495, `CCL` 0.0023 → 0.0019). 큰 오차를 반대 방향 오차로 지우는 것은 #178이
고친 그 병이다. ② 글상자·표 셀·쪽 번호 크롬 안 텍스트는
컨테이너 자체의 세로 자리가 한글과 달라
(noori 1쪽 제목 글상자 −5.35pt·표 셀 −1.3 ~ −2.6pt·쪽 번호 −31.7pt) 앵커를 맞춘 뒤
그 격차가 제 크기로 드러났다 (#193). ③ MS Word 호환 문서 (`targetProgram` 2) 는
줄 상자 모델 자체가 다르다 — `track-changes`의 캐시가 `vertsize=1692`·`baseline=1266`
(0.748) 으로 **글꼴 줄 높이·ascent**이며 비율 160%도 그 값에 걸린다 (#194).
장식 선의 같은 축 (#187) 과 함께 봐야 한다. ④ **글자처럼 취급 개체의 세로 앵커는 아직 CT
줄 ascent 기준이다** (`HwpLineFrame.baseline`) — 개체가 그 줄 글꼴보다 크면 (코퍼스의 전부)
어긋남이 0이고, 상대크기·강제 줄 높이가 CT ascent를 부풀린 줄에서만 갈린다 (#195).
⑤ 상자보다 큰 글자의 잉크가 블록 프레임 위로 나가는데 최상위 텍스트 블록의 히트 자격에는
...
- **폭이 다른 단으로 옮겨진 측정 높이 조각은 그 단 폭으로 다시 잰다** (PR 리뷰 3차 —
  ...
  - **첫 줄 ascent 초과분(#164)의 기준은 앞 조각의 마지막 줄과 나머지의 마지막 줄 가운데 작은
    ascent**(`precedingBaseline`). 조각 상자는 첫 줄 ascent를 빼고 마지막 줄 높이를 더한 것이라
    첫 줄이 마지막 줄보다 키 크면 그 차이만큼 짧다 — 앞 조각의 마지막 줄만 기준이면 그 줄도
    개체 줄일 때 다시 재며 첫 줄로 올라온 개체를 상자가 못 담고, 나머지의 마지막 줄만 기준이면
    그 줄이 개체 줄일 때 같은 일이 난다. CT가 프레임 마지막 줄의 ascent를 강제 줄 높이 쪽으로
    키우는 quirk(16pt 줄 9→11, 32pt 줄 9→19)도 이 초과분으로 뒤 조각에 실린다(합은 보존).
