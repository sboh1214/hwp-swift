...
여백·쪽 번호 자리로 고쳤다 — 이제 0.1pt 안이다. ③ MS Word 호환 문서 (`targetProgram` 2) 는 줄 상자 모델 자체가 다르다 —
`track-changes`의 캐시가 `vertsize=1692`·`baseline=1266` (0.748) 으로 **글꼴 줄 높이·ascent**이며
비율 160%도 그 값에 걸린다 (#194). 장식 선의 같은 축 (#187) 과 함께 봐야 한다. ④ 상자보다
...
- **MS 워드 호환 문서(호환 문서 대상 프로그램 2, `HwpCompatibleDocumentTarget.msWord`)
  ...
  - 한글의 호환 문서 **줄 상자·베이스라인 자체**(#194)는 아직 네이티브 모델로 그린다 —
    위 줄 상자는 장식선에만 쓴다. 그래서 호환 문서 픽스처의 픽셀 핀은 쪽 좌표가 아니라
    같은 줄 안의 선 간격·두께다.
