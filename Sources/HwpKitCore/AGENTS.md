...
- **밑줄(글자 아래·글자 위·변경 추적 삽입)은 줄 단위, 취소선은 run 단위다** (#226, 한글
  ...
  문서의 패턴(#227 — 크기를 따르지 않는다)과 MS 워드 호환 문서(미실측)는 종전 run 크기다.
- **선 모양(표 25)은 `HwpLineShapeGeometry` 한 곳이 채우기 경로로 푼다** (#191, 한글 12.30
  ...
  **축척이 두 갈래다**: 글자선은 글자 크기(대시 단위 0.057em·2중선 띠 0.12em·그 밖 여러 줄
  ...
  1/6·1/6])는 같다. 한글은 OWPML 이름과 반대로 `DOT`(`longDotLine`)를 긴 점선, `DASH`
...
- **MS 워드 호환 문서(호환 문서 대상 프로그램 2, `HwpCompatibleDocumentTarget.msWord`)
  ...
  한글 문서와 같고 기준 크기는 글자 모양 기본 크기다. 실선이 아닌 선 모양의 패턴은 아직
  한글 문서 값이다 (#227). 레이아웃 호환성 플래그(표 56)는 관여하지
