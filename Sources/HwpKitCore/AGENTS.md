...
- **각주 블록 높이도 셀과 같은 규약이다** (#94): 갈리는 축은 배치 방식이 아니라
  ...
  **줄 캐시**에서 왔으면 그 캐시가 표보다 짧을 수 있으므로(저작 뒤 셀 내용이 커진 문서) 표는 캐시
  ...
  없으면 표가 블록 밖 다음 각주 위로 흘러나간다). 하한은 표만 담는다: 캐시와 CT 줄 수가 다른
  문단의 뒤 줄이 블록 밖에 그려지는 것은 캐시 높이를 믿는 종전 축이다. 한글이 저장한 캐시는 표를
  담으므로 코퍼스에서는 걸리지 않는다. 하한은 글상자(내용이 저작 높이를 넘는 개체)와 높이를 넘기지 않는 경로의
  안전망으로도 남는다.
