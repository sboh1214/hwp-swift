...
- 장식 선의 세로 위치는 **글꼴 지표가 아니라 글자 크기 비례**다 (#136 실측:
  한글.app 12.30 PDF 내보내기, 5~100pt·세 글꼴에서 같은 비율). 취소선 +0.35em ·
  글자 위 밑줄 +0.87em · 변경 추적 삭제선 +0.29em이 `HwpRenderTuning.Text`에
  있고, 글자 아래 밑줄 −0.20em과 변경 추적 삽입 밑줄(선 아래 모서리 −0.35em)은 아직
  in-place 상수다 (실측 중심은 각각 −0.17em·−0.255em이라 별건). 선 두께는 전부 0.4pt 고정인데
  한글은 0.04em 비례다 — 10pt에서만 일치한다. 밑줄 종류 raw 2(글자 가운데)는
  취소선과 같은 선으로 합류시킨다
