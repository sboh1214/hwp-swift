...
- U+FFFC 마커 run만 여기서 지운다 (`strippingControlMarkerRuns`) —
  ...
  당기므로 범위 재계산이 없다. 조각 사이 개행은 직전 글자에서 **문단
  스타일과 폰트만** 상속한다 — 속성 없는 개행은 RTF에서 앞 문단의 스타일을
  잃고, 전체를 상속하면 하이퍼링크·밑줄 같은 글자 속성이 원문에 없던
  개행까지 번진다.
- **그 상속은 빈 결과 위에서도 불린다.** 첫 조각이 마커뿐이면 조립 결과가 빈
  채로 다음 조각 앞 개행이 오므로, 직전 글자를 묻는 `attributes(at:)`가 `-1`로
  트랩한다 — `newlineAttributes`의 `result.length > 0` 가드가 그 자리다.
  가드 없이도 정상 문서는 전부 초록이라 (무력화 실험 확인)
  `testLeadingMarkerOnlyFragmentDoesNotCrashNewlineAttributes`가 짝으로 있다.
