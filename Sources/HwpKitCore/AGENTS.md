...
- **그 이동은 기하 질의에도 반영한다 — 단 넓히는 것이지 옮기는 것이 아니다** (#197 리뷰).
  ...
  (= 줄 상자 + 옮겨진 run마다 **그 run의 가로 범위만** 가진 밴드)를 두고
  `textLineRegions`(claim)와 `hyperlinkRegions`(방출 ≡ 히트)가 그것을 쓴다.
  `hitEligibleFrame`의 `.text`도 가로만 넓히던 것을 `textBounds`로 바꿔 세로 여유를 함께
  준다 — 안 그러면 그 글리프 위의 탭이 rect 판정에 닿기도 전에 블록 단계에서 기각된다.
  ...
  - **하나의 rect로 합치지도 않는다** (리뷰 2·3차): 줄 전체 폭에 최대 오프셋을 걸면 안
    ...
    R54 그대로다. 가로 범위는 **글리프 위치**(`CTRunGetPositions`)에서 낸다 — 문자열
    ...
    **`hyperlinkRegions`도 같다**: 스팬 전체 폭에 최대 오프셋을 걸면 같은 URL의 안 옮겨진
    ...
    가로 범위)를 그대로 내고, 옮겨진 run마다 **스팬 가로 범위로 클립한** 밴드를 하나씩 더
    낸다 — 한 스팬이 여러 rect를 내도 소비처는 `first { contains }`·`isEmpty`뿐이라
    무해하고, `.hyperlink` 커맨드는 아무것도 그리지 않아 시각 격차도 없다.
...
- 장식 선은 글리프가 아니라 **줄 원점** 기준이다 — `glyphBaselineOffset`(글자 위치·
  ...
  한글은 첨자 취소선만 올라간 베이스라인을 따라가고 줄어든 크기로 그리는데
  (6.36pt 글리프에서 +2.28pt) 우리는 제자리에 그려 ~4.4pt 어긋난다(#179).
  위쪽 밑줄은 한글도 제자리이고 **크기만 첨자 축소 전 값**을 쓰므로
  `spaceTargetSize`로 그린다(상대크기는 반영, 첨자 축소만 제외). 코퍼스에
  첨자 + 장식 조합이 0건이라 남은 격차의 화면 영향은 없다
