...
`boxHeight`) — 앵커·전진량·밑줄 되돌림이 그것을 공유한다. **CT 줄 origin의 y·슬롯·글꼴
...
- 장식 선의 세로 위치와 **두께**는 **한글 문서에서는 글꼴 지표가 아니라 글자 크기
  ...
  단위로 `0.12 × round(크기/3)`)이 `HwpRenderTuning.Text`에 있고 하한은 없다(5pt는 0.2pt).
...
- **MS 워드 호환 문서(호환 문서 대상 프로그램 2, `HwpCompatibleDocumentTarget.msWord`)
  ...
  고정 0.36pt이고 밑줄은 한글 문서와 같은 가장자리(아래 0.15em·위 0.85em)에 그 얇은 선을
  얹는다** (#210, 2026-09-22 한글 12.30 실측: 9개 글꼴 × 5~100pt 22개 크기에서 두께가 전부
  ...
  - **줄 상자·베이스라인·전진량도 같은 글꼴 줄 상자다** (#194, 2026-09-20 한글 12.30 실측:
    ...
    - **글자처럼 취급 개체 줄**: 베이스라인 = max(글꼴 상자 베이스라인, 개체 높이), 상자 =
      ...
      기준(932)이고 고정·여백만·최소는 그 상자에 적용한다 (3455 + 300, max(3455, 1600)). 밑줄 되돌림
      (`underlineReturnDrop`)은 호환 문서에서 걸리지 않는다 — 베이스라인이 곧 개체 바닥이고 밑줄은
      줄 상자 가장자리에서 잰다 (개체만 있는 줄은 상자 = 개체라도 되돌리지 않는다).
...
- 장식 선은 글리프가 아니라 **줄 원점** 기준이다 — `glyphBaselineOffset`(글자 위치·
  ...
  되돌려 가를 수 없어서다. 렌더러는 취소선 경로만 이 키를 더하고 밑줄 두 종과 두께는
  `spaceTargetSize`(상대크기 반영·첨자 축소 제외)를 쓴다. 글리프·기하 질의·서식 복사는
...
- 키 큰 인라인 개체 줄의 밑줄 되돌림(`underlineReturnDrop`)은 **글자 아래 밑줄에만**
  적용한다. 실물이 그 선을 개체 하단(= 줄 상자 바닥, 베이스라인 아래 0.15 × 개체
  높이)에 남기기 때문이고, 같은
