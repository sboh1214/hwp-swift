...
- 장식 선의 세로 위치와 **두께**는 **글꼴 지표가 아니라 글자 크기 비례**다 (한글.app
  12.30 PDF 내보내기 실측 — #136: 취소선 5~100pt·세 글꼴, 글자 위 밑줄 10/40/100pt·두
  글꼴; #176: 글자 아래 밑줄과 두께 5~100pt 13개 크기·함초롬바탕/돋움·Apple SD·
  HY울릉도M 네 글꼴에서 같은 값). 취소선 +0.35em · 글자 위 밑줄 +0.87em · 글자 아래
  밑줄 −0.17em · 두께 0.04em(한글 PDF는 0.12pt 장치 단위로 `0.12 × round(크기/3)`)이
  `HwpRenderTuning.Text`에 있고 하한은 없다(5pt는 0.2pt). 밑줄 종류 raw 2(글자
  가운데)는 취소선과 같은 선으로 합류시킨다
- **변경 추적 표시선의 상수(삭제선 +0.29em·삽입 밑줄 −0.26em·두께 0.064em)는 변경
  추적 고유 기하가 아니라 MS Word 호환 문서의 기하다** (2026-09-12 실측, #176).
  `track-changes` 픽스처는 호환 문서 대상 프로그램 2(`CT_MSWORD`, HWPX
  `targetProgram="MS_WORD"`)인데, 한글은 `targetProgram`만으로(`layoutCompatibility`
  플래그 무관) 장식선 기하를 바꾼다: 네이티브 문서에서는 삽입 밑줄 = 일반 밑줄
  (−0.17em·0.04em), 삭제선 = 일반 취소선(+0.35em)이고 같은 문단의 일반 구간과
  추적 구간이 같은 y로 이어진다; 호환 문서에서는 일반 밑줄·취소선도 글꼴 지표를
  따른다(밑줄 중심 ≈ −(winDescent + 두께/2), 두께 ≈ 0.05 × 글꼴 줄 높이, 취소선 ≈
  +0.27 × winAscent — 함초롬 −0.258/+0.29, Apple SD −0.326/+0.25, HY울릉도M
  −0.167/+0.23). 코퍼스의 유일한 변경 추적 실물이 호환 문서라 상수는 그 값에 맞춰
