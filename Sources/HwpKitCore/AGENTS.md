...
- **그 이동은 기하 질의에도 반영한다 — 단 넓히는 것이지 옮기는 것이 아니다** (#197 리뷰).
  ...
  - **스팬의 줄 상자 rect는 양끝 오프셋이 아니라 run별 진행 폭 범위를 화면에서 잇닿은 구간별로
    ...
    `CTRunGetPositions` 첫 글리프(run 안 글리프는 시각 순서라 가장 왼쪽)에 run 텍스트 매트릭스를
    건 시작점 + `CTRunGetTypographicBounds` 폭 — positions는 매트릭스 **적용 전** 좌표라(장평
    50% run: 266.797인데 실제 133.398) 렌더러(`drawRun`)처럼 매트릭스를 걸어야 하고, 폭은 적용 후
    값이다. `CTLineEnumerateCaretOffsets`·양끝 오프셋의 secondary 값도 검토했으나 클러스터 짝짓기·
    경계 쪽 선택이 문서화되지 않은 규칙에 기대 run이 낫다. 인접 run의 끝과 다음 run의 시작은
    CT가 같은 누적값을 줘 비트 단위로 같고(프레임 안 줄의 개행 run은 폭 0) 잇닿음 판정 여유는
    0.001pt다. 스팬 소속은 밴드와 같은 **포함** 판정(`belongs(to:)`)이고, 잇닿은 run을 합친 구간마다
    rect 하나 — 단방향 줄에서는 구간이 늘 하나라 종전 상자와 0.001pt 안에서 같다(단일·다중 스팬·
    글꼴 폴백·꼬리 공백·장평·줄 넘김 21케이스 비트 동일, `testUnidirectionalSpansKeepTheLegacySpanBox`
    가 종전 산식을 기준으로 잠근다). run 범위는 줄 캐시 옆 `SpanLineGeometry`에 두되 **스팬이 닿는
    줄만** 처음 닿는 스팬이 걷는다. 비용은 오히려 준다 — 스팬 × 줄마다
    ...
    **한글 12.30 실측** (합성 HWPX → 한글 저장 HWP·PDF, 함초롬바탕 24pt): 한글도 히브리·아랍
    글자를 CT와 같은 시각 순서로 놓고 글리프 x가 0.1pt 안에서 같다(ג 138.2 vs 138.27). 그러나
