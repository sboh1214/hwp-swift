...
- **조각의 참조 마커 번호는 배치 직전에 다시 굽는다** (#95 리뷰 반영).
  ...
  **다만 개체 앵커 축은 닫았다** (PR 리뷰): 재매김이 실제로 편집을 냈으면 배치가
  ...
  위 첨자 한 자 폭 ≈ 3.3pt). 이 축은 순환이 아니다 — 번호가 그 시점에 이미 정해져
...
- **글자 위치(표 33)는 `glyphBaselineOffset` 한 키로만 나른다** — 조판 문자열에
  ...
  커스텀 키만 쓰므로 영향이 없다. 첨자 몫은 `hwp.scriptBaselineOffset`에도 따로 실린다
  (#179, 아래 장식 선 항목).
...
- 장식 선은 글리프가 아니라 **줄 원점** 기준이다 — `glyphBaselineOffset`(글자 위치·
  ...
  (`FixtureDecorationLineRenderTests+Script`). **남은 격차는 첨자 글리프 자체다** —
  우리 위 첨자 올림 0.33em·아래 첨자 내림 0.30em·축소 0.67배 vs 한글 0.446em·0.12em·
  0.64배라 첨자 취소선이 한글보다 10pt에서 0.9(위)·1.7pt(아래) 낮다. 이 축은 첨자
  글리프 전체(각주 번호 포함)의 문제라 #204로 분리했다.
