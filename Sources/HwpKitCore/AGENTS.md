...
- **좁아진 단으로 옮겨진 조각은 폭 허용 오차 없이 실제 줄바꿈으로 판정한다** (PR 리뷰 —
  종전 `sameWidth`의 0.5pt는 앵커 재조판 문턱이지 줄바꿈 판정이 아니다: 단 폭은 HWPUNIT
  단위로 저작돼 0.4pt 좁은 단이 실제로 있고, 잰 폭에 꼭 맞던 31자 줄은 0.01pt만 좁아져도
  다시 나뉜다). 놓이는 단 폭 ≥ 잰 폭이면 CT 줄이 늘 수 없어 그대로 단다. 좁으면 렌더러가
  그 폭에서 조각을 접을 때만(자연 폭 ≤ 1.06배) 표식을 단 조각을 실제로 조판해 그려지는
  줄 수가 측정 줄 수 이하일 때 단다 — 넘으면(라틴 "a " / "b×70 " 조각이 0.3pt 좁은 단에서
  세 줄) 종전 접힘(가로 6% 이내 넘침)이 상자를 세로로 넘치는 것보다 낫다. 30자 + 1자 같은
  한글 조각은 좁은 단에서도 두 줄이라 표식이 붙어 #166 증상이 비등폭 단에서도 고쳐진다.
  표식이 붙은 조각의 줄바꿈이 잰 폭과 달라지므로 **줄 안 개체 앵커 문맥과 예약 재해석도
  같은 정확 비교로 재조판한다** (`fragmentAnchorLines`·`placedFragment`, "앵커 규칙" 절).
...
- 가드: `HwpMeasuredLineFragmentTests` — 술어 단위(왼쪽·가운데·오른쪽 정렬)와 세
  ...
  단(PR 리뷰 재현)의 한 줄 조각 접힘 유지(흐름 분할·균형 재배치)와 두 줄 조각 표식 유지,
  그 두 줄 조각의 줄 안 도형이 그려진 마커(둘째 줄 첫머리)에 놓이는지.
...
- 다단 세부: 캐시 run이 없는 문단의 균형 재배치·조각 높이는 라인 수 기준
  ...
  **넓은 단 → 좁은 단** 이월에서는 줄이 늘어 글자가 블록 프레임 아래로 넘치는데,
  그 줄에 앵커가 있는 글자처럼 취급 개체도 함께 내려간다 (#164, PR 리뷰) — 개체는
  자기 글리프가 실제로 그려지는 줄·x에 놓이므로 **글자 대비로는 정확**하고, 넘치는
  까닭은 조각 높이가 여전히 문단 진입 단 폭 기준 근사라서다(main도 같은 양만큼 글자가
  넘쳤다 — 렌더러는 블록 높이로 자르지 않는다). 고치려면 조각 경계·높이를 목적 단
  폭으로 다시 계산해야 한다. **개체를 흐름 폴백으로 되돌리거나 앵커 y를 블록 하단으로
  클램프하는 처방은 금지** — 개체가 자기 글자에서 떨어져 #164로 되돌아간다. 계약 고정:
  `testWideToNarrowCarryoverKeepsTheObjectOnItsDrawnMarkerLine`
