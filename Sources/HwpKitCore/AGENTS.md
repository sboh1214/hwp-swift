...
- **페이지에 걸친 문단의 각주는 조각(run)마다 그 페이지에 수집한다** (#95).
  ...
  **캐시가 더 앞 조각이라 하면 캐시를 따른다** (#165, `earliestOrdinalRanges` —
  ...
  반대 방향(캐시가 더 늦음)은 그려진 조각을 따른다. 마지막 조각이 나머지를 전부
  가져가므로 어느 조각에도 안 그려진 컨트롤(마커 없는 컨트롤·CT가 잘라낸 라인)도
...
- **절대 캐시 모드의 각주는 본문 아래 자리에 한글의 이어짐 규칙으로 싣는다** (#165,
  ...
  각주도 모두 함께 넘어간다. 여러 문단짜리 각주의 뒤 문단이 0에서 다시 시작하면 그
  ...
  잘리고 블록 높이는 캐시라 조각 끝에 빈 줄이 남을 수 있다 — 본문 조각과 같은 축.
