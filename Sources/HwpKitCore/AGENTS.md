...
표 높이도 그 단에서 다시 잡는다(`rescaledInlineTableHeights` — 아래 "비등폭 단" 항목). **표 셀·
...
- **각주 블록 높이도 셀과 같은 규약이다** (#94): 갈리는 축은 배치 방식이 아니라
  ...
  **줄 캐시**에서 왔으면 그 캐시가 표보다 짧을 수 있으므로(저작 뒤 셀 내용이 커진 문서) 표는 캐시
  ...
  잰다(`HwpFootnoteLayout.lineCacheIsStale`, #214 PR 리뷰): 줄 예약이 그려지는 높이라 표 줄이 커지며
  그 아래 글줄이 모두 내려가는데, 캐시 높이를 믿으면 줄 수가 캐시와 같아도 둘째 줄 이후가 블록 밖
...
- **절대 캐시 모드의 각주는 본문 아래 자리에 한글의 이어짐 규칙으로 싣는다** (#165,
  ...
  블록은 전진량에서 줄 간격을 뺀다 — 단 **stale 캐시로 CT 높이까지 커진 블록은 뺄 몫이
