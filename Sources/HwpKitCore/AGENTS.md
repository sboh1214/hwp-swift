  ...
  → HwpDocumentActor.buildDocument (HwpKitNative)
     ...
     → HwpPaginator (actor) # 페이지 lazy 생성 + 문서 단위 HwpTextAttributeCache 소유
        ...
        └─ 문서/구역 끝: HwpFootnoteLayout.placeFlow  # 미주 (표 134 bits 8-9)
           문서 끝 미주는 새 쪽에서 시작 (한글.app 실측 2026-07-06); 구역 끝 미주의 "남은 자리에 안 들어가면 새 쪽" 판정은 배치와 같은 높이(첫 문단이 미주의 끝이면 줄 간격 제외·미주 모양·미주 단위 개체 판정)로 (#165 리뷰)
...
- `HwpPageChromeBuilder` — 머리말/꼬리말/쪽 번호 크롬 블록. 활성 컨트롤·감춤 마스크 상태 소유.
  쪽 번호 글자 모양 id는 색인이 불변이라 init에서 한 번 푼다 (쪽마다 스타일을 다시 찾지 않는다)
...
- **자동 내어쓰기**(`autoIndent`): 라벨 + 실제로 방출한 거리 빈칸 폭(`gapSpaceWidth`
  ...
  CT 문단**(첫 문단 구분자까지)뿐이고 조각이 문단 구분자 바로 뒤에서 시작하면 첫
  줄도 그대로다 — 한 줄 끝(코드 10, `\n`)이나 본문 U+2029 뒤는 측정에서도 CT 문단이
  새로 시작해 `firstLineHeadIndent`에 놓이므로, 조각 전체를 바꾸면 그 뒤 줄들이
  측정과 다른 폭으로 접혀 줄 수가 갈린다(측정·렌더 공유 줄바꿈 규약). 경계는
  `NSString.getParagraphStart`의 문단 정의(LF·CR·CRLF·U+2029)로 CoreText와 같게
  찾는다 — U+2028·U+0085는 CT가 줄 구분자로만 봐 첫 줄 들여쓰기가 다시 걸리지 않는다.
...
- **실측(한글.app 12.30, 2026-09-07, `numbering-sequence` 사본 3쪽)**: 셀 테두리
  ...
  12.82pt(표 공통 속성 height 1282)로 그린다 (**#160에서 수정** — 아래 "캐시 높이
  우선" 절 참조). ② 자리 차지(위·아래 배치) 표를 품은 문단의 본문 줄을 한글은 표
  **아래**에 두는데 우리는 문단 줄 뒤에 표를 방출했다 (**#161에서 수정** — 위
  "앵커 규칙 (표 70)"의 띠 배치 절 참조. 남은 몫은 표 x의 바깥 왼쪽 여백 2.83pt다).
  글상자·각주·미주·머리말/꼬리말의 번호 문단은 실물 픽스처가 없어 합성 입력으로만
  잠갔다.

**문단 번호·개요 번호 생성** (#153, `Numbering/`): `HwpParagraphNumbering.generate`
가 구역 배열을 **조판과 무관하게** 한 번 훑어 문단마다 `HwpParagraphNumber`(종류·
1수준부터의 번호(개수가 수준)·조립한 라벨)를 만들고, `HwpPaginator`가 init에서
그 표를 만들어 `paragraphNumbering`(불변 `Sendable`이라 `nonisolated let` — actor
밖 동기 코드가 읽는다)으로 낸다. 조판이 문단을 재측정·재배치해도 카운터가 한
번만 느는 것은 이 분리가 보장한다(#154의 라벨 렌더가 문단마다 읽는다). 조회
열쇠는 `HwpParagraphPath` — 최상위는 `HwpParagraphKey`, 컨테이너 안 문단은
컨트롤 서수 + `childParagraphs(of:)` 서수를 이어 붙인 경로다(`paraId`는 비고유).
정의 해석은 위 진단과 같은 `HwpNumberingHeadingReference`이고 현재 구역 정의는
조판과 같이 문서의 첫 구역 정의에서 시작한다(그 정의 문단보다 앞선 문단도 같은
구역).

카운터 규칙은 **한글.app 12.30 실측**(2026-09-06, `numbering-sequence` 픽스처 쌍 —
같은 세션에서 모두 선택·복사하기로 받은 라벨 텍스트가 오라클)이다:
- **정의마다 목록이 하나다** (`HwpNumberingCounter`의 정의별 상태). 같은 정의의
  문단은 사이에 본문·다른 정의의 목록·구역 경계·표가 끼어도 자기 번호를 잇는다
  (정의 3의 `3.`→구역 경계→`4.`·`5.`→표 셀의 정의 6 `9.`·`10.`→`6.`, 그 뒤 정의 6이
  `11.`). 개요와 번호 매기기는 종류별로 따로 센다 — 한글은 문단 번호 적용에 새
  정의(id 2)를 만들어 개요 정의(id 1)와 공유하지 않으므로 같은 정의를 공유하는
  실물은 없다.
- **정의의 첫 문단에서만** 시작 번호 방식을 본다: CoreHwp `HwpNumbering.
  continuesPreviousList`(정의 전체 시작 번호 0)면 같은 종류에서 직전에 쓰인
  정의의 번호를 **물려받고**(구역 나누기가 만든 정의 4가 앞 구역의 `가.`·`1.`
  뒤를 `나.`·`2.`로 이었다 — 개요 "앞 구역의 개요 번호에 이어서"), 1 이상이면
  `startingNumber(forLevel:)`에서 새로 센다(새 번호로 시작 7 → 정의 5 `7.`). 값은
  번호가 아니라 방식이다 — 정의 3은 `start=5`인데 수준별 배열이 [1…]이라 `1.`
  부터 셌다: **수준별 배열이 있으면 배열이 이기고**, 배열이 없는 5.0.2.5 미만
  저장본(헌법주석)에서만 정의 전체 값이 1수준 시작 번호다(0은 1). 새 번호 N은
  두 자리에 함께 적힌다(정의 5·6). 값은 65,535로 접는다.
- **출력 크기 상한이 둘이다** — 시작 번호를 접는 것만으로는 폭주가 막히지 않는다.
  형식 문자열은 표 38 WORD 길이(65,535 단위)까지라 `^1`을 3만 번 적은 정의 하나로
  문단마다 수 MB 라벨이 생기고, `HwpPaginator`가 문서를 여는 즉시 전 문단의 라벨을
  만들어 보관한다(리뷰 실측: 21KB HWPX, 번호 문단 50개가 112MB·9.8초). 그래서
  라벨은 `HwpParagraphNumber.textUnitCeiling`(512 UTF-16 단위)에서 **만드는 도중**
  스칼라 경계로 끊고(`HwpNumberingLabelFormatter`의 `BoundedBuilder` — 다 만든 뒤
  자르면 이미 늦다), 문서 전체는 `HwpParagraphNumbering.maximumDocumentEntries`
  (20,000 — 탐색 목록과 같은 헌법주석 10배)에서 순회를 멈추고 `isTruncated`로
  알린다. 두 상한이 함께 걸려도 표는 약 20MB다.
- **입력 쪽 일도 유계다.** 형식 분해는 문단마다 하되 **출력 천장까지만** 한다
  (`HwpNumberingFormat.pattern(unitCeiling:)` — 토큰은 적어도 1단위, 문자 조각은
  스칼라 폭만큼을 보장하므로 보장 단위가 천장에 닿는 접두에서 멈춘다). 전체를
  분해하면 65,535단위 형식 × 20,000 문단 ≈ 13억 스칼라이고, 정의·수준마다 전체
  토큰 배열을 메모하면 DocInfo 입력 상한 안에서도 배열만 GB 단위라 둘 다 안 된다
  — 천장까지만 분해하면 일도 저장도 형식 길이와 무관하다. 그 분해는 스칼라 뷰를
  **배열로 펼치지 않고** 인덱스로 한 칸 앞을 본다 — `Array(unicodeScalars)`는 천장이
  있어도 호출마다 형식 전체를 복사·디코드한다(가드 `NumberingPerformanceTests`).
  순회는 `HwpPaginator.init`의 동기 경로라 조판의 쪽 단위 지연·문단 단위 취소
  관찰 밖이다 — 그래서 걸음 수(문단 **과 컨트롤** 각 한 걸음)에
  `maximumVisitedNodes`(500,000 — 항목 상한은 번호 문단에서만 줄어들어 번호 없는
  문단 수백만 개나 책갈피 수백만 개를 품은 문단 하나는 막지 못한다)를 두고,
  256걸음마다 `Task.isCancelled`를 봐 취소된 로드는 걷다 만다(둘 다
  `isTruncated`). 컨테이너 자식은 `childParagraphSequence(of:)`(게으른 원본 —
  `childParagraphs(of:)`는 그것을 펼친 배열)로 열어 상한·취소에서 멈추면 그 뒤
  문단을 꺼내지 않는다 — 배열로 펼치면 셀 수십만 개를 상한을 보기 전에 복사한다.
  구역 정의 탐색도 예산 안이다 — 최상위 문단의 컨트롤을 첫 구역 정의까지 걸음으로
  세며 보고(자식 순회는 그만큼 다시 깎지 않는다), init의 첫 구역 정의 사전 탐색은
  같은 상한·취소 관찰을 따로 센다(순회가 같은 노드를 다시 밟으므로 합치면 두 번
  깎인다). 조판의 `sectionDef(in:)`는 첫 구역 정의에서 멈추는 루프라 중간 배열이
  없다.
  init에 두는 이유는 #154의 조판 문자열 조립·복사·접근성이 actor 밖 동기 코드라
  `nonisolated let`으로 읽어야 해서다 — 옮기려면 그 동기 접근부터 다시 설계해야
  한다. HWPX `hh:heading@level`이 3비트 밖(9·10수준)이면 매퍼가 머리 종류 없음으로
  접고 진단으로 남기므로(`Sources/CoreHwp/Hwpx/AGENTS.md`) 여기까지 오지 않는다.
- 개요는 **구역 시작**에서 구역 정의의 정의를 활성화하고(같은 정의를 두 구역이
  가리키면 잇는다 — 실물은 없고 정의별 목록 규칙의 귀결), 번호 매기기는 구역
  시작을 보지 않는다.
- 상위 수준을 매기면 하위는 비워지고, **건너뛴 상위 수준은 시작 번호로 매겨진
  것으로 친다** — 1수준 뒤 바로 3수준이 오면 2수준은 시작 번호가 되고 다음
  2수준 문단은 그다음 번호다(`1.`→3수준 `1)`→2수준 `나.`). 비워진 하위 수준을
  형식이 참조하면 시작 번호를 보인다(대화상자 미리보기 규약, 실물 없음).
- 컨테이너(표 셀·글상자·각주·미주·머리말/꼬리말) 안 문단은 품은 문단 **뒤에**
  문서 순서로 같은 종류의 카운터를 쓴다 — 표를 품은 빈 문단이 `5.`, 셀이 `9.`·
  `10.`(셀은 자기 정의), 표 뒤 문단이 `6.`. 머리말/꼬리말 안 번호 문단은 실물이
  없다(같은 규칙으로 센다).
- `^n`·`^N`은 1수준부터 그 수준까지를 각 수준의 번호 모양으로 잇는다
  (`HwpNumberingLabelFormatter` — `outline-numbering` 둘째 정의의 9·10수준으로
  한글.app 실측 문자열 `I.가.1.가.1.가.①.㉮.ㄱ)`·`ㄱ.I0)`을 핀).

오라클 셋: 헌법주석의 **생성 목차**(구역 0 문단 50-377, 1수준 표제 280개의 `I.`…
가 조문(구역)마다 다시 시작 — 41개 구역 정의 중 첫 것만 시작 번호 0이고 40개가
1; 2-7수준 1,664개는 외부 근거 없이 스냅샷으로만 잠근다), 한글.app 저장본
`outline-numbering`의 PrvImage 라벨과 `numbering-sequence`의 복사 텍스트 라벨,
1,944개 전체의 커밋 스냅샷 `Tests/HwpKitCoreTests/ParagraphNumberingSnapshots/`
(`RECORD_NUMBERING_SNAPSHOTS=1`). **남은 것**: 도움말의 셋째 방식 "이전 번호 목록에
이어"의 저장 형식(대화상자에서 비활성이라 만들지 못했다 — 정의별 목록 규칙이 그
동작을 이미 낸다), 같은 정의를 두 구역이 가리키는 실물, 머리말 안 번호 문단.
...
**베이스라인은 줄 상자 상단 + `baselineDistance`다** (#178). 같은 캐시가 그 값을
직접 적어 두며 `baselineDistance = 0.85 × lineHeight`이고, `lineHeight`(= `vertsize`)
는 그 줄의 **상대크기 적용 전 기본 글자 크기**다 (키 큰 글자처럼 취급 개체가 있으면
그 개체 높이). 즉 비율 160%의 여분 6pt는 상자 **아래** `lineSpacing`으로 가고 상자
자체는 1em이다. 한글.app 12.30 (2026-09-12) 이 저장한 합성 스윕 34줄 (크기 8종 ×
줄 간격 종류 4종 × 글꼴 3종 × 상대크기 2종) 이 전부 0.85였고, 같은 문서의 PDF 벡터
베이스라인이 `lineLocation + baselineDistance`와 최대 0.10pt (한글 PDF의 0.12pt 장치
양자화) 차이였다.

**baseline은 앵커가 정하고, 상자 상단은 전진량이 정한다** (#178·#180). 렌더
(`HwpDrawnTextLayout.lines`)와 측정(`HwpParagraphLayout.layout`)이 둘 다 줄바꿈 뒤
`HwpLineAdvance.advanceParts(of:in:)`(`Text/HwpLineAdvance.swift` — 측정은 그 합 `advances`)로 줄마다 전진량을 받아 상자
상단을 쌓고, baseline은 자기 상자 상단 + `baselineAnchor(of:)`(0.85 × 상자 높이,
`HwpDrawnTextLayoutAnchor.swift`)다. 줄에서 읽는 **지표 한 벌**은
`HwpDrawnTextLayoutMetrics.swift`가 소유한다 (`LineMetrics`: `textBoxHeight`·`delegateAscent`·
`boxHeight`) — 앵커·전진량·줄 단위 밑줄의 기준(`underlineReference`, #226)이 그것을 공유한다. **CT 줄 origin의 y·슬롯·글꼴
지표(ascent·descent·leading)는 세로 배치에 쓰지 않는다** — `FrameChunk.origins`는 x(들여쓰기·
정렬)만 쓴다.

**줄 상자는 줄마다 그 줄의 상대크기 적용 전 기본 글자 크기 최댓값**(run의
`hwp.baseFontSize`, 없으면 조판 글꼴 크기)이고, 줄 공간을 예약한 글자처럼 취급 개체(run
delegate)가 더 크면 개체 높이다. 한 문단이 크기 섞여 줄바꿈되면 한글 캐시의 `vertsize`도 줄별로
다르다 (2026-09-12 실측: 10pt 45자 + 40pt 12자 문단이 1000·4000·4000) — 문단 최대가 아니다.

**마커 run도 자기 글자 모양을 싣는다** (`appendControlMarker`) — 그 크기가 어디에 드는지는 마커가
**줄 공간을 예약했는지**(delegate ascent > 0)가 가른다 (#217, 폭이 아니다). 예약하지 않은 마커(책갈피·
필드 표식·메모 앵커·자리 차지 개체 앵커 — 높이 0 delegate)는 글자처럼 상자에 든다: 마커만 있는
줄의 상자가 0이 되지 않고(noori 1쪽 자리 차지 그림 문단), 한글도 10pt 글에 든 40pt 책갈피·자리 차지
그림 마커로 상자를 40pt로 잡는다. **예약한 개체 마커의 글자 모양은 줄 상자·베이스라인에 들지 않고
비율 여분의 기준에만 든다** (`LineMetrics.objectMarkerBaseFontSize` → `textBoxHeight`). 한글 12.30
실측(2026-09-24, `inline-object-marker-size` 픽스처 쌍 — 함초롬바탕 10pt 글·160%): 40pt 마커의 20pt
그림 줄 `vertsize` 2000·`baseline` 1700·`spacing` 2400, 8pt 그림 1000·850·2400, 20pt 마커의 8pt 그림
1000·850·1200, 표·바깥 여백(20 + 7 + 3 → 3000)·상대 크기 50% 마커·셀 안도 같고(도형은 픽스처 밖
합성 문서의 `.hwp` 저장본으로 확인 — HWPX 매퍼가 `hp:rect`를 아직 승격하지 않는다) 줄 간격 고정 30 →
`spacing` 1000·여백만 5 → 500·최소 12 → 200(상자 10)이다. 개체만 남은 가운데 줄(8pt 그림 7개)은 마커가
40pt든 10pt든 800·680이고 여분만 마커 기준(2400·600)이다. 문단 끝 글자(CR)는 개체 마커 run에 실려도
CR 자신의 글자 모양이라 상자에 든다 — 40pt 마커의 8pt 그림만 있는 문단은 CR이 10pt면 1000, 40pt면
4000. 이 구분이 없던 동안(#178 규칙의 과적용) 개체 마커가 글자보다 크면 상자가 마커 크기로 부풀어
그 줄의 글자와 개체가 함께 내려갔고(베이스라인 K3 17pt·K7 25.5pt), 뒤 문단은 줄 상자 차이(K3 20pt·
...
(`maxCharacters`)은 문단 끝이 아니라 싣지 않는다. 밑줄 자리(`underlineReference(of:endsParagraph:)`,
#226 — 밑줄은 줄 상자 바닥에 붙는다)·단 구분선 끝(`measuredTrailingSpacing` — 마지막 줄을 조판해 `LineMetrics`로 잰다)도 같은 값을 본다. MS 워드 호환 문서는 이 키 대신 문단 끝
...
| --- | --- | --- |
| 비율 | p % | h + t × (p − 100) / 100 |
| 고정 | pt | v — 상자보다 작으면 줄이 겹친다 (한글도 그렇다) |
| 여백만 지정 | pt | h + v |
| 최소 | pt | max(h, v) |

한글 12.30.0 (2026-09-15) 이 저장한 합성 스윕 27문단의 줄 캐시(`vertsize`·`spacing`)와
PDF 벡터 베이스라인이 이 표 그대로다: 10pt 여러 줄 문단이 비율 100/160/300 → 10/16/30,
고정 5/16/30 → 5/16/30, 여백만 0/3/10 → 10/13/20, 최소 5/16/30 → 10/16/30, 비율 80 → 8;
10pt/40pt 혼합 문단(줄별 최대 10/40/10)이 여백만 0 → 10/40/10, 최소 16 → 16/40/16,
**고정 16 → 16/16/16**(40pt 줄의 baseline이 다음 줄 baseline보다 아래 — `spacing` −24), 비율
160 → 16/64/16; 상대크기 250%(실제 25pt) 줄도 상자 10·전진량 10(글리프가 넘친다 — #202의
답); 30pt 글자처럼 취급 표 줄이 비율 160 → **36**(= 30 + 10 × 0.6), 여백만 3 → 33, 최소 16 →
30, 고정 16 → 16. 우리 렌더는 이 문서 55줄 전부 한글 캐시와 0.000pt(한컴 글꼴, 줄바꿈까지
같다)다. **비율의 여분은 글자 상자 기준이다** — 개체가 상자를 정한 줄에서도 `spacing`은 그 줄
글자들의 기본 크기 × (p − 100)/100이다 (실물 캐시: `CCL` 40.87pt 로고 줄 600 = 10 × 0.6,
`noori` 61.34pt 그림 줄 840 = 12 × 0.7 · 627.01pt 표 줄 1052 = 15 × 0.7, `BinData` 199.38pt
그림 줄 600). 고정·최소·여백만 값은 표 43 여백 계열과 같은 **1/2 단위**로 저장된다 (#192 —
대화상자 16pt → 3200; `HwpLineSpacingRule.init(paraShape:)`가 나눈다).

**규칙은 조판 문자열이 나른다** — `HwpAttributedStringKey.lineSpacing`
(`HwpLineSpacingRule.attributeValue`), `HwpParagraphLayout.attachParagraphStyle(to:paraShape:tabStops:)`
가 CT 문단 스타일과 함께 단다 (`HwpTextRunBuilder.attachParagraphStyle`). 문자열을 직접 만드는
호출부(테스트 포함)도 이 함수로 붙여야 측정·렌더가 같은 자리에 줄을 놓는다. 규칙 표식이 없는
문자열(공개 `HwpPaintCommand.drawText` 호출자)은 `HwpLineSpacingRule.fallback`이 CT 스타일을
...
**문단 사이 간격은 줄이 CT 문단을 끝낼 때만 든다** (`HwpLineAdvance.paragraphGap`) — 마지막
글자가 LF·CR·U+2029이고 `hwp.lineBreak` 표식(한 줄 끝, 코드 10)이 아니면 그 문단의 아래 간격 +
다음 줄 문단의 위 간격(음수는 0). 한 줄 끝은 CT에는 문단 구분자지만 한글에서는 같은 문단의 줄
나눔이라 간격이 없다 (실측: 위/아래 10pt 문단의 한 줄 끝 앞뒤 줄이 16pt 그대로, 진짜 문단
경계는 아래 + 위 = 20). 컨테이너 블록이 문단들을 `\n`으로 이은 문자열
(`HwpCombinedBlockString.combine`)은 문단마다 자기 스타일이 붙어 있으므로 줄 자신의
문단(아래)과 다음 줄의 문단(위)에서 각각 읽는다. 청크 이월은 앞 청크 마지막 줄 상자 상단 +
그 줄 전진량(+ 문단 간격)이 다음 청크 첫 상자 상단이라 예산과 무관하다.

**측정 줄 프레임도 같은 모델이다** — `HwpLineFrame.origin.y`는 줄 상자 상단(문단 첫 줄 기준),
`baseline`은 앵커(0.85 × 상자). 문단 높이 = 위 간격 + 전진량 합(마지막 줄의 줄 간격 몫 포함 —
한글 캐시의 `lineHeight + lineSpacing` 합과 같다) + 아래 간격. 조각(`HwpFragmentLineAdvances`)의
높이는 전진량 합 그대로다 — 원점이 상자 상단이라 종전 CT baseline 원점 모델의 첫 줄 ascent
초과분 보정(#164 `ascentExcess`·`chargedHeight`)이 없다. 글자처럼 취급 개체의 세로 자리
(`HwpObjectAnchorGeometry.inlineAnchorOrigin`)도 이 모델이다 (#195): 개체의 바깥 상자를 **그
높이의 글자 하나**로 보고, 바깥 상자 상단에서 높이 × `HwpLineFrame.objectBaselineRatio` 내려간
자리를 그 줄의 앵커에 맞춘다 — 한글 문서는 글자 상자와 같은 0.85, MS 워드 호환 문서는 1(바깥
상자 바닥이 베이스라인, `LineMetrics.inlineObjectBaselineRatio`). 개체가 상자를 정한 줄(코퍼스의
전부)은 앵커 = 비율 × 개체 높이라 개체 상단 = 상자 상단이고 렌더가 그 줄 글자를 그리는 자리와
같은 기준이다; 상자보다 작은 개체는 베이스라인 위로 0.85 × 높이만 올라간다 (종전의 "바닥을
앵커에"는 한글보다 0.15 × 높이 위였다 — 이슈 #195의 40pt 줄 20pt 그림 −2.99pt). 한글 12.30
실측(2026-09-21, 합성 HWPX → PDF, 0.12pt 안): 함초롬바탕 40pt 줄의 4~50pt 그림 7단·바깥 여백
4종·상대 크기·줄 간격 5종·글꼴·정렬·한 줄의 세 그림·10/20pt 줄·표·사각형·글상자·셀 안·글상자
안·쪽에 걸친 문단 뒤 조각이 전부 0.85, 같은 표본의 MS 워드 호환 문서는 전부 1. 헌법주석 665쪽
(s24/p490)의 인용 표 줄 상자 상단이 한컴 글꼴 모드에서 캐시 그대로 662.41pt다 (수정 전 666.59).
여기서 "개체"는 **바깥 상자**(개체 + 바깥 여백, 표 70 `marginArray`)다 (#193): 줄 예약
(`HwpTextRunBuilder.inlineObjectReservation`)이 폭 + 왼·오른쪽, 높이 + 위·아래 여백을 한
...
**표의 높이는 저작 높이가 아니라 그려지는 높이다** (#214·#218). 표 높이는 셀 내용이 정하는 행의
합이라(#160) 공통 속성(표 69)의 저작 높이와 다를 수 있는데, 한글은 줄 상자·앵커를 그 실제 높이로
잡는다 — 한글 12.30.0(6446) 실측(2026-09-23, 합성 HWPX → PDF·재저장 줄 캐시): 저작 90.24·실제
410.24pt(10pt 셀 문단 32행) 표의 줄이 `vertsize` 41590(= 410.24 + 위·아래 여백 2.83 × 2)·`baseline`
0.85배라 쪽마다 표 하나(13쪽; 종전 우리는 101.90pt 간격으로 겹쳐 3쪽), 10pt 줄의 저작 8·실제 10pt
표는 줄 상단(베이스라인 − 8.5), 공통 높이 30pt가 행 합 12.82pt보다 **큰** 표도 12.82pt(최댓값이
아니다), 한 줄 두 표·40pt 줄·둘째 줄·각주 안 표도 같다. 본문보다 큰 표(769.20pt)는 나누지 않고 새
쪽 머리에 통째로 놓여 쪽 아래로 넘친다(우리 흐름 경로의 진행 보장과 같은 결과). 한글은 저장할 때
공통 높이를 실제 높이로 고쳐 쓰므로(코퍼스의 글자처럼 취급 표 14개가 전부 저작 = 실제) 저장본에서는
드러나지 않고, 합성·다른 도구 문서·저작 뒤 셀 내용이 커진 문서에서만 드러난다. 조판 문자열을 만들 때는
표 레이아웃이 없으므로 **표를 그 자리에 그리는 호출부가 높이를 넘긴다**
(`HwpTextRunBuilder.inlineTableHeights` — 넘긴 표만 저작 높이를 대신하고 폭·바깥 여백은 그대로):
본문은 측정 직전에 문단의 글자처럼 취급 표를 배치와 같은 `layoutTable`로 조판해 블록 높이
(`HwpTableFrame.flowBlockHeight`)를 넘기고(`HwpPaginator.inlineTableHeights(for:)`), 배치가 같은 표를
두 번 조판하지 않게 결과를 문단 단위로 메모한다(`InlineTableFrameMemo` — 열쇠는 문단 경로·서수·단
폭·크기 해석기, 측정 메모와 같은 입력). 각주·미주는 수집기(`HwpParagraphObjectCollector.table`)와 같은
입력으로 외곽 높이를 넘긴다(`HwpTableLayout.inlineTableHeights(in:…)`). 조각이 폭이 다른 단에 놓이면
표 높이도 그 단에서 다시 잡는다(`rescaledInlineTableHeights` — 아래 "비등폭 단" 항목). **표를 실은
줄 캐시가 그 표(+ 위·아래 바깥 여백)보다 낮으면 캐시가 낡았다**(`HwpParagraphLayout.lineCacheIsStale`,
#214 PR 리뷰 — 저장 뒤 셀 내용이 커진 문서): 줄 예약은 그려지는 높이라 CT 줄은 표만큼 커지는데, 캐시
높이로 전진하면 블록은 캐시만큼만 내려가고 표는 실제 높이로 그려져 뒤 문단이 표를 덮는다. 한글도 그런
문서를 열 때 캐시를 버리고 다시 조판한다 — 한글 12.30 실측(2026-09-23): 한글이 저장한 문서의 4행 표
(51.28pt) 셀마다 문단 다섯을 더해 371.28pt로 키운 사본(호스트 줄 캐시 `vertsize` 5694 그대로)에서 한글은
표 줄을 37694로 다시 저장하고 뒤 문단을 320pt 내려 그린다(1단·2단·위 10/아래 5pt 문단 간격·두 단에
걸친 구역 첫 문단 모두 같다). 그래서 본문의 세 캐시 소비처가 같은 술어로 캐시를 버린다: 흐름 배치의
문단 높이(`lineCacheHeightIsUsable` → CT 높이), 비절대 모드 다단 캐시 run 배분(`placeCachedColumnRuns`
→ 줄 단위 배분 — 표 높이는 그 표를 실은 줄의 **단 폭**에서 잰다, `columnRunInlineTableHeights`; 위치를
못 풀면 run 단 폭 가운데 가장 낮은 높이), 절대 캐시의 낡은 캐시 보정(`staleAdjustedHeight` — 블록을
CT **줄 범위**(문단 높이 − 저작 위·아래 간격)로 키우고 뒤 문단을 그 차이만큼 민다; 간격은 캐시 위치에
이미 들어 있어 CT 문단 높이로 밀면 두 간격만큼 더 내려간다 — 같은 실측의 간격 문단에서 한글도 320pt만
밀었다). 밀린 문단이 쪽 아래를 넘으면 쪽을 넘겨 새 쪽 머리에 놓고, 쪽에 걸친(여러 run) 문단은 보정
대상이 아니다 (아래 "stale 캐시 보정" 항목). **신선한 캐시를 버리지 않는 경계가 둘이다** — 판정은 우리
레이아웃을 한글 캐시와 견주므로 그것을 믿을 수 없는 자리에서는 캐시를 따른다: ① 행 병합으로만 덮인 행이
있는 표(`HwpParagraphLayout.rowsHaveOwnCells` 거짓)는 견주지 않는다 — 우리 표 레이아웃이 그 행에
기본 높이를 얹어(`HwpTableLayoutFrames.resolvedRowHeights`) 한글보다 높게 잡으므로(1열 2행 병합 표: 한글
20pt, 우리 22.82pt; 두 행 병합 이름 칸 옆 값 칸 양식 꼴은 차이가 더 크다) 견주면 뒤 문단을 민다. 그 행
높이 결함 자체는 별건이다. ② 흐름 배치는 CT 높이가 캐시보다 클 때만 캐시를 버린다(절대 캐시 보정의
`lineExtent > height`와 같다) — 공통 폭이 0이라 줄이 표를 예약하지 못한 문단은 CT가 더 낮아, 버리면 블록이
줄어 겹침이 커진다. 한글이 저장한 캐시는 표를 담으므로 코퍼스에서는 걸리지
않는다. 가드: `HwpInlineTableStaleLineCacheTests`·`HwpInlineTableStaleLineCachePageTests`·
`HwpInlineTableStaleLineCacheGuardTests`. **표 셀·
글상자·머리말/꼬리말은 넘기지 않는다**: 셀 안 중첩 표는 줄 앵커가 아니라 문단 **아래**에 쌓여
(`HwpTableLayoutFrames.nestedFrames`) 줄 예약과 이미 이중 계산인데 예약을 키우면 빈자리만 커지고,
글상자·머리말 안 표는 그 자리에 그려지지 않는다 — 한글은 셀 안에서도 실제 높이로 줄을 잡고 표를 줄
안에 둔다(같은 실측: 셀 줄 `vertsize` 5128, 바깥 표 7010). 그쪽은 앵커 배치와 함께 고칠 몫이다.
...
`FixtureInlineTableFragmentTests`, `HwpParagraphLayoutTests`(고정 3600 → 18pt, 여백만 500 →
2.5pt). 렌더 해시(로컬 기준선)는 이 변경으로 `CCL` 1쪽과 헌법주석 20쪽이 바뀌었다 — 전부
각주 문단의 줄 피치가 CT 정수 분할 드리프트(12.7/10.7pt 혼재)에서 캐시와 같은 11.7pt 균일로
...
줄 상자 모델 자체가 다르다 — `track-changes`의 캐시 `vertsize=1692`·`baseline=1266`은 **글꼴 줄 상자**
(`HwpMsWordLineBox`)이고 비율 160%도 그 값에 걸린다. 장식 선의 축 (#187) 과 같은 상자를 세로 배치도
읽는다 (아래 장식 항목의 #194 절). ④ (해결, #195) 상자보다
**작은** 글자처럼 취급 개체의 세로 자리는 바깥 상자를 글자로 보는 위 규칙으로 고쳤다 — 내용이
저작 높이보다 키운 표(한글은 실제 높이의 0.85)는 #214에서 본문·각주를 고쳤고(위 "표의 높이" 항목,
표 셀·글상자·머리말/꼬리말 안은 남았다), **개체를 실은 마커 run의 글자 모양이 본문보다 클 때의 줄
상자**도 해결했다 (#217 — 한글은 줄 공간을 예약한 개체 마커의 크기를 `vertsize`에 넣지 않고 비율
여분 기준에만 넣고, 높이 0 마커(책갈피)의 크기는 `vertsize`에도 넣는다; 위 "마커 run도 자기 글자
모양을 싣는다" 문단). ⑤ 상자보다 큰 글자의 잉크가 블록 프레임 위로 나가는데 최상위 텍스트 블록의 히트 자격에는
세로 여유가 없다 (#196) — 한글도 그 자리에 그리므로 잉크가 아니라 자격을 넓혀야 한다.
⑥ (해결, #207) 1단 본문 문단도 쪽 끝에서 **들어가는 줄까지 남기고** 나머지를 다음 쪽에 잇는다 —
아래 "쪽 끝 줄 단위 분할" 절. (해결, #222) 쪽 끝 적합 판정도 전진량이 아니라 줄 **상자**
하단까지다 — 같은 절. 남은 격차: 첫 줄과 그 각주가 함께 안 들어가면 한글은
줄을 남기고 각주만 이월하지만 우리는 문단을 통째로 옮긴다 (한글 12.30 실측 MB·ME, 미등록).
빈 쪽에서도 같다 — 줄의 각주가 그 줄과 함께 안 들어가면 한글은 줄을 모두 남기고 각주를 본문 아래
남은 자리까지만 싣고 나머지를 다음 쪽에 잇지만(2026-09-24 실측, 20pt·160% 20줄 + 마지막 줄 두 줄
각주 → 20줄 + 각주 첫 줄), 우리는 그 줄을 각주와 함께 다음 쪽으로 넘기고, 진입 분할 게이트를
못 지나는 문단은 문단 높이가 단에 들어가면 통째로 놓아 각주가 본문을 덮는다(#222 이전과 같다);
쪽마다 각주 번호를 새로 시작하는 구역(표 134 numberingMode 2)의 각주 문단은 뒤 조각 참조
번호를 다시 매기는 기계가 흐름 경로에 없어 나누지 않는다.
...
구역 정의(표 130)의 머리말/꼬리말/쪽 번호 감추기는 **구역 첫 쪽에서만** 표 145
마스크와 합쳐 소비된다.

**stale 캐시 보정**: 캐시 줄 높이 (h)보다 큰 글자 크기가 선언된 문단(`cacheIsStale`)과
글자처럼 취급 표를 실은 줄이 그 표보다 낮은 문단(`HwpParagraphLayout.lineCacheIsStale`, #214 PR
리뷰 — 위 "표의 높이는 저작 높이가 아니라" 항목)은 캐시가 저장 당시와 안 맞는 것 — 한글.app도 열 때
재조판한다 (CharShape 실측: 상대크기 170 줄, 표 쪽은 #214 PR 리뷰 실측). 절대 캐시 모드에서 그
문단만 CT **줄 범위**(CT 문단 높이 − 저작 위·아래 간격 — 두 간격은 캐시 위치에 이미 들어 있다)로
넓히고 이후 문단을 페이지-로컬 오프셋으로 민다 (`staleLineExtent`). 두 판정 모두 신선한 캐시(h ≥
글자 크기이고 표를 담는 줄)에서는 발동하지 않는다 — 헌법주석 페이지 절단 유지. 여기서 글자 크기는
줄 공간을 예약한 개체 마커의 글자 모양을 빼고 문단 끝 글자(CR)의 기본 크기를 더한 값이다 (#217 —
한글은 마커 크기를 줄 상자에 넣지 않아 신선한 캐시의 h가 그보다 작고, CR 크기는 마지막 줄 상자에
넣는다(#206). 마커만 빼고 CR을 안 보면 40pt 마커의 8pt 그림 + 40pt CR 문단의 10pt 캐시를 신선하다고
믿어, 그려지는 40pt 줄이 뒤 문단을 덮었다 — PR 리뷰). 캐시의 쪽 절단점은
보정 **전** 자리로 정해졌으므로, 보정으로 밀린 문단의 첫 run 줄 상자가 본문 아래를 넘으면 쪽을
넘기고 그 문단을 새 쪽 머리에 놓는다(`staleShiftOverflows` → `pendingRebaseLocation`: 새 쪽의 보정을
그 문단의 캐시 위치만큼 당긴다 — 캐시 위치는 옛 쪽 기준이다). 같은 쪽의 뒤 캐시 문단은 캐시 간격을
그대로 잇고, 캐시의 다음 쪽 절단점에서 보정이 풀린다 — 그래서 한글이 두 쪽을 합쳐 다시 흘리는 곳에서
우리는 쪽이 하나 늘 수 있지만 글이 종이 밖으로 사라지지는 않는다. 한글 12.30 실측(2026-09-23): 앞
표가 563.28pt로 커진 문서의 뒤 표 문단은 2쪽 머리에, 제 표가 627.28pt로 커져 남은 자리에 안 들어가는
문단은 2쪽 머리에 통째로 놓이고 그 뒤 문단은 3쪽 머리다 — 둘 다 한글 PDF와 0.1pt 안. 여러 run으로
나뉜 문단(쪽에 걸친 문단)은 보정 대상이 아니다 — 조각마다 CT 높이를 다시 배분할 수 없어 그 문단의
커진 표는 뒤 줄을 덮는 종전 동작이다.

**절대 캐시 모드** (`detectAbsoluteCacheMode`: 첫 loc > 0인 캐시 문단이 다수):
1단 문단을 캐시가 준 y에 그대로 배치하고, 세그먼트 loc이 줄어드는 지점
(`cacheRuns`)을 한글의 페이지 절단점으로 사용한다. **같은 loc도 절단점일 수 있다** (#214,
`HwpAbsoluteCachePlacer.isPageBreak`): 앞 세그먼트가 자리를 차지했는데(전진량 lineHeight +
lineSpacing > 0) 같은 위치에서 **새 줄**(표 62 bit 17)이 시작하면 새 쪽이다 — 쪽을 채우는 글자처럼
취급 표를 품은 문단이 잇달면 한글은 문단마다 쪽 머리(0)에 저장하는데, loc만 보면 0 → 0이 절단점으로
안 보여 `inline-table-actual-height`의 32행 표 셋이 한 쪽에 겹쳤다. 예외가 둘이다: 한 줄이 여러
세그먼트로 나뉜 줄(어울림 개체 양옆 — 이어지는 세그먼트는 bit 17이 꺼짐, 각주 줄 캐시
`HwpFootnoteCacheLines.lines`와 같은 규칙)과, 쪽 중간에 새로 열린 단 밴드의 첫 캐시 문단이다. 한글은
밴드마다 loc을 밴드 상단부터 다시 센다(`Column` 쌍의 밴드 첫 문단이 모두 0) — 그래서
`openColumnBand`가 앞 밴드의 loc·전진량 기록(과 낡은 캐시 보정 — 새 밴드 상단이 이미 담았다)을
지우고, 그 밴드의 캐시 문단은 쪽이 넘어갈 때까지 위치에 더해 **자리**로도 쪽 넘김을 판정한다
(`bandStartOverflows`: 첫 run — 한글이 한 쪽에 놓은 줄들 — 의 줄 상자가 캐시 자리에서 본문(각주 예약
제외) 아래를 넘으면 새 쪽). 종전에는 앞 밴드 마지막 줄 loc(> 0)과 견줘 `0 < 19200`으로 쪽을 넘겨,
1단 → 다단 → 1단 밴드가 한 쪽에 있는 한글 저장본(한글 1쪽)이 2쪽이 됐고, 기록을 지우기만 하면 쪽을
채운 문단 뒤의 단 정의 문단(쪽 머리 0)이 같은 쪽에 겹친다(한글 4쪽 → 3쪽) — 두 사례 모두 #214 2차
리뷰가 한글 12.30 저장본으로 확인했다. 첫 줄만 보거나 밴드 첫 문단에서 판정을 끄면, 첫 줄은
들어가도 한글이 문단 보호·외톨이줄 보호로 통째로 옮긴 여러 줄 문단(#207)과 그 뒤 문단들이 위치가
늘기만 해서 본문 아래로 넘친다 (3차 리뷰). 절대 캐시 모드 문서의 문단 사이·문단 안 전이 전수(헌법주석 캐시 문단 11,056개
포함)에서 같은 loc 규칙이 판정을 바꾸는 곳은 그 픽스처의 두 곳뿐이다 (절대 캐시 모드가 아닌 문서의
다단 캐시 run은 스캔하지 않았다 — `section-marks`의 0 → 0은 쪽 나누기 문단이라 무관). 여러 run에 걸친 문단은
...
셀 문단 전부가 캐시로 측정된 셀은 저작된 셀 높이 (표 80)를 신뢰하되,
**문단 기준으로 떠 있는 개체 높이를 하한으로 얹는다** — 그 개체는 줄 상자에
없어 캐시에도 저작 높이에도 잡히지 않는다 (#91, 아래 "한계·미지원" 참조).
저작 높이가 **캐시 줄 상자를 쌓은 높이 + 위아래 안쪽 여백**(`PlacedCell.cachedCellHeight`,
`HwpTableLayout.cachedLineBoxHeight` — 셀 문단의 측정 높이(캐시 전진량 + 문단 위/아래
간격)를 배치와 같은 순서로 쌓고 마지막 줄의 줄 간격과 마지막 문단의 아래 간격을 뺀 값,
즉 배치가 그리는 마지막 줄 상자의 아래)보다 작으면 그 값으로 올린다 (#160·#193). 한컴오피스 한글 12.30 macOS는
`표 만들기`로 만든 셀의 높이를 내용과 무관하게 **여백 합 282 HWPUNIT**로 저장하므로
(`numbering-sequence` 쌍과 2026-09-07 실측 2×2 표 HWP·HWPX 저장본의 4셀 전부 —
`Sources/CoreHwp/AGENTS.md` 표 80 항목) 그대로 믿으면 행이 2.82pt로 접혀 셀 글자가
다음 문단과 겹친다. 경계는 줄 **간격을 뺀** 줄 상자다 — 줄 간격까지 든
`cachedParagraphHeight`를 경계로 삼으면 저작 높이가 살짝 아래인 정상 셀(헌법주석
25·noori 8)까지 부푼다. 캐시 위치가 아니라 측정 높이를 쌓는 이유는 배치와 같은
산술이어야 하한이 그려진 글자를 실제로 담기 때문이다(첫 문단의 문단 위 간격, 누적이
깨진 캐시) — 실물 셀 문단의 `lineLocation`은 셀 안에서 **문단을 넘어 누적**되고
(헌법주석 s22/p201 5문단 셀 0·1300·…·10400, noori 0·2700·6000·9000, 새 표 2문단 셀
0·1600) 그 범위가 이 합과 같다. 중첩 표는 하한에 세지 않는다. 한글.app 실측(155%
줌, 창 캡처 4.137px/pt): numbering-sequence 3쪽 행 12.81pt(우리 12.82), 새 표 1행 4줄
60.67pt·2행 2문단 29.01pt(우리 60.82·28.82 — 표 공통 속성 height 8964 = 6082 + 2882가
그 합이다). 헌법주석 s24/p490의 글자처럼 취급 표 3개(저작 1000 < 2줄 1108, 표 공통
속성 1108)가 10 → 11.08pt로 바뀌지만 1,030쪽 핀·블록 스냅샷·noori 골든은 그대로다
(레코딩 머신의 렌더 해시 기준선은 origin/main 대비 헌법주석 p666·numbering-sequence
p2 두 쪽만 갈려 재기록했다).

**컨테이너 내용 범위** (#193, `HwpContainerContentExtent`): 표 셀·글상자의 내용은 첫
문단의 위 간격부터 마지막 줄 **상자** 아래까지다 — 마지막 줄의 줄 간격 여분과 마지막
문단의 아래 간격은 범위 밖이다. 세로 정렬(표 89 가운데·아래)의 여유와 내용이 정하는 행
...
10pt 모두 같고, 아래 정렬은 상자 바닥이 안쪽 아래, 위 간격 10pt는 범위에 든다. **기준은
마지막 줄 하나다** — 고정 줄 간격이 앞 줄 상자보다 작아 줄이 겹치면 앞 줄이 더 아래까지
내려가는데, 한글은 그 줄을 컨테이너 밖으로 흘려 보낸다 (2026-09-20 실측: 30pt + 10pt 두 줄·
고정 16pt 셀에서 아래 정렬 30pt 줄이 셀 아래로 4pt 넘고, 저작 282 행이 26 + 2.82 = 28.80pt).
그래서 캐시 경로의 `CachedLineExtent.bottom`도 최댓값이 아니라 **마지막 세그먼트**의 줄 상자
아래다 — 최댓값이면 같은 문서에서 4pt 어긋난다. 행: 저작
282 셀에 아래 간격 10pt 문단 → 12.82pt, 두 문단이 각자 아래 간격 10pt면 38.82pt(첫 문단 몫만).
종전에는 문단 rect 바닥을 내용 끝으로 써서 가운데 정렬이 (여분 + 아래 간격)/2만큼 위였다.
행 분할(`HwpTableSplitter`)은 이 범위를 쓰지 않는다 — 절단선은 줄 **전진량** 경계에 맞추고
셀 문단 rect도 전진량 합이라, 전진량이 상자보다 작은 문단(비율 100% 미만·작은 고정값)은 어느
줄 경계에서 자르든 바로 위 줄 상자가 (상자 − 전진량)만큼 절단선 아래로 내려간다. 절단선이
마지막 줄 상자 안에 들어 그 줄이 통째로 위 조각에 남는 것도 같은 크기의 잔여다 (PR 리뷰 —
마지막 문단 rect를 상자 아래까지 늘려 그 줄을 넘기면 바로 앞 줄이 같은 몫만큼 넘치고 쪽
채움만 준다). 상자를 기준으로 자르는 모델(조각마다 마지막 줄 상자까지 예약)은 한글.app 실측
뒤에 — 미착수 격차.
...
**쪽 끝 줄 단위 분할** (#207): 1단 흐름 배치(`placeFlowParagraph`)는 문단 전체가 남은
자리에 안 들어가면 첫 조각의 줄 수를 `entryFragmentLineCount`로 재어 하나라도 남으면
`appendParagraphAcrossColumns`로 그 줄들을 현재 쪽에 두고 나머지를 다음 쪽에 잇고, 한 줄도
못 남기면 종전대로 쪽을 확정하고 다음 쪽에서 다시 처리한다(`return false` — 재처리가 각주
참조 번호·자동 쪽 번호·진단 쪽을 새 쪽 기준으로 다시 잡으므로 이 경로는 남긴다). 진입 판정과
조각 루프의 첫 반복은 **같은 함수·입력**(`HwpFragmentRemainder.fit(in:policy:columnIsEmpty:
extra:)`)이라 갈리지 않는다. 한글 12.30 실측(2026-09-21, 합성 HWPX → 한글 재저장 줄 캐시,
`probes/207`)의 규칙 — 보호 없음: 들어가는 줄을 그대로 남긴다(74줄 문단도 첫 줄만 남는
자리면 첫 줄을 남긴다); **외톨이줄 보호**(표 44 bit 16, `HwpParagraphSplitPolicy`): 경계 양쪽
최소 두 줄, 남길 줄 = min(k, n − 2)이고 2 미만이면 통째(5줄 k=2 → 2+3, k=3·4 → 3+2, 4줄 k=3
→ 2+2, 3줄·2줄은 통째, 74줄 k=1도 통째 뒤 새 쪽에서 41+33); **문단 보호**(bit 18): 부분 채운
쪽에서는 통째, 빈 쪽보다 길면 새 쪽에서 나뉜다; 문단 위 간격은 첫 줄과 함께 판정하고
(12 + 16 ≤ 33.62 → 첫 줄이 723.2 + 12) 통째로 옮기면 새 쪽 머리에도 든다(99.2 + 12 = 111.2);
저장본 줄 캐시 높이를 따르는 문단은 CT 줄 수 == 캐시 줄 수일 때만 나눈다(다르면 마지막 줄의
전진량이 캐시 잔여 1pt 하한이라 판정이 관대하다). **각주는 참조 줄의 쪽에 실리고 그 줄의 적합
판정에 든다**(`HwpFlowFragmentFootnotes` — 절대 캐시 run 귀속 #95의 흐름판): 줄마다 "그
줄까지 그려진 마지막 컨트롤 서수 + 1"을 두고 커서부터 그 값까지가 조각 범위이며, 예약은
`anticipatedFootnoteHeight(ordinals:collectsNested:)`로 같은 술어(비최종 조각 = 범위 안 직접
각주, 최종 조각 = 범위 밖 컨테이너의 중첩 각주까지)로 잰다 — 남은 49.62에 4줄 문단의 첫 줄 뒤
각주 24.17이 들어가면 1줄 + 각주만 남는다(한글 MA). 마커 ↔ 컨트롤이 어긋난 문단과 쪽별 번호
모드 2는 문맥 nil → 문단 전체 예약 + 문단 단위 수집 + 각주 문단은 통째 이동(종전).

**쪽 끝 적합은 줄 상자까지** (#222): 줄을 쪽·단에 남길지는 그 줄의 **상자 하단**(앞 줄들의
전진량 합 + `HwpFragmentLineAdvances.fitHeight` — 줄 상자 높이)이 남은 자리에 들어가는지로
판정하고, 줄 간격 여분과 문단 아래 간격은 쪽·단 아래로 넘쳐도 된다. 조각의 각주 예약은 그 줄
상자가 아니라 **지금까지 놓은 줄 상자 하단의 최댓값**에 더한다(PR 리뷰 — 고정 줄 간격이 앞 줄
상자보다 작으면 앞 줄이 더 아래로 내려가 뒤 줄의 각주 영역과 겹친다; 각주가 없으면 결과가 같다). 방출 높이(블록 높이·커서
전진)는 그대로 전진량 합이라 판정과 방출이 갈린다 — `fit`의 조건만 상자고 반환 높이는 전진량이다.
한글 12.30 실측(2026-09-21·24, `probes/206` `so206-pagefit`·`probes/222` `so222`·`so222e`·
`so222c`·`so222d`, 줄 캐시 없는 합성 HWPX를 한글이 다시 저장한 `linesegarray`와 PDF — 1단 사례
전부와 2단 `CX`·`CZ`의 대상 줄·`DA` 구분선이 우리 재조판과 일치한다; 2단 문서의 나머지 차이는 이
규칙과 무관한 기존 격차다: 쪽 나누기로 닫히는 밴드를 우리만 균형 재배치하고(`CZA`), 단 나누기(문단
헤더 `columnType` bit 3)를 조판이 읽지 않는다(`DB`·`DC`)): 남은 17.62에 16pt·160% 한 줄(상자 16, 전진량 25.6)은 남고 18pt는 넘어간다;
**상자 하단이 본문 하단과 같으면 넘긴다**(17.62pt 줄 → 넘김, 17.61pt → 남김; 줄 간격 100%로
전진량까지 같아도, 고정 줄 간격 줄·세 줄 문단의 마지막 줄도 같다) — 그래서 판정은 엄격 부등호
(`HwpPageEndFit.fits`, 부동소수 오차는 HWPUNIT의 절반 0.005pt로 흡수); **고정 줄 간격이 상자보다
작은 줄도 상자**다(상자 10·전진량 8인 줄을 남은 9에서 넘긴다 — `min(전진량, 상자)`로 잡으면
틀린다); 문단 아래 간격 20pt도 판정에 안 든다; 문단 보호 세 줄은 상자가 다 들어가면 남고, 외톨이줄
...
각주 예약을 뺀 본문 높이로 판정해, 안 들어가면 조각 루프로 보낸다 — 단 전체 높이와만 견주면 문단
자신의 각주가 마지막 줄을 덮는다, PR 리뷰. 진입 분할 게이트 `canSplitAtEntry`를 못 지나는 문단은
상자가 각주 위에 안 들어가도 기준(#222 이전)이 나누던 자리, 곧 문단 높이가 단보다 클 때만 나눈다 —
조각별 각주 귀속이 없어 더 넓게 나누면 각주가 마지막 조각 쪽으로 가고 문단 전체 예약이 쪽마다
...
바닥에서 잰다. 고정 줄 간격이 상자보다 작아 상자가 블록 아래로 나가면 그 몫은 **음수**다 — 쓰는 쪽이
상자 바닥까지 잰다(PR 리뷰 — 한글 12.30 실측 `so222n`: 12pt·고정 8pt 줄로 끝나는 균형 배분 밴드의
구분선은 전진량 끝이 아니라 상자 바닥 44에서 끝난다; 변경 막대도 그 상자까지 긋고(미실측 — 그려진
글줄 옆에 막대를 두는 규약), 측정 규칙 `measuredTrailingSpacing`도 같이 음수를 낸다). 줄 캐시로 높이를
잡는 블록(절대 캐시 run·단별 캐시 run·캐시 높이 흐름 문단)은 음수 줄 간격을 0으로 잡아
(`HwpAbsoluteCachePlacer.lineAdvance`) 블록이 이미 상자 바닥까지라 몫이 0 이상이다 — 단별 캐시 run
블록은 그래서 음수 간격에도 0 표식을 단다(없으면 음수 측정 규칙이 구분선을 상자 아래로 더 내린다). 균형 재배치가 새로 만든 조각은 원래 블록의 끝을 담았으면 그 기록을 물려받고, 다른
단 폭으로 다시 잰 조각은 다시 잰 줄로 잰다(`RebalancePlan.newBlockLineBoxGaps`, PR 리뷰 — 한글
12.30 실측 `so222r`: 쪽 중간의 단 정의로 닫혀 3 + 3으로 배분된 2단 밴드도 구분선이 마지막 문단의
아래 간격 20pt와 무관하게 줄 상자 바닥에서 끝난다; 기록이 없으면 측정 규칙이 줄 간격만 빼 그 간격만큼
길었다). 변경 추적 막대도 넘친 몫에서 멈춘다 — 본문 하단(각주가 있는 쪽은 각주 영역 상단,
`PendingPlacement.paintedTop` — 구분선 획이 위 여백보다 굵으면 그 획 상단으로 예약 경계와 같다: 쪽 확정이
각주 자리를 막대보다 먼저 잡고 블록은 종전 순서로 붙인다)에서
끊되, 기록된 줄 상자는 자르지 않는다(PR 리뷰 — 각주를 덮는 남은 격차의 문단도 그 줄에 막대가 선다).
같은 실측에서 줄 캐시 없는 각주의 스택도 마지막 줄 상자까지로 바로잡았다
(아래 각주 ③ — 종전에는 CT 각주의 줄 간격 여분 6pt가 영역에 들어 본문이 그만큼 먼저 넘어갔다).
...
스냅)로 나뉜다. 비례 환산은 조판 문자열 앞에 전치된 생성 라벨(#154,
`HwpTextRunBuilder.numberingLabelLength`)을 뺀 본문 길이에만 걸고 접두를
더한다 — 라벨은 WCHAR 스트림에 없어 전체 길이로 환산하면 경계가 앞으로 당겨진다. 캐시가 없으면 라인 단위로 단을 채우고, 본문 텍스트가 첫
...
- treatAsChar + 문단 라인에 U+FFFC 앵커 존재: 그 줄 위치에 인라인 배치.
  ...
  **표도 포함** (`appendInlineAnchoredTable` — noori 보도자료 표 실측: 캐시 줄
  높이 = 표 높이 + 위·아래 바깥 여백, 표는 왼쪽·위쪽 여백만큼 안 — #193). 그 표 높이는
  저작 높이가 아니라 **표 레이아웃이 낸 높이**다 (#214 — 줄 예약이 블록과 같은
  `flowBlockHeight`를 쓴다). **개체가 자기
  문단 text 블록과 겹치는 것이 정상** (overlap 검사는 text-text 쌍만)
- treatAsChar (앵커 없음) 또는 textWrap ∈ {square, tight, through, topAndBottom}: 흐름 위치에 배치 + 높이 소비. 앵커 없는 표는 이 경로 (row 분할 유지)
- **자리 차지 표는 문단 글줄 앞 띠가 우선이다** (#161, `appendBandedTableIfNeeded`).
  한글 12.30은 세로 기준이 '문단'인 자리 차지(`topAndBottom`) 표를 그 문단의 글줄
  **위**에 두고 문단 줄을 표 높이 + 위·아래 바깥 여백만큼 내리는데, 그 결정이 줄
  캐시에 남는다 — 문단 진입 시점의 흐름 위치와 `paragraphAnchorTop` 사이가 표 높이 +
  여백 이상이면 그 띠가 표 자리다. 기준점은 `paragraphAnchorTop`이 **아니다**
  (절대 모드에선 캐시 y라 이미 띠 뒤 자리다) — `processParagraph`가 진입 시점 흐름
  위치를 `paragraphEntryFlow`에 잡아 둔다. 게이트는 문서 전체 판정인
  `absoluteCacheMode`가 **아니라 이 문단이 실제로 캐시로 배치됐는지**다
  (`placedFromCache` — 같은 문서에도 캐시 없는 문단이 있고 `placeParagraphText`가
  그것을 흐름 배치한다; 그 문단의 간격은 문단 위 간격이라 띠가 아니다). 한 문단에
  조건을 만족하는 표가 여럿이면 앞 표가 쓴 자리(`bandUsed`) 아래 남은 띠만 본다 —
  안 들어가면 흐름 배치로 돌아간다. 컨테이너 안(글상자 등) 문단의 표는 앵커 문맥이
  없어(`controlIndex == nil`) 아예 대상이 아니다.
  **대상은 `topAndBottom` 하나다** — `consumesFlow`로 게이트하면 어울림(`square`)과
  미지 값(nil)까지 들어오는데, 한글은 어울림 표 **옆**으로 글을 흘리므로 같은 띠
  규칙이 성립하는지 실측한 적이 없다.
  **앞 문단의 아래 간격은 띠가 아니다** — 절대 캐시 배치는 그 여백을 커서로 소비하지
  않고 캐시 줄 위치에 이미 담고 있어서, 빼지 않으면 앞 문단이 의도한 여백에 표가 들어간다
  (한글 12.30 실측 2026-09-16: 앞 문단 아래 간격 20pt는 표 **앞**에 그대로 든다 — 캐시
  60412 → 62412). 띠는 그 간격 **뒤**에서 시작한다(`precedingSpacingBottom`/
  `authoredAfterGap`) — 절대 캐시 블록 높이(`absoluteRunBlockHeight`)는 줄 위치·높이·줄
  간격만으로 만들어져 아래 간격을 담지 않으므로 그 몫이 `entry.top`과 캐시 줄 사이에
  남는다(밴드가 열리면 앞 문단이 다른 밴드의 것이라 0으로 되돌린다). **앞 문단의 몫은 그
  문단이 캐시로 배치됐을 때만 뺀다** — 흐름 배치는 `paragraphHeight`(= `totalHeight`,
  CT가 `paragraphSpacing`으로 담는다)로 커서를 전진시켜 아래 간격을 **이미 소비**했으므로,
  또 빼면 같은 여백이 두 번 빠져 유효한 띠가 좁다고 오판되고 표가 글줄 뒤로 밀려 뒷
  문단을 덮는다(절대 캐시 문서에도 캐시 없는 문단이 있다 — `track-changes` s0p0이
  실물이다). **이 문단 자신의 위 간격은 빼지 않는다** (#190 실측, 1단·2단 모두): 한글은
  자리 차지 표를 품은 문단의 위 간격을 표 앞에도 표와 글줄 사이에도 쓰지 않아(캐시 간격
  4412 = 283 + 3×1282 + 283, 간격 없는 문단과 같다) 캐시 간격이 곧 표 + 여백이고, 빼면
  그런 문단의 띠가 좁다고 오판돼 표가 글줄 뒤로 밀린다(#161 때는 미실측이라 뺐었다).
  **적합성은 방출할 블록 높이로 잰다** — `rowFrame.maxY` 최댓값은 셀 간격
  (표 76 `cellSpacing`)이 있는 표에서 첫 행 앞 간격 한 칸을 더 담는데
  (`HwpTableLayoutFrames`의 `yOffset = spacing`), 방출은 `HwpTableSplitter.segmentFrame`이
  그것을 0으로 정규화한다. 판정과 방출이 갈리면 들어가는 표를 거부해 같은 겹침을 만든다.
  **그 정규화 자체는 별건이다** — 세그먼트 방출은 표 외곽 높이보다 앞뒤 간격 2칸만큼
  짧아서(실측: 외곽 35.66 vs 방출 30.00, `cellSpacing` 283) 같은 표가 글자처럼 취급·
  글 앞뒤로 경로(32.83)와도 높이가 다르고 `.split`·`.none` 분기의 쪽 넘김도 그만큼
  이르다. 한글이 그런 표에 실제로 몇 pt를 예약하는지는 실측 표본이 없어(저장소 표
  17개 전부 `cellSpacing` 0) 미확정이다. 간격이 있는 문단에서 표가 간격 앞·뒤 어디에 놓이는지는
  미실측이고, 실물(`numbering-sequence` s2p1·s2p2, 헌법주석 두 자리)은 위·아래 모두 0이다.
  **앞선 표가 글줄 자리로 갔으면 띠를 닫는다**(`bandClosed`) — 띠에 못 든 표가 흐름으로
  가거나 줄 안 앵커에 놓인 뒤 그 다음 표를 띠로 되감으면, 뒤 표가 앞 표보다 위에
  그려져 문서 순서와 그리는 순서가 뒤집힌다.
  표를 띠에 놓아도 **블록 배열 순서는 문서 순서 그대로**이고(선택·복사 단위 순서)
  흐름 커서는 `max(원래 자리, 표 아래 + 여백)`으로 되돌린다. 2007 계열 저장본은 표를
  글줄 **뒤**에 두는데(헌법주석 구역 22 문단 201·구역 25 문단 45) 간격이 요구량에 못
  미쳐 자연히 이 경로를 안 탄다.
- **흐름 배치 문단의 자리 차지 표는 글줄 앞에 흘린다** (#190, `appendTablesPrecedingText`).
  다단 밴드는 절대 캐시 모드라도 흐름 배치(`appendParagraphAcrossColumns`)를 타고 캐시
  없는 문단도 그렇다 — 그런 문단에는 띠(캐시 간격)가 없으므로 판정이 아니라 **순서**가
  답이다: `processParagraph`가 `placeParagraphText` **앞**에서 `precedesParagraphText`
  (띠 판정과 같은 술어 — 자리 차지·세로 기준 문단·오프셋 0·바깥 여백 넷)에 맞는 표를
  `appendTableSegments`로 먼저 흘리고 글줄이 그 뒤를 따른다. 두 경로의 갈림은
  `absoluteCacheRuns(for:)` 하나가 소유한다 (배치와 판정이 같은 술어). 놓은 서수는
  `tablesPlacedBeforeText`(열쇠는 그 문단의 `HwpParagraphPath`)에 남겨 문단 단위 방출과
  진단(`controlsPlacedBeforeParagraphEnd`)이 건너뛴다 — 진단 쪽은 표의 첫 조각이 **실제로
  놓인** 쪽(`appendTableSegments`의 반환값)이다(조각을 내기 전에 쪽을 넘길 수 있어 놓기 전
  값은 한 쪽 이르다). **측정 앞에 놓는다**: 표가 단·쪽을 넘기면 글줄은 그 단 폭으로 재야
  하고(비등폭 단), 표 셀 각주가 번호를 먼저 가져가므로 본문 각주 참조 미리보기도 그 뒤
  카운터로 구워야 참조와 각주가 같은 번호를 쓴다(PR 리뷰 — 뒤에 놓으면 참조 1)·각주 2)).
  글줄이 표 뒤에서 쪽에 안 들어가면 종전대로 문단을 다음 쪽에서 다시 처리하되
  `appendTablesPrecedingText`가 같은 문단이면 건너뛰어 표가 두 번 놓이지 않고, 글줄은 그
  쪽의 쪽 번호·각주 카운터로 다시 재어진다. **그 재시도는 문단 진입 처리 — 구역 시작·쪽
  나누기 확정(`flushPageBeforeProcessing`), 구역·단 정의, 새 번호 지정 — 를 되풀이하지
  않는다**(`isRetryingParagraphWithPlacedTables`) — 첫 시도에서 이미 적용됐고 표가 그 상태로
  쪽을 넘겼다. 되풀이하면 구역 시작 번호가 다시 놓여 쪽 번호가 7·7이 되고(구역 첫 문단의
  표가 쪽을 채운 경우 — 종전엔 구역 첫 문단이 빈 쪽에서 재시도될 일이 없었다), 새 번호 7을
  셀 각주가 쓴 뒤 본문 각주가 다시 7)을 받으며, 구역 시작의 미주 확정이 표 셀의 미주를 새
  쪽 머리에 낸다(PR 리뷰 2·3차). 표를 놓지 않은 문단의 재시도는 종전대로 되풀이한다(아직
  아무 번호도 소비되지 않아 무해하고, 쪽마다 새로 시작하는 문서는 `cacheCurrentPage`의 리셋
  뒤 다시 적용돼야 한다). **새 쪽 번호 지정의 보류 리셋은 문단의 첫 콘텐츠가 쪽에 놓이는
  지점에서 확정한다**(`applyPendingPageNumberReset` — 글줄 `appendBlock`과 표 조각
  `appendTableSegmentBlock`이 같은 지점) — 표가 먼저 놓이는 문단은 표의 첫 조각이 실린
  쪽부터 새 번호가 이어진다(PR 리뷰 3차: 새 쪽 번호 9 + 두 쪽 표 → 9·10·11, 글줄에서만
  확정하면 1·2·9). **표보다 앞선 쪽 장식(머리말·꼬리말·쪽 번호 위치·감추기)은 표의
  첫 조각이 놓이는 순간 등록한다**(`pendingChromeRegistration` → `appendTableSegmentBlock`의
  `registerPageChromePreceding` → `chromeRegisteredBeforeText`, 글줄 뒤 방출은 그 서수를
  건너뛴다) — 종전 순서(서수 순 등록 뒤 표 흐름)를 지켜 표가 실제로 시작하는 쪽부터 장식이
  실린다(PR 리뷰: 쪽 번호 컨트롤 뒤 10행 표에서 첫 쪽 번호가 빠지고 둘째 쪽이 `- 2 -`).
  놓기 **전**에 등록하면 첫 행이 안 들어가 쪽을 넘길 때 문단이 없는 앞 쪽이 그 장식으로
  확정된다(PR #212 리뷰: 쪽 끝 문단의 번호 컨트롤 + 표가 다음 쪽으로 가는데 앞 쪽에 `- 1 -`).
  표 뒤의 장식은 종전대로 글줄 뒤 방출이 등록한다. **개요의 시작 쪽도 같은 지점**이다 —
  `appendTableSegmentBlock`이 `currentParagraphFirstPlacedPage`를 잡고 재시도에서 지우지
  않으므로 제목 문단의 표가 1쪽부터면 목록도 1쪽이다(글줄의 3쪽이 아니라; 루트 AGENTS
  "쪽 기준이 둘이다"). **조각을 하나도 안 낸 표(행 없는 표)는 놓인 것이 아니다** —
  `appendTableSegments`가 nil을 돌려주면 서수를 기록하지 않아 문단 위 간격을 걷지 않고 띠도
  닫지 않는다(종전 경로가 그대로 처리한다). **여백이 빈 단마저 소진하면 여백을
  버린다**(`fittingTableMargins`: 첫 행 + 위·아래 여백 > 용량) — 제목 행 반복을 끄는
  #13과 같은 폴백으로, 그대로 두면 가용 높이가 음수가 돼 splitter가 0 높이 조각을 상한까지
  낸다(PR 리뷰: 본문 200.8pt·행 30pt·여백 110 + 110pt → 4,098쪽). 용량은 진입 때
  **빈 단 높이**(`currentColumnFrame.height`, 이 쪽의 각주 예약을 빼지 않은 값)이고 —
  `effectiveContentHeight`로 재면 앞 문단의 각주 예약 때문에 다음 쪽에 정상적으로 들어갈
  여백까지 잃는다(PR 리뷰 3차: 각주 80pt 뒤 행 100pt + 여백 30·30) — 조각 루프가 단·쪽을
  넘긴 뒤에는 **넘긴 쪽의 예약을 뺀 용량**으로 여백과 남은 높이를 다시 잰다(종전엔 앞 쪽의
  `freshPage`를 그대로 써 앞 문단 각주만큼 덜 채웠고, 여백과 만나면 들어갈 행을 잘랐다).
  이월한 쪽의 예약이 여백까지 못 담으면 남은 조각은 여백 없이 흘린다. **나누지 않는 표
  (`appendWholeTable`)는 원자 단위가 표 전체**라 첫 행 기준 폴백을 지나도 표 + 여백이 목적지에
  안 들어갈 수 있다 — 넘긴 뒤 목적지 용량으로 다시 재어 여백만 버린다(PR #212 리뷰: 190pt 표
  + 여백 10·10이 위 여백만큼 아래 여백·각주 자리로 넘쳤다). 그 높이는 띠 판정과 같이 **방출할
  블록**(`HwpTableSplitter.segmentFrame`)으로 잰다 — `rowFrame.maxY` 최댓값은 셀 간격 표에서
  첫 행 앞 간격을 더 담아(32.83 vs 30) 딱 들어가는 여백을 버리거나 이르게 넘긴다(PR #212
  리뷰 4차). 반면 본문 각주 예측(`anticipatedFootnoteHeight`)은 `.table`을 건너뛰므로 글줄 앞
  표의 셀 각주가 `effectiveContentHeight`와 예측에 **이중 계상되지 않는다**(같은 라운드의
  지적은 이 술어를 놓친 것 — 프로브로 반증). 그 판정은 **이 표 자신의 셀 각주
  예약**(`anticipatedNotesForNextSegment`, 분할 경로의 `remainingAfterCellNotes`와 같은 시산)도
  미리 넣는다 — 수집이 놓기 직전이라 그 뒤에 재면 예약 전엔 들어가고 예약 뒤엔 안 들어가는
  표가 각주 자리로 넘친다(PR #212 리뷰 2차: 2행 표 + 셀 각주 48pt + 여백 30·30이 17pt 겹쳤다).
  **제목 행 반복(표 76 bit 2)의 판정도 쪽마다다** — 진입 때는 빈 단 용량
  (`currentColumnFrame.height`)으로 폭주(#13)만 막고, 단·쪽을 넘긴 조각은
  `refitContinuation`(`ContinuationFit` — 표의 여백 + 이 조각의 제목 행 반복)이 넘긴 쪽의
  예약을 뺀 용량으로 여백 → 제목 행 순으로 다시 재어 안 들어가는 것만 그 조각에서 뺀다. 진입
  쪽의 `effectiveContentHeight`로 표 전체의 반복을 끄면 앞 문단의 큰 각주가 남은 쪽에서
  시작한 표는 첫 조각이 바로 넘어가는데도 뒤 쪽의 제목이 전부 사라진다(PR #212 리뷰 2차).
  **넘김 경로는 둘이고 둘 다 같은 재평가를 거친다** — 조각 루프의 넘김과 셀 각주 예약의 넘김
  (`remainingAfterCellNotes`, 그 조각의 예약을 `reserved`로 뺀 용량). 후자만 낡은 여백을
  들면 여백만 버리면 통째로 들어갈 행을 잘라 조각과 쪽이 는다(PR #212 리뷰 3차: 행 100 +
  예약 78 + 여백 30 → 92.6 + 7.4). 진단(`collectUnsupported`)의 쪽은 종전대로 문단이
  **들어선** 쪽(`paragraphFirstPage`, 표를 놓기 전에 포착)이고 표의 첫 조각 쪽을 따르는 것은
  개요뿐이다(루트 AGENTS "쪽 기준이 둘이다"). `appendTablesPrecedingText`의 표 순회는 취소를
  보고(`where !Task.isCancelled`) 앞선 장식 훑기는 `chromeScannedBefore`부터 이어 O(N)이다.
  **바깥 여백은 조각마다 든다** (`TableFlowMargins`): 조각은 위 여백 뒤에서 시작하고 아래
  여백을 남기며 — 이어지는 단·쪽의 조각도 단 상단 + 위 여백에서 선다 — 아래 여백은 적합
  판정에도 든다. 글줄 뒤 흐름 배치(종전 경로·어울림 등)는 여백을 실측하지 않아 `none`이다.
  **글줄 앞에 표를 놓은 문단은 저작 문단 위 간격을 쓰지 않는다** — 표 앞에도, 표와 글줄
  사이에도 없다(`placeFlowParagraph`가 `paragraphHeight`와 측정 프레임에서 함께 걷어
  `heightIsMeasured` 판정이 그대로다). 블록 배열 순서는 표 → 글줄이다(2단 실물의 컨트롤
  문자가 글줄 앞에 있다).
  **실측**(한글 12.30, `line-shapes` 2단 HWP/HWPX + 합성 변이 7종의 PDF, 2026-09-16):
  1단 마지막 줄 끝 659.20 → 표 662.03(위 여백 2.83)부터 7행 89.74 → 2단 상단 99.20 +
  2.83 = 102.03부터 10행 128.20 → 글줄 233.06(= 캐시 `vertpos` 13386 = 283 + 12820 +
  283). 아래 여백 600(6pt)이면 7행째(남은 5.05pt)가 다음 단으로 가고, 위 여백 1000이면
  2단 조각이 109.20에서 서며(여백 값 자체가 되풀이된다), 문단 위 간격 20pt는 표에도
  글줄에도 나타나지 않았다(같은 간격을 준 보통 문단은 20pt 내려간다). 고치기 전에는
  글줄을 1단 659.20에 먼저 놓고 표를 6행/11행으로 갈랐다(fidelity 0.0067).
  **범위 밖**: 어울림(`square`)·오프셋 있는 표·비표 개체(그림·도형)는 종전대로 글줄 뒤
  흐름이고, 표 x의 바깥 왼쪽 여백 2.83pt는 #161부터 남은 별건이다.
  **실측**(한글.app 12.30이 저장한 PDF, `numbering-sequence` 3쪽, 쪽 상단 기준 pt):
  표 잉크 133.88~147.12(테두리 stroke 포함) — 우리 블록 134.03~146.85, `5.` 줄
  151.12~158.50(우리 줄 상자 149.68~165.68), `6.` 166.88~174.38(165.68~181.68).
  고치기 전에는 표를 165.68에 방출해 `6.`과 12.82pt 통째로 겹쳤다.
  **남은 격차**: 같은 PDF에서 표 잉크 x는 87.75~507.75인데 우리는 85.04~504.58이다
  — 차이 2.83pt = 바깥 **왼쪽** 여백(283 HWPUNIT)이고, 흐름 표 배치가 그 몫을
  반영하지 않는다. 세로와 달리 흐름 표 전체(헌법주석 블록 스냅샷 표본 포함)를
  움직이므로 별건이다
...
- `HwpPaintCommand` = `@unchecked Sendable` enum. CF/NS 참조 타입을 담기 때문에 자동 Sendable 불가. enum case는 가로챌 init이 없어 **소유권 경계에서 동결**한다 — `HwpLaidOutParagraph.init`이 `NSAttributedString`을, `HwpShapeGeometry.init`·`HwpBorderSet.Edge.geometry`(셀 테두리 경로, #191)가 `CGPath`를 복사본으로 저장한다 (mutable 서브클래스를 그대로 retain하면 Hashable 불변성·actor 경계 안전성이 깨진다). 새 참조 타입 payload를 추가하면 그 생산 지점에서 같은 복사를 해야 한다. 소유권 경계를 거치지 않고 **painter가 `.drawText`를 직접 만드는 경로도 마찬가지** — `HwpMemoPanelPainter`는 헤더를 `NSMutableAttributedString`으로 조립한 뒤 `NSAttributedString(attributedString:)`로 동결해 싣는다. 이제 actor 안전성 말고 **소비자**도 생겼다: HwpKitNative의 줄 배치 캐시가 문자열 신원 (`===`)으로 조판 결과를 재사용하므로 "신원이 같으면 내용도 같다"가 전 생산 경로에서 성립해야 한다 (깨지면 조용한 오조판)
...
(`HwpLineAdvance.advanceParts`의 `line` — 문단 사이 간격 제외)까지, 곧 다음 줄 상자 상단까지다. 가로는
...
- **오라클은 한글 편집 화면의 클릭이다** (12.30.0 6446, 2026-09-26, 155%). 링크를 한 줄씩 번갈아
  ...
  결과: 줄 사이 경계 = 다음 줄 상자 상단(10pt 160% 상자 바닥 +6.0 — 실측 +5.8…+6.1, 40pt 문단 끝
  글자·책갈피·글자 줄 +24.0 — 실측 +23.9…+24.1, 40pt 그림 줄 +6.0 — 실측 +6.3, 여분은 글자 상자
  기준), 문단 아래·위 간격 띠는 **어느 링크도 아니다**, 100%면 경계 = 상자 바닥(밑줄은 다음 줄
  몫), 고정 8pt(상자 10)면 다음 줄 상단(상자 바닥 2pt도 다음 줄 몫), **표 셀·글상자·각주·쪽·
  문서의 마지막 줄은 상자 바닥에서 끝난다**(밑줄도 안 열린다 — 셀 높이 100pt로 아래가 비어
  있어도). 다음 줄 글자가 제 상자 위로 솟아도(상대 크기 200%·150%) 경계는 그대로다 — 한글은 줄
  띠로만 판정한다. 손 모양 커서는 줄 상자까지뿐인데 우리에겐 호버 커서가 없어 클릭 띠만 맞춘다.
  한글 PDF의 링크 주석은 글꼴 지표 상자라 오라클이 아니다(#201과 같은 이유).
- **줄 뒤에 무엇이 오는지가 띠 하단을 정한다** (`HwpDrawnTextLayout.ListEnd`, 문자열의 마지막
  줄에만 적용 — 앞 줄들은 늘 `.followed`): `.followed`(같은 목록의 다음 문단이 잇는다 — 셀·글상자의
  가운데 문단, 같은 각주의 앞 문단) = 전진량, `.end`(셀·글상자·각주 배열의 마지막) = 줄 상자,
  `.unknown`(본문 문단 블록과 공개 `hyperlinkRegions(attributedString:origin:lineWidth:)`) = 둘 중 큰
  값. **각주는 배열이 아니라 블록이 답한다** — 각주 영역은 문단마다 블록 하나라 배열 인덱스로 보면
  두 문단 각주의 첫 문단까지 목록 끝이 된다. 스택이 아는 "이 쪽에 놓인 그 각주의 마지막 항목"을
  `HwpFootnoteBlock.isNoteEnd`로 실어 배열 판정과 AND 한다 — 이어짐 스태커(`StackEntry.isNoteEnd`,
  쪽 끝에서 나뉜 앞 몫도 참)와 흐름 스태커(`HwpFootnoteLayout.stackBlocks` — 넘쳐 다음 쪽으로 간
  문단 앞의 같은 각주 블록을 끝으로 뒤집는다)가 같은 뜻이다. 히트
  ...
  아니다).
- **본문 블록의 마지막 줄은 한글과 다르게 줄 간격 몫까지 둔다** (`.unknown`): 문단 블록은 자기가
  쪽·단의 끝인지 모른다. 그 몫은 블록 프레임(= 전진량 합 + 아래 간격) 안의 빈 자리라 다른 블록을
  가리지 않는다. 줄 간격 몫이 음수(고정 < 상자, 비율 < 100%)면 상자까지다 — 전진량만 쓰면 쪽 끝
  줄의 베이스라인·밑줄이 띠 밖이 된다(80% 한 줄 문단의 베이스라인 탭이 `.text`로 떨어졌다). 다음
  블록이 이으면 그 블록이 먼저 히트되므로 겹친 몫은 한글처럼 다음 줄 것이다. 컨테이너 문단은
  반대다 — 몫을 두면 셀 높이(마지막 줄 간격 제외, #160) 밖 다음 행 셀까지 넘쳐 그 칸의 빈자리를
  누르면 윗 셀 링크가 열리므로 한글대로 뺀다.
- **뒤 블록의 프레임 위 글자 claim은 앞 블록 링크에 진다** (`HwpHitTester.textClaim`·`footnoteClaim`):
  문단마다 블록이고 뒤 블록이 먼저 히트되는데, 프레임 밖 글자 claim(R53의 텍스트 축 — 첫 줄 잉크가
  글꼴 ascent − 0.85 × 기본 크기만큼 프레임 위로 샌다)이 방출된 앞 줄 띠의 아래를 가져갔다. 결정론
  글꼴(Menlo) 10pt 0.78pt, 함초롬바탕 10pt 2.2pt, 40pt 줄 아래는 8.8pt라 **밑줄 위 탭까지** 안 열렸다
  — 이 이슈의 증상이 다른 길로 남아 있었다(#233 리뷰, 픽스처 `r08`). 양보는 **위쪽만·글자만**이다:
  아래로 옮겨지거나 옆으로 넘친 글자는 뒤 블록 쪽이라 종전대로 claim하고(그쪽은 페인트 순서가
  뒤집힌 겹침이다), 각주의 개체·셀 칠이 덮은 자리는 문단을 뺀 같은 층 판정
  (`footnoteLayersClaim`)으로 가려 그대로 claim한다. 앞 블록에 링크가 없으면 claim 그대로다. #200의
  `testTallRelativeSizeGlyphAboveTheFrameClaimsTheTap`은 반대(큰 글자가 앞 문단 링크를 이긴다)를
  단언하고 있었는데 한글 실측과 어긋나 뒤집었다. **아래 블록은 `hit`의 같은 순회가 이어서
  판정한다**: 양보하는 claim은 `BlockAnswer.yielding`으로 돌아와 순회가 들고 내려가고, 처음 나온
  확정 답(`.final`)이 링크면 그 링크가, 그 밖이면 가장 위의 글자 claim이 이긴다(`resolve`). 링크·
  프레임만 따로 보던 첫 형태는 중간 블록의 **프레임 밖** claim을 지나쳤다 — 각주 도형이 제 프레임
  밖으로 넘쳐 그 자리를 덮으면 링크 조회는 그 가림을 nil로 접고 프레임 검사는 밖이라 통과시켜, 이
  글자가 없을 때 `.footnote`이던 자리가 글자를 더하자 도형 밑에 가려진 링크로 바뀌었다(#233 리뷰
  P2). 표 칸막이 바깥 절반·아래로 옮겨진 글자도 같은 구멍이었다. 중간 블록의 프레임 위 글자 claim도
  양보하므로 가려지지 않은 링크 띠는 그대로 열린다. **재귀로 짜지 않는다** — 둘째 형태
  (`textClaim`이 아래 블록을 다시 훑음)는 같은 자리에 쌓인 글자 claim 수만큼 호출이 깊어져, 1pt 줄
  수천 개에 거대한 글자를 얹은 조작 문서가 탭 한 번에 잡을 수 없는 스택 오버플로를 냈다(8MB 스레드
  3,000겹, iOS 1MB 스레드 500겹 — `maxNestingDepth`가 막는 것과 같은 결함 부류). 순회가 claim을
  들고 내려가 깊이가 0이고 블록마다 한 번 판정된다.
- **CT 상자와 합집합으로 두지 않는다**: 함초롬바탕 10pt의 CT ascent(10.7)가 상자 상단(8.5)보다 높아
  합집합이면 앞 줄의 줄 간격 띠(한글은 앞 줄 몫)를 이 링크가 가져간다 — 뒤 블록이 먼저 히트되고
  링크는 양보하지 않으므로 앞 링크가 진다. 첫 줄 상자 상단 = 블록 상단이라 띠가 프레임 위로 나가지
  않는 것도 이 선택의 값이다. 줄 상자가 0인 줄(잴 run이 없음)만 CT 상자로 폴백한다 — 글꼴 속성이
  없는 문자열도 CT가 기본 글꼴(Helvetica 12)을 달아 그 크기의 상자다.
...
- **띠는 줄을 놓은 기하 그대로다** — `lines`가 `lineGeometries`에서 받은 줄 상자 상단·높이·줄 몫
  ...
  `hyperlinkRegions`가 23% 느렸다(디버그, 40스팬 ~14줄 1.69 → 2.08ms) — 지금은 종전과 같다(1.62ms).
- 가드: `HwpHyperlinkClickBandTests`(합성 11건 — 이슈의 세 표본·줄 경계·문단 간격·고정/100%·음수
  줄 간격 몫·목록 끝·셀·각주 방출·흐름 스태커의 각주 끝), `HwpHyperlinkClickBandClaimTests`(합성
  9건 — 뒤 블록의 프레임 위 글자 claim: 상대 크기·Menlo ascent·각주 문단, 개체 칠은 claim 유지;
  양보 탐색은 중간 블록의 각주 도형·표 칸막이·옮겨진 글자에서 멈추고, 중간 글자 claim은 다시
  양보하며, 칠하지 않은 자격 영역은 지나친다; 그 밖에 글자 claim 1,000겹을 256KB 스레드에서
  히트해 스택 깊이를 잠근다),
  `HwpHyperlinkClickBandTextboxTests`(합성 4건 — 최상위·셀·각주 글상자의 목록 끝, 배열 끝 길이 0
  문단; 목록 끝이 실리는 자리(방출 순회의 글상자 갈래·히트의 글상자 `containerHit`)를 하나씩 끄면
  각각 빨개진다),
  `FixtureHyperlinkClickBandTests`(`hyperlink-click-band` 쌍 — 표본 29개의 띠를 문서 자신의 줄 캐시
  값으로, 히트를 한글 실측으로, 1쪽 링크 줄 22개의 베이스라인 자리를 줄 캐시 `vertpos + baseline`으로;
  HWPX 쌍은 링크 필드가 아직 강등 컨트롤이라 링크가 0개임을 따로 핀). 무력화 실측: CT 상자로
  되돌리면 합성 11건 중 10건(흐름 스태커 판정만 남음)·claim 6건(멈춤 3건은 CT 상자에서도 성립)·
  픽스처 2건 전부, 목록 끝을 무시하면(`.end` = 전진량) 합성 4건·픽스처 2건, `.unknown`을 전진량만으로
  두면 합성 1건·픽스처 띠 1건, claim 양보를 끄면 claim 6건·픽스처 히트·#200 테스트, 양보 탐색을
  링크·프레임만 보는 첫 형태로 되돌리면 멈춤 3건, 재귀 형태로 되돌리면 천 겹 테스트가(신호 10으로
  죽는다) 빨개진다.
...
- **빈 문단은 빈 문단 앵커로 단위가 된다** (#145). 글자가 없는 문단의 조판
  ...
  #124 폴백이 앵커 run(글꼴·문단 스타일·줄 간격 규칙)을 꼬리로 넘겨 그 문단을 종결하는
  개행이 서식을 입는다 (`newlineAttributes`의 상속 목록 — `hwp.lineSpacing`이 빠지면
  `HwpSelectionRTF`가 CT 힌트로 떨어져 고정이 최소로 나간다, #180 PR 리뷰). 빈 문단은 캐럿 자리가 하나라 선택이 **닿기만 하면**
...
- **`HwpFontResolver.testDeterministic`은 폰트 조회 세 축을 모두 닫는다** —
  ...
  가드 4종이 CI와 기여자 머신에서 함께 돈다 (루트 AGENTS.md "렌더 가드 다섯 층").
...
- **표식을 다는 경로 = 조각 높이가 측정 줄 전진량만으로 난 경우.** 후보는 셋 — 쪽·단
  ...
  판정과 부착은 `HwpParagraphLayout.measuredLineFragment(_:heightIsMeasured:measuredLineCount:
  measuredWidth:columnWidth:)` **하나**가 맡고 표식이 있던 입력은 먼저 벗긴다(표식 단 블록을
  다시 나누는 경로 — 균형 재배치·표 다중 쪽 분할).
- **한 줄 조각(측정 줄 1개)에는 달지 않는다.** 문단 전체 블록의 한 줄은 한 줄 넘침 규칙으로
  접혀 잰 것일 수 있어 표식이 두 줄로 되돌리고, 문단에서 잘라낸 한 줄은 잰 폭에 들어가
  좁지 않은 단에서는 표식이 렌더를 바꾸지 못한다.
- **폭이 다른 단으로 옮겨진 측정 높이 조각은 그 단 폭으로 다시 잰다** (PR 리뷰 3차 —
  표식은 접힘만 막아, 넓은 단에서는 CT가 줄을 합쳐 잰 높이의 상자 아래가 비고 좁은 단에서는
  줄이 늘어 상자를 넘쳤다). 흐름 분할은 `HwpFragmentRemainder`(나머지 줄·전진량·잰 폭)를
  `remeasureRemainderIfNeeded`가 단이 바뀔 때마다 목적 단 폭으로 다시 재고, 균형 재배치는
  `rebalancedFragment`가 옮겨진 조각을 다시 잰다(`bandTextBlocks`가 문단 모양을 나른다).
  그래서 표식 판정에 오는 측정 높이 조각은 늘 잰 폭 그대로다. 한 줄로 잰 문단을 통째로
  옮기는 갈래(`moveWholeParagraphToNextColumn`)도 같은 헬퍼로 다시 잰다 — 다시 잰
  문자열(목적 단으로 다시 푼 개체 예약)을 그대로 그리고, 여러 줄이 됐으면 통째로 놓지 않고
  조각 루프가 그 단부터 나눈다(PR 리뷰: 목적 단보다 큰 상자는 여백·뒤 내용으로 넘친다).
  세부 규약(리뷰 워크플로가 잡은 것들):
  - **문단 지표는 문단 전체 문자열로** — `layout(metricsReference:)`. 조각 부분 문자열로
    `ParagraphMetrics`를 만들면 비율 줄 간격의 마지막 줄 높이가 조각의 최대 글자 크기로
    잘려(20pt 한 글자로 시작하는 10pt 본문의 나머지 마지막 줄이 32pt가 아니라 16pt) 상자가
    그려지는 줄보다 짧아진다. CT 줄 원점은 부착 스타일(문단 전체)을 따르므로 지표만 갈렸다.
    균형 재배치는 `BandTextBlock.metricsReference`(뒤 조각 블록에 실린 문단 전체 문자열)로
    같은 기준을 지킨다 — 뒤 조각 블록은 쪽의 첫 단위라 오늘의 재배치가 다른 단으로 옮기지
    않지만 지표의 출처를 구조로 잠근다 (PR 리뷰).
  - **조각 첫 줄의 ascent 초과분 보정은 없다** (#180) — 줄 프레임 원점이 줄 상자 상단이라
    조각 상단이 곧 첫 줄 상자 상단이고, 다시 잰 나머지도 전진량 합 그대로다 (종전 CT baseline
    원점 모델에서는 #164 `ascentExcess`·`precedingBaseline`이 그 몫을 앞뒤 조각 사이에서 옮겼다).
  - **문단 아래 간격은 문단 끝을 담은 조각의 몫** — 균형 재배치의 가운데 조각(3단 이상)은
    다시 잰 높이에서 `paragraphSpacing`을 뺀다. 문단 위 간격은 커서가 이미 소비했고 0 하한은
    `authoredBeforeGap`과 같다.
  - **자르지도 다시 재지도 않는 블록은 문자열·높이 그대로** — 쪽·단 경계로 나뉜 문단의 뒤
    조각 블록(줄 목록 없는 단위 하나)이 밴드 닫힘 재배치를 지나며 제자리에 남을 때 표식을 다시
    판정하면 줄 수를 몰라(단위 1개) 벗겨져 #166 증상이 되살아난다 (HEAD의 잔여 결함).
  - **비용 상한** — 다시 재기는 나머지 전체를 복사·조판하므로 단이 바뀔 때마다 하면 (단 수 ×
    나머지 길이)로 이차다. 문단당 `HwpFragmentRemainder.maximumRemeasures`(64)와 줄 예산
    (`maximumLineFrames − 놓은 줄`)을 두어 병적 문서(1pt 폭 단 + 수백만 자)에서도 선형이고
    문단당 줄 상한을 우회하지 못한다. 넘으면 잰 폭의 줄로 두고 표식 판정이 가린다.
- **캐시 높이 문단의 조각만 잰 폭과 다른 단에 온다** — 그 판정은 폭 허용 오차 없는 실제
  줄바꿈이다 (PR 리뷰 2차 — 종전 `sameWidth`의 0.5pt는 앵커 재조판 문턱이지 줄바꿈 판정이
  아니다: 단 폭은 HWPUNIT 단위로 저작돼 0.4pt 좁은 단이 실제로 있고, 잰 폭에 꼭 맞던 31자
  줄은 0.01pt만 좁아져도 다시 나뉜다). 놓이는 단 폭 ≥ 잰 폭이면 CT 줄이 늘 수 없어 그대로
  단다(줄이 합쳐져 아래가 빌 수는 있다 — 캐시 축). 좁으면 렌더러가 그 폭에서 조각을 접을
  때만(자연 폭 ≤ 1.06배) 표식을 단 조각을 실제로 조판해 그려지는 줄 수가 측정 줄 수 이하일
  때 단다 — 넘으면(라틴 "a " / "b×70 " 조각이 0.3pt 좁은 단에서 세 줄) 종전 접힘(가로 6%
  이내 넘침)이 상자를 세로로 넘치는 것보다 낫다. 표식이 붙은 조각의 줄바꿈이 잰 폭과
  달라지므로 **줄 안 개체 앵커 문맥과 예약 재해석도 같은 정확 비교로 재조판한다**
  (`fragmentAnchorLines`·`placedFragment`, "앵커 규칙" 절).
...
- **한글 12.30 실측** (2026-09-11, 합성 HWPX): 305pt 폭·함초롬바탕 10pt에 `가` 311자
  ...
  첫 쪽에도 똑같이 있던 #178 베이스라인 축이고, 그 축은 이제 닫혔다 (위 "라인
  세그먼트 캐시"). 수정 전은 32자를 한 줄로 접어 다음 문단 위에 줄 하나가 비었다.
- 가드: `HwpMeasuredLineFragmentTests` — 술어 단위(왼쪽·가운데·오른쪽 정렬)와 세
  경로 각각의 합성 문서(표식·높이 = 줄 수 × 전진량·렌더 줄 수·마지막 줄이 상자
  마지막 줄 자리, 한 줄 조각 무표식, 0.3pt 좁은 단의 라틴 조각 실제 줄바꿈 판정 — 0.5pt
  허용 오차를 되살리면 빨개진다). `HwpMeasuredLineFragmentCacheTests` — 캐시 세 줄·CT 네 줄
  문단에서 세 경로의 잔여 조각이 무표식·한 줄 접힘을 유지하고 앞 조각은 표식, 0.4pt 좁은
  단(PR 리뷰 재현)으로 옮겨진 한 줄·두 줄 조각이 다시 재어져 두 줄 상자(흐름 분할·균형
  재배치)이고 줄 안 도형이 그려진 마커(둘째 줄 첫머리)에 놓이는지.
  `HwpMeasuredLineFragmentRemeasureTests` — 좁은 단 → 넓은 단(흐름 분할·균형 재배치)에서
  나머지가 다시 재어져 상자 높이 = 그려지는 줄이고 뒤 문단이 바로 아래 오는지, 넓은 단 →
  좁은 단에서 늘어난 줄이 상자에 담기는지, 캐시 높이 문단은 다시 재지 않는지.
...
- 다단 세부: 캐시 run이 없는 문단의 균형 재배치는 라인 수 기준 근사다(단마다 줄 수를
  같게 나눈다). 비균등 단으로 이월된 텍스트 조각은 draw 시 그 단 폭으로 다시 줄바꿈되는데,
  **측정 높이 문단의 나머지는 흐름 분할(`HwpFragmentRemainder`·`remeasureRemainderIfNeeded`)
  과 균형 재배치(`rebalancedFragment`)가 목적 단 폭으로 다시 재어** 조각 경계·높이·줄 안
  개체 앵커가 그 줄바꿈의 것이다 (#166 PR 리뷰 — 종전엔 조각 높이가 진입 단 폭 기준
  근사라 넓은 단에서는 줄이 합쳐져 상자 아래가 비고 좁은 단에서는 줄이 늘어 글자가 블록
  프레임 아래로 넘쳤다). 문단 머리가 아닌 나머지는 표식을 단 채로 재어 문단 단위 한 줄
  넘침 규칙을 건너뛰고(조각을 접으면 한글의 줄바꿈과 갈린다), 문단 머리부터 통째로 옮긴
  나머지는 문단이라 그 규칙을 따른다. **저장본 줄 캐시 높이를 따르는 문단은 다시 재지
  않는다** — 그 나머지는 잰 폭의 줄과 캐시 잔여로 놓이고, 넓은 단에서는 줄이 합쳐져 상자
  아래가 빌 수 있다(한글이 잰 높이라 우리가 다시 잰 값으로 바꾸지 않는다). 어느 경우든
  줄 안 개체는 자기 글리프가 실제로 그려지는 줄·x에 놓인다 (#164). **개체를 흐름 폴백으로
  되돌리거나 앵커 y를 블록 하단으로 클램프하는 처방은 금지** — 개체가 자기 글자에서 떨어져
...
- **각주 블록 높이도 셀과 같은 규약이다** (#94): 갈리는 축은 배치 방식이 아니라
  ...
  **공통 속성이 없는 개체의 기본값은 배치와 같아야 한다** (R51 #3): `collect`의
  ...
  예약이 **충분한지**도 따로 본다 (R65): 예약은 저작 치수(`inlineObjectReservation` →
  ...
  `inlineObjectReservation`가 nil이면 (공통 속성 없음 · 비 treatAsChar · 저작 치수 0)
  예약이 0이다. 각주·미주 안 **표**는 #214부터 줄이 그려지는 높이를 예약하므로
  (`HwpTableLayout.inlineTableHeights`) 예약 상자로는 이 하한에 걸리지 않는다 — 하한만으로는
  블록이 표를 담아도 같은 문단의 글줄이 저작 높이 줄에 남아 표와 겹쳤다. 대신 문단 높이가
  **줄 캐시**에서 왔으면 그 캐시가 표보다 짧을 수 있으므로(저작 뒤 셀 내용이 커진 문서) 표는 캐시
  마지막 줄 **상자** 아래(줄 간격 제외 — 각주 끝 블록은 그 줄 간격을 빼고 쌓는다)를 넘친 만큼
  하한을 둔다(`noteContainerFloor`의 `textBottom` ← `objects(tableContainmentBottom:)`, #214 리뷰 —
  없으면 표가 블록 밖 다음 각주 위로 흘러나간다). 하한은 표만 담는다 — 그래서 **표를 실은 캐시
  줄이 그 표(+ 위·아래 바깥 여백)보다 낮으면** 캐시가 낡은 것으로 보고 각주 높이를 캐시가 아니라 CT로
  잰다(`HwpParagraphLayout.lineCacheIsStale`, #214 PR 리뷰 — 본문과 같은 술어, 위 "표의 높이는 저작
  높이가 아니라" 항목): 줄 예약이 그려지는 높이라 표 줄이 커지며 그 아래 글줄이 모두 내려가는데,
  캐시 높이를 믿으면 줄 수가 캐시와 같아도 둘째 줄 이후가 블록 밖
  다음 각주 위에 그려진다(실제 30pt 표 + 10pt 두 줄 캐시: 둘째 줄 베이스라인 24.5 → 44.5pt, 블록 30pt).
  표를 실은 줄은 표 컨트롤의 WCHAR 위치를 덮는 마지막 세그먼트로 찾는다(못 풀면 가장 높은 캐시 줄).
  CT로 잰 개체 각주는 종전 캐시 무효 경로와 같이 마지막 줄 간격을 빼지 않는다. 판정·높이는 예약과
  배치가 공유하는 `measureNote` 안에 있다. 캐시와 CT 줄 수가 다른 문단의 뒤 줄이 블록 밖에 그려지는
  것은 캐시 높이를 믿는 종전 축이다. 한글이 저장한 캐시는 표를 담으므로 코퍼스에서는 걸리지 않는다. 하한은 글상자(내용이 저작 높이를 넘는 개체)와 높이를 넘기지 않는 경로의
  안전망으로도 남는다.
  ...
  470·894쪽 (루트 AGENTS.md "렌더 가드 다섯 층").
...
- **페이지에 걸친 문단의 각주는 조각(run)마다 그 페이지에 수집한다** (#95).
  ...
  **캐시가 더 앞 조각이라 하면 캐시를 따른다** (#165, `earliestOrdinalRanges` —
  `cachedControlOrdinalRanges`가 run 첫 세그먼트의 `textStartingIndex`와 컨트롤 문자의
  WCHAR 위치로 한글의 귀속을 복원한다). 배치는 CT 라인을 세그먼트 수에 비례해 나누므로
  폰트 대체로 CT 줄바꿈이 한글보다 늦으면 마커가 다음 조각으로 밀리는데 (실측: 결정론
  폰트 헌법주석 21건), 그 각주를 그려진 조각에 귀속시키면 한글이 이미 다른 각주로 채운
  다음 쪽을 넘치게 하고 넘침이 각주 이어짐으로 뒤 쪽에 연쇄해 한글에 없는 쪽이 생긴다
  (실측: 8쪽 증가, 한 구역의 각주 140개가 구역 끝까지 밀려 각주 전용 쪽 5장). 그 대가로
  그 쪽엔 **참조 없는 각주**가 뜬다 (마커는 그려진 조각에 남는다) — 연쇄보다 낫다.
  반대 방향(캐시가 더 늦음)은 그려진 조각을 따른다. **번호는 수집이 확정한 값을 들고
  가 뒤 조각의 마커에 적용한다** (`renumberedNoteMarkers(carrying:)`): 재매김은 그 조각의
  서수 범위만 훑는데 앞 조각이 가져간 서수는 어느 조각의 범위에도 없어, 쪽마다 번호를
  새로 시작하는 문서 (표 134 모드 2) 에서 마커가 문단 조판 때 구워진 옛 번호로 남는다.
  마커는 조각 하나에만 있으므로 (걸친 서수는 `ordinalsSpanningSlices`가 뺀다) 누적해도
  같은 마커가 두 번 바뀌지 않고, 자동 쪽 번호는 쪽마다 값이 달라 쌓지 않는다. 캐시
  귀속 계산은 이전 커서부터 이어 훑어 선형이다 (`cachedControlOrdinalRanges` — 처음부터
  훑으면 O(run 수 × 컨트롤 수)라 조작 문서가 첫 쪽 생성 전에 멈춘다).
  마지막 조각이 나머지를 전부 가져가므로 어느 조각에도 안 그려진 컨트롤(마커 없는 컨트롤·CT가 잘라낸 라인)도
  유실되지 않고, 서수가 컨트롤 배열 밖이면 nil이라 **문단 단위 수집으로 폴백**한다
  (유실보다 몰림이 낫다).
...
- **조각의 참조 마커 번호는 배치 직전에 다시 굽는다** (#95 리뷰 반영).
  ...
  **다만 개체 앵커 축은 닫았다** (PR 리뷰): 재매김이 실제로 편집을 냈으면 배치가
  재매김된 조각을 그 단 폭으로 **다시 조판해** 앵커 줄을 뽑는다
  (`renumberedAnchorLines`) — 안 그러면 옛 번호 폭으로 잰 줄이 남아 마커 **뒤**의
  글자처럼 취급 개체가 옛 x·옛 줄에 놓인다 (모드 2 + 시작 번호 9에서 10) → 9),
  참조 번호 한 자 폭 ≈ 3.8pt). 이 축은 순환이 아니다 — 번호가 그 시점에 이미 정해져
  있어서다. 술어는 **인스턴스 동일성**이라(재기록은 바꿀 게 없으면 입력을 그대로
  돌려준다) 번호가 이어지는 문서에서는 발동하지 않아 렌더가 불변이다
- **절대 캐시 모드의 각주는 본문 아래 자리에 한글의 이어짐 규칙으로 싣는다** (#165,
  `HwpFootnoteContinuation.swift` — 한글 12.30이 내보낸 헌법주석 PDF 1,030쪽의 각주
  번호·이어짐·줄 수를 전수 대조해 확정, 1,027쪽 일치). 규칙: ① 스택은 바닥 정렬이고
  마지막 줄의 **상자** 아래가 본문 하단에 닿는다 (줄 간격은 세지 않는다). 쪽 몫의 높이는
  마지막 줄이 아니라 전진량 끝의 **최댓값**까지다 (PR 리뷰, `HwpFootnoteCacheLines.height`
  — 정본 `cachedLineExtent`와 같은 정의): 고정 피치보다 큰 앞 줄이 마지막 줄 아래까지
  내려오면 그 몫을 빼고 재는 스택 계획이 안 들어가는 각주를 들여 본문 위에 놓는다. 빼는
  줄 간격도 그 몫의 "전진량 끝 최댓값 − 상자 아래 최댓값"이다 (`trailingSpacing`; 줄이
  고르면 마지막 줄의 줄 간격). 코퍼스는 줄이 고르다 (블록 전수 동일). ② 자리 =
  본문 하단 − 본문 마지막 줄 상자 아래 (`HwpPaginator.footnoteBodyBottom`, 캐시 run
  블록은 전진량에서 줄 간격을 뺀다 — 단 **stale 캐시로 CT 줄 범위까지 커진 블록은 뺄 몫이
  0**이다: 커진 몫을 "블록 아래 − 캐시 잉크"로 재면 재조판으로 늘어난 글자 높이까지 빠져
  구분선이 커진 글자 위에 그어진다, PR 리뷰. 표·글상자 등 그 밖의 본문 블록은 프레임이
  아니라 **그려지는 하한**(`HwpHitTester.paintedObjectBounds` — 프레임 ∪ 자손 개체 칠
  영역, 텍스트 여유 제외)이다, PR 리뷰: 오버레이·쪽 기준 자식은 컨테이너를 키우지 않고
  그 아래로 그려지므로 프레임만 보면 각주 스택이 그 개체를 덮는데, 겹침 가드
  `FixtureFootnoteOverlapTests`는 그려지는 범위로 재므로 두 자가 갈리면 가드만 잡는다 —
  같은 함수를 쓴다. **최상위 도형·그림 블록의 자기 칠도 든다** (PR 리뷰): 도형은 경로를
  프레임 원점으로 옮겨 클립 없이 긋고(회전 경로·miter 팁, `HwpShapeGeometry.paintedRect(in:)`
  — 셀 도형과 같은 산식) 그림은 테두리를 프레임 경로 중앙에 그어 절반이 밖이다. 이 자를
  최상위 글상자의 자기 테두리도 든다 (PR 리뷰 — 셀 글상자와 같은 `effectiveBorderWidth` 절반). 이
  자를 대자 헌법주석의 유일한 다각형(162쪽 2.83pt 표시)이 점 개수를 2바이트로 잘못 읽어 쪽을
  가로지르던 선이 각주 34·35를 다음 쪽으로 밀어냈다 — 원인인 CoreHwp 파서(4바이트 개수)를
  고쳐 블록 전수 동일을 지켰다. 개체 기하를 자리에 쓰기 시작하면 기하의 결함이 배치 결함이
  된다. **미주 블록도 본문이다**, PR 리뷰: 미주는 흐름 콘텐츠라 쪽 위부터
  놓이고 쪽 각주는 하한을 잰 **뒤에** 붙으므로 그 시점의 `.footnote` 종류 블록은 전부
  미주다 — 그것을 빼면 문서 끝 미주 쪽에 이월된 각주가 빈 쪽처럼 쪽 전체에 바닥 정렬돼
  진행 보장으로 놓인 미주를 덮는다, 재현: 미주 하단 104.67 위 구분선 92.6pt. 캐시 run
  블록의 뺄 몫은 블록 **인덱스**로 기록되므로 (`trailingSpacesBelowLineBox` — #222부터 흐름
  블록도 기록하고 균형 재배치 조각에 물려준다) 블록 배열을
  다시 쓰는 다단 균형 재배치(`rebalanceColumnBand`)가 열쇠도 함께 옮긴다, PR 리뷰 — 오늘은
  캐시 run이 1단 밴드에만 놓이고 재배치가 꼬리 블록만 갈아 끼워 무동작이지만, 위치 열쇠가
  배열 재작성과 따로 놀면 전제가 바뀔 때 다른 블록의 줄 간격을 뺀다) − 구분선
  위·아래 여백. 선 두께는 안 센다 — 선은
  위 여백 끝에 **가운데** 맞춰 긋고 아래 여백은 선 가운데부터다
  (`HwpFootnoteLayout.separatorLine`, 흐름 모드도 같은 산식). ③ 각주 사이 여백은 앞
  각주 마지막 줄 상자에서 다음 첫 줄까지다 — 마지막 줄의 줄 간격을 **대체**한다. "같은
  각주"의 판정은 표시 번호가 아니라 **식별자**(`Input.noteId`, 리셋 없는 문서 순서
  일련번호, 공개 `Input.init`은 `noteId:` 인자로 지정하고 생략하면 표시 번호를 쓴다)다: 쪽마다 번호를 새로 시작하면 (표 134 모드 2) 이월된 각주와 그 쪽의 새
  각주가 같은 번호를 갖는데, 번호로 묶으면 둘이 한 각주가 돼 사이 여백이 사라지고 앞
  조각의 마지막 줄 간격이 높이에 남으며 하나가 안 들어가면 다른 하나까지 밀린다
  (사이 피치 11.83 = 줄 900 + 여백 283; `NoteMeasurement.stackingHeight(isNoteEnd:)`,
  예약 `measuredFootnoteHeight(isNoteEnd:)`도 같은 산식). CT 높이로 쌓는 각주 — 줄 캐시가
  **없는** 각주(흐름 조판 문서·캐시를 버린 HWPX)와 개체를 담아 낡은·쪽 걸침 캐시 대신 CT 높이를 지키는
  각주 — 도 같다: CT 프레임의 전진량 끝에서 **모든 줄 상자 아래의 최댓값**까지를 뺀다
  (`HwpFootnoteLayout.stackTrailingGap` — 위 캐시 규약과 같은 정의라 줄이 고르면 마지막 줄의 몫이고,
  앞 줄이 더 크면 그 줄 상자가 스택 안에 남는다; 마지막 줄만 보는 컨테이너 규칙 `trailingGap`
  (#193)을 쓰면 30pt 개체 줄 + 10pt 줄·고정 16pt 각주가 26에 잘렸다, PR 리뷰)(#222, 한글 12.30 실측
  `probes/222` FN: 10pt·160% 각주 한 줄의 상자 바닥이 본문 하단에 닿고 영역은 8.5 + 5.67 + 10 =
  24.17). 종전에는 0을 빼 영역이 6pt 커졌고 구분선이 그만큼 위였다. 고정 줄 간격이 상자보다 작아
  상자가 전진량 끝 아래로 나가면 그 몫은 음수라 스택이 상자 바닥까지 자란다(PR 리뷰 — 한글 12.30 실측
  `so222n` NF: 10pt·고정 8pt 두 줄 각주의 마지막 줄 상자 바닥이 본문 하단 756.82에 닿는다; 한글이
  저장한 그런 각주는 캐시 줄 간격이 음수라 `HwpFootnoteCacheLines.lines`가 받지 않아 이 CT 경로다). ④ 통째로 안 들어가는 각주는
  줄 캐시에 분할 지점 (세로 위치 리셋 — `0, 1172, 2344, 0, 1172` 또는 첫 줄만 앞 쪽인
  `0, 0, 1172`) 이 있고 **그 앞 몫 전체가 들어가면** 그 앞 몫만 싣고 나머지를 다음 쪽
  **첫** 각주로 잇는다 (`Input.placedLineCount`, 구분선은 다시 긋고 번호는 반복하지 않으며
  이어지는 줄은 내어쓰기 자리 — `continuationFragment`). 첫 줄만 보고 나누면 안 된다 (PR
  리뷰): 분할 지점까지의 줄이 전부 방출되므로 자리가 캐시가 저작된 자리보다 작을 때
  (stale 보정·그려지는 자손으로 본문 하한이 내려간 쪽) 스택이 자리를 넘고 바닥 정렬이
  그 스택을 본문 위로 올린다 — 앞 몫이 안 들어가면 통째로 다음 쪽이다 (`headEntries`가
  싣기 전에 같은 산식으로 잰다; 진행 보장은 앞 몫에도 같다). 코퍼스는 자리 = 한글의
  자리라 첫 줄 판정과 결과가 같다 (블록 프레임 전수 동일). 앞 조각은 다른 분할 경로(표 셀·다단
  run)와 같은 **이어짐 표식**(`markedAsContinuedFragment`)을 단다 — 컨테이너 문단은 위치
  열쇠가 없어 복사가 이 표식으로 조각을 잇고, 양쪽 정렬은 이 표식으로 조각 끝 줄을
  벌린다 (한글 PDF 750쪽 각주 224의 마지막 줄이 양쪽 정렬돼 있다). 분할 지점이 없으면 통째로 옮기고, 뒤
  각주도 모두 함께 넘어간다. **조각 경계는 언제나 문단 전체 기준 절대 캐시 줄 인덱스로
  잰다** (`NoteMeasurement.absoluteLineRange`·`sourceAttributed`/`sourceLines`) — 이미
  잘린 조각을 남은 줄 기준으로 다시 환산하면 다음 쪽이 원본 기준으로 계산한 경계와
  반올림에서 갈려 (캐시 4줄·CT 5줄을 세 쪽에 나누면) 가운데 줄이 사라지거나 겹친다.
  경계는 **문자 단위로 보존된다** (`Input.placedLength`): 이어지는 조각은 그 쪽의 폭으로
  문단을 다시 조판하므로, 구역이 바뀌어 폭이 달라지면 줄 나눔이 달라져 캐시 줄 비례
  환산이 앞 쪽의 마지막 글자와 다른 자리를 가리킨다 (실측: 451 → 200pt에서 7자 유실).
  앞 쪽이 **실제로 그린** 문자 길이를 이월 입력에 실어 그 위치부터 잇는다 — 폭이 그대로면
  비례 환산 결과가 곧 그 위치라 값이 같다. **각주 모양도 같이 실어 보낸다**
  (`Input.footnoteShape`): 번호 라벨은 구역 각주 모양의 장식 문자·번호 모양을 따르므로
  구역이 바뀐 쪽에서 다시 만들면 라벨 길이가 달라져 조판 문자열이 통째로 밀리고, 폭이
  같아도 `placedLength`가 다른 글자를 가리킨다 (실측: `(1)` → `1`에서 `word45`가 `rd45`로
  잘림). 예약도 그 모양으로 쟀으므로 같은 값을 써야 예약 ≡ 배치가 성립한다 —
  `sizeResolver`(R44 #1)와 같은 규약이고, `measure`가 처음 잰 값을 각인해 나른다.
  각인은 **"아직 안 잼"과 "기본 모양(nil)으로 잼"을 가른다** (`Input.measuredShape` —
  `MeasuredShape?`의 이중 optional): 둘을 같은 nil로 두면 첫 `place`에서 모양을 생략해
  기본 모양으로 잰 이월 입력이 다음 쪽의 모양을 새로 채택해 문자열이 밀린다 (실측: `1` →
  `(1)`에서 앞 조각의 마지막 두 글자 중복). 구분선 여백·길이는 **쪽**의 몫이라 그대로
  현재 구역 모양을 쓴다. 경계가 줄 중간에 떨어지면 그 줄의 앞부분을
  잘라 내되 줄 기하와 앵커는 앞 쪽 폭 기준이라 근사다 (분할 각주는 개체를 안 담으므로
  앵커는 버린다).
  **개체를 담은 각주는 나누지 않는다** (`NoteMeasurement.carriesObjects`): 개체는 앵커
  줄이 아니라 문단에 붙어 문단 **전체** 조판 기준 좌표로 수집되므로, 나누면 뒤 줄의
  그림이 앞 조각에 남아 블록 밖에 그려지고 뒤 조각은 빈다 — 한글이 그런 각주를 어떻게
  나누는지는 **실측이 없어** (코퍼스에 그런 각주 0건) 통째로 옮긴다. 같은 이유로 그런
  각주는 **CT 높이를 그대로 쓴다**: 쪽 몫의 합은 한글이 두 쪽에 나눠 그린 높이인데 우리는
  한 쪽에 통째로 그리므로, 그 합으로 낮추면 블록이 CT 좌표의 개체를 못 담아 다음 각주와
  겹친다 (실측: 블록 44.16pt에 개체 하단 79.35pt). **두 판정의 범위는 각주 단위로 같다**
  (#165 리뷰): 문단 단위로 보면 개체가 뒤 문단에만 있을 때 앞 문단만 캐시 합으로 줄어 그
  문단의 마지막 글줄 위로 뒤 문단의 그림이 올라온다. 판정은 `measure`가 `noteId`별로 한 번
  (`hasCollectibleObject` — 수집기 관문을 되풀이한 술어; 관문 `collectible`은 **요소가 하나도
  없는 컨트롤을 거른다**, PR 리뷰 — 빈 컨트롤은 `objects()`가 아무것도 내지 않는데 공허하게
  참이면 그 각주가 개체를 담은 것으로 판정돼 쪽 끝 분할이 막힌다; 관문을 지난 요소도 **그릴
  것이 있어야** 센다, PR 리뷰 — `emitsObject`가 `collect(component:)`의 관문(그림은 BinData
  참조와 크기, 글상자는 수집 대상, 도형은 **크기**)을 조판 없이 되풀이한다. 도형의 관문은
  세부 레코드가 아니다 (PR 리뷰) — `shape()`가 `resolvedSize`를 먼저 요구하므로 선·사각형
  레코드가 있어도 크기가 0이면 내지 않는데, 레코드만 보고 개체로 세면 그 각주의 분할이
  막힌다. 크기 폴백도 `resolvedSize`처럼 **축마다**다 — 공통 속성의 너비와 요소의 현재
  높이로 서는 도형은 그려지므로 두 축을 한 출처에서 같이 요구하면 술어가 그리는 쪽의
  상위집합조차 아니다. 조판이 있어야 아는 실패는 못 가리므로 그리는 쪽의 상위집합이고,
  넓은 쪽은 나누지 않는 안전한 방향이다) 하고, **예약도 같은 범위를 본다** — 개체를 담은
  각주의 문단은 그 문단에 개체가 없어도 캐시 빠른 길(`measuredFootnoteTextHeight`)로 가지
  않는다 (가면 예약이 캐시 합, 배치가 CT 높이로 갈린다). **술어는 수집 대상 전체다**
  (#165 리뷰): 높이 하한 술어 `hasFloatingObject`는 글 앞으로·글 뒤로 개체(오버레이 —
  흐름을 안 점유하고 줄에도 안 담긴다)를 빼는데 배치는 그 개체를 수집해 CT 높이를
  지키므로, 예약만 하한 술어로 판정하면 같은 각주를 다르게 본다 (실측: 예약 58.36 vs 배치
  78.2pt). `hasFloatingObject`는 표 셀의 하한 재수집 게이트로만 남는다. 여러 문단짜리 각주의 뒤 문단이 0에서 다시 시작하면 그
  문단부터 통째로 다음 쪽이다. 들어맞음 허용 오차는 부동소수 잡음뿐 — 한글은 1 HWPUNIT
  여유에도 싣는다. 진행 보장: 빈 쪽에도 안 들어가는 분할 지점 없는 각주는 그대로 싣는다
  (코퍼스 0건). 결과: 헌법주석 겹침 366쪽 → **0쪽**, 쪽수 1,030 그대로, 이어짐 쪽 64
  (한글 64). 이어지는 조각의 CT 줄 수가 캐시와 다르면 (결정론 폰트) 조각 텍스트는 비례로
  잘리고 블록 높이는 캐시라 조각 끝에 빈 줄이 남을 수 있다 — 본문 조각과 같은 축. 그
  비례 환산은 **양 경로가 같은 원본·같은 절대 인덱스**를 쓰므로 줄을 잃지는 않는다. **줄 단위
  대응이 조각을 비우거나 뒤 몫의 글까지 삼키는 경계만 글자 위치로 보간한다** (PR 리뷰,
  `fragment`): CT 줄이 캐시 줄보다 적으면 짧은 쪽 몫의 양 끝이 같은 CT 줄로 환산돼 빈 조각이
  되고 (앞 조각은 구분선과 빈 자리만 그리고 글은 뒤로 밀린다), 앞 조각의 끝이 마지막 CT 줄로
  올림되면 한글이 다음 쪽에 이어 놓은 줄이 사라진다 — 헌법주석 인쇄 442·799쪽의 이어지는
  조각이 빈 블록이었다. 그때만 캐시 경계 i를 CT 줄 `floor(i·m/N)` 안의 글자 위치(줄 길이 ×
  소수 몫)로 잡는다; 줄 단위 대응이 서는 경계는 그대로라 나머지 조각의 글과 좌표는 불변이다
  (전 경계를 보간하면 같은 줄 수인 문단의 조각까지 줄 중간에서 갈린다). 보간한 UTF-16 위치는
  **합성 문자 경계**로 맞춘다 (`composedBoundary`, PR 리뷰) — 대리 쌍·결합 문자열 가운데서
  가르면 두 쪽에 반쪽씩 그려져 원문이 깨진다 (재현: 이모지 넷을 3몫으로 나누면 `😀����😀`);
  최소 진행도 `start + 1`이 아니라 `start`의 합성 문자 하나다. 그래도 앞 조각이 글을 다
  그렸으면 남은 캐시 줄은 넘기지 않고 (`appendHead` — 빈 블록을 싣지 않는다) 그 조각엔 이어짐
  표식도 달지 않는다 (PR 리뷰 — 캐시 줄이 남았다고 달면 양쪽 정렬이 진짜 마지막 줄을 벌리고
  복사가 다음 문단에 개행 없이 잇는다).
  **이월의 비용은 쪽 수에 비례해야 한다** (PR 리뷰, `HwpFootnoteLayout.SourceLayout`):
  이어지는 조각을 쪽마다 문단 전체를 CT로 다시 조판하고(`measureNote`) 남은 줄을 배열로
  떠서 분할 지점을 훑고(`splitPoint`) 남은 문자열을 미리 잘라 두면(`attributedSubstring`)
  쪽 수 × 줄 수의 일이 돼 리셋 수천 개짜리 문단 하나가 쪽 상한(10,000)까지 시간을 태운다
  (실측, 디버그: 2,000줄·99쪽 29.9s). 이월 입력은 처음 잰 원본 조판·줄 캐시·남은 높이
  누적표(`remainingHeights`)를 나르고 (폭이 같으면 다시 조판하지 않는다 — 구역이 바뀐 쪽만
  다시), 분할 지점은 인덱스로만 훑고(`firstPageBreak(in:after:)`), 조각 문자열은 **실릴 때**
  잘라 낸다(`NoteMeasurement.text`, 지연 — 스택 계획은 높이만 읽는다), 경계 문자를 담은
  줄은 이분 탐색한다. 예약(`measuredFootnoteTextHeight`)도 나른 캐시·누적표를 쓴다. 결과
  0.7s·32,000줄 1,599쪽 10.6s (선형). **후보 각주도 필요할 때만 잰다** (PR 리뷰,
  `MeasuredNotes`): 쪽마다 대기 각주 전부를 재면 독립 각주 N개가 쪽마다 몇 개씩만 실리는
  이월에서 쪽 수 × N의 CT 조판이다 (실측: 55줄 각주 100개·101쪽 41.5s, 200개 165.4s).
  스택 계획·흐름 스택 모두 순서대로 보다 처음 안 들어가는 각주에서 멈추므로 거기까지만
  재고, 재지 않은 각주도 모양을 **각인한** 입력으로 이월된다 (전부 재던 때와 같은 모양).
  절반 상한 배치의 스택 높이도 절반을 넘으면 멈춘다. **대기 각주는 슬라이스로 든다** (PR
  리뷰): 이월을 쪽마다 새 배열로 뜨면 그 복사·retain이 다시 쪽 수 × N이라 페이지네이터의
  `pendingFootnotes`/`pendingEndnotes`는 `ArraySlice`이고 배치는 슬라이스 판
  (`placePending`/`placePendingFlow` → `PendingPlacement`)을 쓰며 공개 경계(`Placement`)에서만
  배열로 만든다. 각주 모양은 **수집 시점에 각인**한다 (`appendPendingFootnote`) — 배치가
  쪽마다 전부에 각인하던 일도 없다. 개체 판정은 그 각주의 **이웃만** 훑고(같은 각주의 문단은
  잇닿아 있다), 이월 예약(`reservedFootnoteHeight(upTo:)`)은 쪽 콘텐츠 높이를 넘으면 멈춘다
  — 읽는 쪽(`effectiveContentHeight`·`usableBottom`)이 거기서 포화해 값이 같다. 프로파일에서
  이 예약이 쪽당 53%였다. **이월 예약은 다음 쪽에 실릴 조각까지다** (PR 리뷰): 분할 지점이
  여러 쪽에 걸친 각주의 남은 전부를 더하면 예약이 쪽 높이를 넘어 `effectiveContentHeight`가
  1pt로 무너지고, 캐시 없는 흐름 문단·표가 자리가 남아도 다음 쪽으로 밀린다 (재현: 다섯 조각
  각주 뒤의 흐름 문단이 셋째 쪽으로) — 배치는 그 쪽에 첫 분할 지점까지의 앞 조각만 싣고 그 뒤
  각주는 넘기므로 예약도 그 조각(`lineLimit`)에서 멈춘다; 한글도 그 쪽에 그 조각만 놓았다
  (분할 지점이 곧 한글의 쪽 경계다). 개체를 담은 각주는 나뉘지 않으므로 그대로다. 분할
  지점은 `splitPoint`와 **같은 둘**이다 (PR 리뷰): 문단 안의 세로 위치 리셋, 그리고 앞 문단의
  남은 마지막 줄보다 위에서 시작하는 뒤 문단 — 그 문단부터 통째로 다음 쪽이라 예약은 앞
  문단까지다 (문단 안 리셋만 보면 문단 경계에서 갈리는 각주가 남은 문단 전부를 예약한다).
  그 판정은 `fragmentShares` **하나**가 하고 (PR 리뷰) 이월 예약과 **수집 시점 예약**
  (`appendPendingFootnote`)이 같이 쓴다 — 참조 쪽에서 새로 수집한 각주도 전부가 아니라 첫
  조각까지만 예약한다 (전부를 더하면 그 쪽의 캐시 없는 흐름 문단이 다음 쪽으로 밀린다; 배치는
  첫 조각만 싣는다). 쪽 끝 문단의 줄 간격 제외도 여기서 정한다: 문단 경계에서 갈리면 **앞
  문단이 쪽 끝**이라 뒤 문단을 먼저 보고 앞 문단을 `endsPage`로 잰다 (사후 보정이 아니라
  선행 판정 — 앞 문단을 줄 간격 포함으로 더한 뒤 멈추면 배치보다 한 줄 간격만큼 크다). 예약이
  분할 지점에서 멈춘 쪽은 `reservationStopsAtSplit`으로 남겨, 그 뒤 같은 쪽에서 수집되는
  각주는 예약을 더하지 않는다 (배치는 나눈 조각 뒤의 각주를 모두 넘긴다). **조각 예약은 절대
  캐시 모드에서만이다** (`Environment.continuesAtCacheBreaks`, PR 리뷰): 흐름 모드의 절반
  상한 배치(`stackBlocks`)는 각주를 통째로 놓으므로 첫 조각만 예약하면 본문이 그 자리를 먹은
  뒤 진행 보장으로 놓인 통째 블록이 본문 위나 쪽 밖으로 나간다 — 페이지네이터가
  `absoluteCacheMode`를 환경에 실어 이월·수집·예측 예약이 같은 판정을 본다. **수집 전
  예측**(`anticipatedFootnoteHeight` — 흐름 폴백 문단·표 셀이 커밋 전에 보는 값)도 같은
  조각·같은 정지 판정이다 (PR 리뷰): 전부를 더하면 첫 조각 옆에 들어가는 문단이 다른 쪽으로
  밀린다; 이미 분할 지점에서 멈춘 쪽의 예측은 0이고 실리는 각주 수(`landed`)로만 사이 여백을
  센다. 멈춘 예측은 **순회도 멈춘다** (PR 리뷰) — 뒤 각주의 개체 술어와 중첩 컨테이너 순회에
  들어가지 않는다; 이어지는 각주 뒤에 흐름 문단·표를 다시 시도하는 쪽마다 그 일이 되풀이돼
  쪽 수 × 문단 수가 된다. **구역이 바뀌면 이월 예약을 새 구역으로 다시 잰다**
  (`applySectionDef`, PR 리뷰): 앞 쪽을 확정하며 잰 예약은 이전 구역의 폭·구분선·쪽 높이
  기준이라 새 구역 첫 쪽의 배치(새 폭·새 구분선)와 갈린다 — 재현: 구분선 위 여백 50pt인 2구역
  첫 쪽에서 옛 예약대로 들어간 흐름 문단이 각주 자리를 먹어 이월 각주가 다음 쪽으로 밀렸다. 결과 200개 3.66s·800개 13.0s·1,600개 25.9s (선형). **각주 하나의
  문단도 필요한 만큼만 잰다** (PR 리뷰): 문단 N개짜리 각주가 문단마다 쪽을 넘기면 통째 판정을
  위해 남은 문단을 전부 재는 `groupHeight`가 쪽 수 × N이다 (실측: 55줄 문단 100개 33.7s·200개
  134s) — 빈 쪽 자리를 넘는 순간 멈추고 (넘은 뒤엔 "안 들어간다"·"빈 쪽에도 안 들어간다"만
  필요하다; 실제로 싣는 진행 보장 분기만 끝까지 잰다), 분할 지점 탐색은 측정을 거치지 않는다
  (`MeasuredNotes.cacheLines(at:)`·`carriesObjects(at:)` — 측정과 같은 출처·같은 답: 수집된
  개체는 모두 그 술어의 관문을 지난 것이다). 결과 100개 1.49s·800개 13.1s (선형). **나뉜
  문단의 이월 입력은 남은 목록의 첫 항목을 대신한다** (`PendingNotes.head`, PR 리뷰) — 꼬리를
  복사해 앞에 붙이면 앞 문단이 여러 쪽에 걸쳐 이어지는 동안 뒤에 남은 각주 수만큼의 복사가
  쪽마다 되풀이된다; 나뉘는 문단은 언제나 남은 목록의 첫 항목이라 머리 하나로 족하다. 절반
  상한 스택 높이의 `where` 필터는 몸통만 건너뛰고 끝까지 훑으므로 `break`다 (PR 리뷰). **각주의
  끝과 개체 유무는 수집 시점에 확정한다** (`Input.noteFacts`, PR 리뷰): 남은 CT 조판이 선형이
  된 뒤에도 묶음의 끝(`groupEnd`)과 개체 술어를 쪽마다 남은 문단을 훑어 다시 알아내면 식별자·
  술어 순회가 쪽 수 × N이다 — 공개 API 입력(사실 없음)만 훑는다. **이월 목록은 처음 본 모양을
  각인한다** (`PendingNotes.stamp`, PR 리뷰): 나뉜 조각만 각인된 입력을 나르고 통째로 넘어간
  각주는 원본 입력을 돌려주면, 공개 `place`로 쪽을 넘기는 호출자가 다음 쪽에 다른 모양을 주었을
  때 잰 뒤 넘어간 각주의 라벨이 그 모양으로 바뀐다 — 항목마다 각인하는 대신 목록 하나가 들고
  읽을 때 입힌다 (페이지네이터 입력은 수집 시점에 각인돼 있어 무동작).
  가드: `HwpFootnoteContinuationSourceLayoutTests` (누적표·분할 탐색 동치, 같은 폭에서만
  원본 재사용, 높이가 문자열 없이 나옴, 큰 앞 줄의 몫, 수집 시점 각인, 실을 문단만 측정,
  머리 대신하기).
  2026-08-03의 "강제 이월 → 1,035쪽" 실험이 실패한 이유는 이어짐이 아니라 **귀속**이다
  (아래 조각 귀속 항목). 가드: `HwpFootnoteContinuationTests` (합성 10종) +
  `FixtureFootnoteContinuationTests` (헌법주석 인쇄 749~752쪽 실측 핀) +
  `FixtureFootnoteOverlapTests` (세 축 전부 0).
- **각주 영역 상단은 본문 상단 아래로 못 내려온다** (#95) — 구분선의 **획**까지다 (PR
  리뷰, `separatorOverhang`): 선은 위 여백 끝에 가운데 맞춰 그어지므로 위 여백이 획 반
  두께보다 좁으면 그만큼 영역 위로 나가는데, 클램프된 쪽에선 그 몫이 머리말·위 여백으로
  샌다 (재현: 굵기 index 15·위 여백 0에서 선 위 64.9 < 본문 상단 72) — 클램프 하한을 그
  몫만큼 내린다 — 그리고 **같은 몫을 자리에서도 뺀다** (PR 리뷰, `areaFloor`): 자리는 그대로
  두고 클램프만 내리면 자리에 꼭 맞게 들어간 스택이 클램프에 밀려 본문 하단 밖으로 나간다
  (재현: 688.87pt 스택이 6.4pt 넘침) — 나눠 넘기거나 옮겨야 할 각주다. **본문이 있는 쪽도
  같다** (PR 리뷰): 자리를 `본문 하한 + 획 몫` 아래에서 재야 본문 하한에 딱 붙은 스택의 선이
  본문 마지막 줄을 긋지 않는다 (재현: 굵기 index 15·위 여백 0에서 선 위 299.94 < 본문 303).
  같은 몫을 **예약도 센다** (`ReservationMetrics.separatorOverhead`, PR 리뷰) — 배치만 빼면
  본문이 그 자리를 먹어 들어갈 조각이 밀린다 (예약 ≡ 배치). 흐름 배치(`placeFlow`,
  `drawSeparator: true`)도 같은 몫을 시작점 아래로 내린다 (PR 리뷰) — 안 그러면 선이 앞
  내용을 긋는다. 상한 없는 배치
  (`limitsAreaToHalfContent: false`)에서 `contentFrame.maxY − 스택 높이`가
  `contentFrame.minY`보다 작아지면 각주 앞부분이 종이 밖으로 잘려 **사라졌다**
  (수정 전 실측: 헌법주석 5쪽, 최악 렌더 인덱스 79의 −217.6pt). 이어짐 (#165) 뒤에는
  진행 보장으로 실린 거대 각주 (빈 쪽에도 안 들어가는 분할 지점 없는 각주, 코퍼스 0건)
  에서만 발동하고, 그때 초과분은 프레임 하단으로 나간다. 절반 상한 모드는
  `areaHeight ≤ 콘텐츠/2`라 무동작이다. 가드: `HwpFootnoteLayoutTests`의 클램프 2종 +
  `FixtureFootnoteOverlapTests` (상단·종이 밖은 불변식 0쪽, 하단 예산 0).
- 표 셀 각주는 소유 문단 페이지가 아니라 그 셀의 행이 실리는 페이지에
  ...
  페이지 수가 한글.app 실측 1,030과 일치 (2026-07-10). 이어짐 (#165) 뒤에도 1,030이다
  — 2026-08-03의 "각주 영역을 본문 하단에 맞추면 1,035쪽" 실험은 귀속 연쇄 때문이었다
  (위 각주 이어짐 항목).
- **셀 높이는 라인 캐시·저작 높이 위에 "떠 있는 개체" 하한을 얹는다** (#91).
  셀 문단 전부가 캐시로 측정됐을 때(`cachedCellHeight`가 있을 때) 저작된 셀 높이
  (표 80)를 신뢰하는 규약은 **글자처럼
...
- 페이지/단 경계로 분할된 문단의 컨트롤은 마지막 조각 처리 후 방출된다 —
  ...
  **예외 둘**: 직접 각주는 조각 단위 귀속을 받고 (#95, 위 각주 항목 — 절대 캐시 run
  경로와 흐름 분할(#207, `HwpFlowFragmentFootnotes`: 조각 문자열의 마커
  `hwp.controlIndex` 속성으로 줄별 서수 끝을 잡는다 — 원본 WCHAR 위치로 되돌리는 환산이
  필요 없다) 둘 다; 컨테이너 안 각주는 그 컨트롤과 함께 마지막 조각으로 간다; 다단 캐시
  run(`placeCachedColumnRuns`)은 아직 문단 단위),
...
- **조각의 줄 앵커** (#164): 조각 블록은 `appendBlock(anchorLines:)`로 **조각 기준**
  ...
  **진단 쪽(`walkUnsupported`)도 같은 갈래를 탄다** (PR 리뷰): 조각과 함께 놓인 개체는
  기록해 둔 조각의 쪽(`inlineControlsPlacedPerFragment`)으로 보고하지만, 그 쪽을
  물려받는 것은 **컨테이너가 콘텐츠로 그리는 자손**뿐이다(`rendersInsideParent` —
  `appendControlBlocks`의 흐름 억제 분기 둘과 같은 술어). 컨테이너가 소비하지 않아
  흐름으로 나가는 중첩 컨트롤(OLE·수식·`oleArray` 품은 도형)은 마지막 조각 뒤에
  그려지므로 그 쪽(`flowPage`)으로 보고한다 — 부모의 쪽을 물려주면 진단이 블록 없는
  쪽을 가리킨다. 방출과 진단이 같은 `containerContext(of:)`를 봐야 두 축이 갈리지 않는다.
  **안에 각주·미주를 품은 개체(`containsNotes`)도 조각에서 놓지 않는다** — 그
  ...
  줄은 문단이 처음 놓인 단의 폭으로 측정되므로, **폭이 다른 단으로 이월된 조각**
  (비등폭 단 — 다단 캐시 run·흐름 분할 둘 다)은 렌더가 그 단 폭으로 다시 줄바꿈하는
  것과 같게 조각 문자열을 목적 단 폭으로 다시 조판한 줄을 앵커 문맥으로 쓴다
  (`fragmentAnchorLines`) — 첫 단 폭의 줄로 앵커를 잡으면 마커가 다른 줄·다른 x에
  놓여 개체가 글자를 덮는다 (PR 리뷰: 134.16→268.37pt 단에서 x 327.53 대신 241.87).
  **"폭이 다르다"는 허용 오차 없는 정확 비교다** (#166 PR 리뷰): 처음엔 0.5pt 안이면
  같은 폭으로 봤는데, 측정 줄 조각 표식이 붙은 조각은 렌더러가 목적 단 폭의 줄바꿈으로
  그리므로 0.4pt 좁은 단에서 30자 + 도형 / `끝`으로 나뉜 조각의 도형이 잰 폭의 줄(첫 줄
  끝)에 남았다. 같은 단·등폭 단의 폭은 같은 기하에서 온 같은 값이라 정확 비교로도
  재조판이 없고, 비등폭 단은 HWPUNIT 단위로 저작돼 0.5pt 안의 차이도 실제 차이다 —
  예약 재해석(`placedFragment`)도 같은 비교다.
  **줄뿐 아니라 개체의 예약 폭도 첫 단에 묶여 있다** (PR 리뷰): 폭 기준이 '단'·'문단'인
  개체(표 70)는 예약이 단 폭의 함수인데 문자열은 문단당 한 번 지어지고, paint 쪽
  (`objectSize`·`layoutTable`)은 개체가 **놓이는** 단으로 다시 푼다 — 134.16→268.37pt
  이월에서 단 너비 50% 개체가 67.08pt만 예약하고 134.19pt로 그려져 뒤 글자를 67pt
  덮었다. 그래서 재조판 **전에** 조각 문자열의 예약을 목적 단으로 다시 풀고
  (`HwpInlineObjectReservation.rescaledForColumn` — 마커가 저장값·기준을
  `inlineObjectWidthRaw`/`inlineObjectWidthBasis`로 나른다), 렌더 문자열과 앵커 문맥에
  **같은 사본**을 준다 (`placedFragment`). 다시 풀 때는 **마커 한 글자씩** 훑는다 —
  `enumerateAttribute`는 값이 같은 이웃 run을 한 범위로 합치므로 크기 기준이 같은 개체가
  잇달아 있으면 한 범위로 오고, 범위 첫 마커의 치수를 통째로 얹으면 뒤 개체의 예약
  폭·높이가 덮인다 (PR 리뷰: 50%·75% 개체가 둘 다 첫 개체 치수로). 마커는 U+FFFC 한
  글자라는 부착 시점의 성질이 그 되짚기의 근거다. 높이는 안 푼다 — 높이 기준엔 '단'이 없다
  (종이·쪽·절대). **글자처럼 취급 표는 예외다** (#214): 예약이 그려지는 높이라 단 폭의 함수다
  (표가 단 폭으로 잘리거나 폭 기준이 단·문단이면 셀 줄바꿈이 달라진다) — `placedFragment`가 폭을
  ...
  마지막 조각은 종전대로 문단 단위 방출이 맡는다 — 단일 run 문단의 블록 순서가
  불변이다. 같은 리뷰가 잡은
  ...
  **셀 각주를 담지 않았다** — 문단 단위 수집이 표 셀을 건너뛰므로(`includeTableCells:
  ...
  ascent 초과분**(`ascentExcess`, 당시): 전진량이 CT baseline 간격이라 줄 k의 ascent가
  줄 k−1의 전진량에 실려 앞 조각이 가져가는데 조각은 독립 프레임이라 첫 줄 ascent를
  자기 상단에서 내린다 — 첫 줄이 큰 개체 줄인 조각이 짧게 재어져 뒤 문단이 개체
  위에 놓였다. 그 몫을 앞 조각에서 빼고 뒤 조각에 더했다. **#180 뒤로는 줄 프레임
  원점이 줄 상자 상단이라 이 보정 자체가 없다** (조각 높이 = 전진량 합).
  ...
  `inlineAnchors(in:)`를 측정과 공유). 그 한 줄의 원점 x는 렌더러가 가운데·오른쪽
  정렬에 주는 가로 오프셋(`slightOverflowAlignmentOffset` — 오른쪽 정렬은 음수)이다
  (PR 리뷰: 0으로 두면 오른쪽 정렬 조각의 앵커가 초과분만큼 오른쪽에 놓인다).
  **재조판된 조각이 이미 한 줄이어도 같은 오프셋을 얹는다** (PR 리뷰) — `layout`의 한 줄
  분기가 낸 원점 0을 그대로 두면 앵커가 그려진 글자보다 초과분만큼 오른쪽에 놓여 표가
  단 오른쪽 경계를 넘는다 (오른쪽 정렬 268.37pt 단에서 x 458.12 대신 450.24). 접기 술어가
  nil이면 (넉넉한 줄) 원본을 그대로 돌려주므로 한 줄 조각도 같은 술어에 태우면 된다.
  **측정의 한 줄 분기(`layout`)는 원점 0 그대로다** — 같은 오프셋을 얹어 보니 단 폭과
  같은 글자처럼 취급 표(noori 1쪽, 마커 폭이 단 폭을 0.4pt 넘는다)가 가운데 정렬
  오프셋 0.2pt만큼 단 왼쪽 밖으로 밀렸다(한글은 단 왼쪽 끝, 블록 스냅샷·렌더 해시
  회귀). 그 축은 별건이다 — 온전 문단은 `anchorLines`가 비어 접기 자체에 닿지 않는다.
  조각이 보장하는 것은 "앵커 = 우리 렌더러의 잉크"라는 **내부 정합**뿐이고, 오른쪽·가운데
  정렬 초과 줄에 대한 한글 실측은 아직 없다 — 실측 없이 클램프를 얹지 말 것. `HwpTableSplitter.paragraphFragment`는 원점을 되돌리지 않는데
...
- 장식 선의 세로 위치와 **두께**는 **한글 문서에서는 글꼴 지표가 아니라 글자 크기
  비례**다 (한글.app 12.30 PDF 내보내기 실측 — #136: 취소선 5~100pt·세 글꼴, 글자 위
  밑줄 10/40/100pt·두 글꼴; #176: 글자 아래 밑줄과 두께 5~100pt 13개 크기·함초롬바탕/돋움·
  Apple SD·HY울릉도M 네 글꼴에서 같은 값; #187: 13개 글꼴 전부 같은 값). 취소선 +0.35em ·
  글자 위 밑줄 +0.87em · 글자 아래 밑줄 −0.17em · 두께 0.04em(한글 PDF는 0.12pt 장치
  단위로 `0.12 × round(크기/3)`)은 **한 크기만 있는 줄**의 값이고 하한은 없다(5pt는 0.2pt).
  밑줄 종류 raw 2(글자 가운데)는 취소선과 같은 선으로 합류시킨다. 산식은
  `HwpDecorationLineGeometry` 한 곳이고 렌더러(`HwpPageLayerDecorations`)는 그 값을 그린다.
- **밑줄(글자 아래·글자 위·변경 추적 삽입)은 줄 단위, 취소선은 run 단위다** (#226, 한글
  12.30 PDF·줄 캐시 실측 2026-09-22·25 — 합성 HWPX 40여 표본 + `mixed-size-decorations` 쌍).
  한글 문서·한글 2007 호환 문서에서 밑줄의 **위 가장자리는 줄 상자 바닥**(베이스라인 아래
  0.15L), 글자 위 밑줄의 **아래 가장자리는 줄 상자 상단**(위 0.85L)이다 — L은 세로 배치가 쓰는
  줄 상자 높이(한글 캐시 `vertsize`, `LineMetrics.boxHeight`: 줄의 글자·문단 끝 글자(마지막
  줄)·한 줄 끝·높이 0 마커의 기본 크기와 글자처럼 취급 개체의 바깥 상자 높이 가운데 최댓값)이라
  같은 값을 `HwpDrawnTextLayout.underlineReference(of:endsParagraph:)`가 싣는다. 두께와 선 모양
  축척은 줄 **글자**의 기본 크기 최댓값 T(`LineMetrics.textFontSize` — 마커 run·한 줄 끝·빈 줄
  앵커·결합 구분자·문단 끝 글자는 세지 않는다)의 0.04배다. 중심은 −(0.15L + 0.02T)·
  +(0.85L + 0.02T)이고 한 크기 줄(L = T)이면 −0.17em·+0.87em이다. 한글 실측(10pt 밑줄 run과 한
  줄에 놓인 것): 40pt 무장식 글자·40pt 공백 −6.84·1.56, 40pt 문단 끝 글자·한 줄 끝·책갈피·
  그림·표 −6.12~−6.24·0.36(위 밑줄 +34.20), 20pt 무장식 + 40pt 문단 끝 글자 −6.36·0.84,
  바깥 여백 위 7·아래 3pt의 20pt 그림(L 30) −4.68·+25.68, 40pt 마커의 8pt 그림 + 20pt 글자
  (L 20, 마커 제외 #217) −3.36·0.84, 여러 줄 문단은 줄마다 따로(10pt만 있는 줄 −1.68). 크기는
  전부 **글자 모양 기본 크기**(`hwp.baseFontSize`, 슬롯 상대 크기 전)라 기본 40pt·상대 크기 50%
  run의 밑줄은 −6.84·1.56·취소선 +14.04, 기본 10pt·200%는 −1.68·+3.48이다. 취소선은 run
  자신의 기본 크기 × 첨자 축소 비율(기본 20pt·상대 크기 50% 위 첨자: 옮겨진 베이스라인 위 0.35 ×
  12.8, 두께 0.84)이다. 한글 2007 호환 문서는 같은 가장자리에 고정 0.36pt, MS 워드 호환 문서는
  글꼴 줄 상자(`msWordLineBox`, #187·#223)다. 선 모양 축척은 한글 문서만 T이고, 한글 2007 호환
  문서는 크기와 무관한 고정 축척(`Scale.hwp200XCharacterLine`, #227), MS 워드 호환 문서(미실측)는
  종전 run 크기다.
- **선 모양(표 25)은 `HwpLineShapeGeometry` 한 곳이 채우기 경로로 푼다** (#191, 한글 12.30
  PDF 실측 — 글자선 13종 × 5·10·20·40·80pt × 밑줄/취소선/위 밑줄, 테두리 13종 × 굵기 16단,
  단 구분선 13종 × 4단; 상수는 `HwpRenderTuning.LineShape`). 로컬 좌표는 x = 선 방향,
  y = 가로지르는 축(0 = 단선 중심, 양수 = 아래)이고 호출자가 아핀 변환으로 옮긴다 —
  글자선은 렌더러(`HwpPageLayerLineShapes`, y-위 텍스트 공간이라 뒤집는다), 테두리는
  `HwpBorderSet.edges`, 단 구분선은 `HwpColumnBandController.columnDividerBlocks`.
  **축척이 세 갈래다**: 글자선은 글자 크기(대시 단위 0.057em·2중선 띠 0.12em·그 밖 여러 줄
  띠 0.2em·물결 진폭 0.112em·획 0.03em), 테두리·구분선은 명목 두께(대시 단위 22/15 t·띠 t·
  물결 진폭 t·획 t/4) — 두 축의 모양 표(긴 점선 5·3, 점선 1·1.5, 일점쇄선 10·3·1·3, 이점쇄선
  10·3·1·3·1·3, 긴 파선 10·3; 여러 줄 [1/4·1/2·1/4]·[1/4·1/4·1/2]·[1/2·1/4·1/4]·[1/6·1/6·1/3·
  1/6·1/6])는 같다. **한글 2007 호환 문서의 글자선은 셋째 갈래 `hwp200XCharacterLine`이다**
  (#227, 2026-09-26 한글 12.30 PDF 실측 — 16개 모양 × 5~100pt × 아래·위·가운데 밑줄·취소선,
  크기가 섞인 줄·상대 크기·첨자·글꼴 넷 표본 493개가 전부 0.13pt 안, `hwp2007-decorations` 쌍):
  글자 크기를 입력으로 받지 않는 **고정 pt**다 — 대시 단위 0.48pt(배수 표는 같다), 원 지름
  1.32pt·간격 3.0pt(중심이 띠가 자라는 쪽으로 0.24pt — 아래 밑줄 아래·위 밑줄 위·취소선 0),
  2중선 띠 1.44pt([1/4·1/2·1/4]), 굵은 여러 줄 띠 4.2pt(가는+굵은 [8/35·8/35·19/35]와 그 거울,
  3중선 [1/7·1/7·3/7·1/7·1/7] — 비율부터 한글 문서와 다르다), 물결 진폭 2.88pt·획 0.72pt·
  꼭짓점 = 단선 중심 위 0.12 + 1.2 × 종류(pt), 2중 물결 진폭 1.44pt·획 0.36pt·둘째 파 1.08pt
  아래·꼭짓점 0.18 + 0.54 × 종류(단일 물결과 다른 꼭짓점 — 한글 문서는 둘이 같고 기점이 단선 위
  가장자리, 계단이 두께다). 두께가 고정 0.36pt인 것(#210)과 같은 갈래라 첨자·상대 크기·줄 크기
  어느 것도 무늬를 바꾸지 않는다. 무늬의 run 단위 재시작·슬롯 전환 이어짐·마지막 반주기
  완주·3D 넷 무표시(우리는 실선 대체)는 한글 문서와 같다. 한글은 OWPML 이름과 반대로 `DOT`(`longDotLine`)를 긴 점선, `DASH`
  (`dotLine`)를 점선으로 그린다 (#177). 띠의 자리는 `Placement`: 아래 밑줄은 단선 띠 위
  가장자리에서 아래로, 취소선·테두리는 가운데, 위 밑줄은 아래 가장자리에서 위로; **물결만
  예외**로 글자선은 단선 위 가장자리에서 두께 × (밑줄 종류: 아래 1·가운데 2·위 3)만큼 위에서
  시작해 진폭만큼 내려오고, 테두리·구분선은 선 중심에서 −y/−x 쪽으로 3t/8 치우친다(위·
  아래·왼·오른 변·구분선 모두 같은 방향 — 바깥쪽이 아니다). 2중 물결은 같은 파를 글자선
  ...
  원형 점선은 채운 원(글자선 지름 = 대시 단위·피치 2.5배, 첫 원 중심 = run 시작; 테두리
  지름 = t·피치 2t·첫 원 중심 = 선 시작). **3D 넷은 한글 macOS가 글자선·테두리 모두
  아무것도 그리지 않는다**(PDF 벡터 0건, PrvImage에도 없음) — 여기서는 실선으로 대체한다
  (사라지는 것보다 낫고 윈도 한글은 그린다). 5·10pt의 ±1u(0.12pt) 정수화 차는 안 좇는다.
  ...
  - **테두리는 셀 모서리에 중심**(양쪽으로 t/2, 종전 셀 안쪽 띠가 아니다)이고 공유 모서리는
    ...
    원형은 연장 없음(가로 변이 모서리를 메운다), 세로 물결은 이웃 가로 변 폭의 절반 연장한
    곳에서 시작(실측: 위 테두리 있는 왼 물결 변 첫 꼭짓점 = 모서리 − t/2), 여러 줄은
    ...
    내면 나중 셀의 채움이 앞 셀 테두리의 바깥 절반을 덮는다. 히트(`tableHit`)도 같은 두
    단계다 — **모든 셀의 내용을 셀 역순으로** 본 뒤에야 어느 셀의 채움 ∪ 칸막이든 가림으로
    치는데, 셀마다 내용 → 칠을 섞어 보면 앞 셀에서 뒤 셀 자리로 넘친 개체(R44 #2)가 뒤 셀
    채움 위에 그려졌는데 히트는 뒤 셀 채움에서 멈춘다(PR 리뷰). 중첩 표는 자기 덩어리로
    부모 셀 내용의 제 자리(walker `onNestedTable`~`onNestedTableEnd`)에 들어 각주의 글 뒤로
    표(R47 #1)도 그대로 텍스트 뒤에 남는다. 히트 자격 영역(`HwpHitCoverage.paintedRects`,
    텍스트 포함 갈래)은 셀 rect에 `paintedBounds(around:)`(테두리 바깥 절반)을 더해 R54
    `자격 ⊇ 칠`을 지키고, 프레임 밖 그 띠(중첩 표 칸막이 포함, `HwpTableFrame.paints`)의
    탭은 표 블록이 claim한다(`HwpHitTester.hit`; 글상자 블록 자신의 테두리 바깥 절반은 아직
    안 한다 — #191 밖의 기존 격차); 배치 하한 `paintedObjectBounds`는 종전대로 셀 rect다
    (한글이 표 아래 각주 자리를 선 바깥 절반까지 재는지 미실측 — 그림·글상자 획 절반은 세고
    셀 테두리 절반은 안 세는 비대칭이 남는다).
  - 단 구분선은 밴드가 닫힐 때(`closeColumnBand`·`cacheCurrentPage`, `dividersEmitted`로 한
    ...
    개체·쪽 장식은 안 센다; 닿았는지는 블록 아래와 그 줄 상자 아래 중 낮은 쪽으로 잰다 — 고정 줄
    간격이 상자보다 작아 상자가 블록 아래로 나가면 다른 단의 표가 그 사이에서 끝나도 닿은 것이다,
    #222 PR 리뷰) 본문 텍스트 블록마다 (블록 아래 − 그 블록 마지막 줄의 줄 간격) 중 가장 낮은
    자리, 표면 사용량 그대로 — 이고 둘째 단이 비어도 그린다. 줄 간격은
    **블록마다** 낸다(`columnDividerBlocks(trailingSpacing:)`): 흐름 배치가 기록한 마지막 줄 상자
    아래 몫(`trailingSpacesBelowLineBox`, #222 — 줄 간격 여분 + 문단 아래 간격)이 있으면 그 값
    (한글 12.30 실측 `probes/222` DA: 단 끝 문단의 아래 간격 20pt는 구분선에 들지 않고 구분선은 그
    줄 상자 바닥에서 끝난다 — 쪽 끝 적합이 줄 상자까지라 그 간격이 본문 아래로 넘친 단에서도 같다),
    블록 문자열에 `hwp.cachedTrailingLineSpacing`이 있으면 그 값, 없으면 블록 문자열을 그 블록이 그려지는 폭으로
    ...
    뒤 문단 값이 샐 수 있다(PR 리뷰). 문단 아래 간격이 있는 마지막 문단은 실측했다 — 쪽 끝 단
    (`so222d` DA)과 쪽 중간에서 균형 배분된 밴드(`so222r`) 모두 아래 간격과 무관하게 줄 상자
    바닥에서 끝난다(#222). 미실측: 캐시 없는 문단의 마지막 줄에 더 큰 글자(줄 경계를 모른다),
    캐시 없는 문서의 밴드 사이 간격(0으로 둔다 — 한글은 줄 간격만큼).
  - PrvImage fidelity는 이 변화를 **거꾸로** 본다 — 한글 화면 렌더는 0.12mm 선을 최소 1px로
    ...
    (4px/pt 픽셀 핀)·`HwpLineShapeGeometryTests`·`HwpBorderSetEdgeTests`·`HwpColumnDividerTests`.
- **변경 추적 표시선은 일반 선과 같은 기하다** (#187 실측: 한글 문서 13개 글꼴·MS 워드
  호환 문서 32개 글꼴 전부 삽입 밑줄 = 글자 아래 밑줄, 삭제선 = 취소선 — 같은 줄에서
  장치 좌표까지 같다, `track-changes-native` 픽스처). 종전(#176)의 전용 상수(삭제선
  +0.29em·삽입 밑줄 −0.26em·두께 0.064em)는 `track-changes` 픽스처가 MS 워드 호환
  문서라서 나온 함초롬돋움의 호환 문서 값이었고 지웠다. 삽입 표식은 색만
  `hwp.trackInsertUnderline`으로 나르고 삭제 표식은 취소선 키 그대로다.
- **MS 워드 호환 문서(호환 문서 대상 프로그램 2, `HwpCompatibleDocumentTarget.msWord`)
  에서는 밑줄·취소선이 글꼴 지표에서 나온다** (#187, 2026-09-15 한글 12.30 실측: 글꼴
  32종 + OS/2 비트를 바꾼 합성 글꼴 7종 × 10·40·80pt, 혼합 줄 12조합, `compat-decorations`
  쌍). `HwpIndex.compatibleDocumentTarget`을 조판이 모든 run에
  `hwp.compatibleDocumentTarget`으로 싣고 렌더러가 `msWord`와 `hwp200X`를 가른다 —
  훈민정음(4)만 한글 문서 기하다 (실측 일치). **한글 2007 호환(1)은 두께가 크기와 무관한
  고정 0.36pt이고 밑줄은 한글 문서와 같은 가장자리(아래 0.15em·위 0.85em — 크기가 섞인 줄에서는
  줄 상자 바닥·상단, #226)에 그 얇은 선을 얹는다** (#210, 2026-09-22 한글 12.30 실측: 9개 글꼴 × 5~100pt 22개 크기에서 두께가 전부
  0.36pt, 중심은 40pt에서 −6.12·+34.20·+14.04pt, `hwp2007-decorations` 쌍) — 취소선 중심은
  한글 문서와 같고 기준 크기는 글자 모양 기본 크기다. 실선이 아닌 선 모양의 무늬도 크기와
  무관한 고정 pt다 (#227 — 위 선 모양 항목의 셋째 축척). 레이아웃 호환성 플래그(표 56)는 관여하지
  ...
  - **밑줄(글자 아래·글자 위·삽입)은 줄 단위**: 줄의 run 상자(글자 크기 × 글꼴 상자)를
    **축별 최댓값**으로 합친 줄 상자에서 아래 밑줄 중심 −(descent + 0.021 cell)·위 밑줄
    ...
    (descent·ascent는 가장자리에서 0.15 cell 안쪽). 문단 끝 글자·개체가 줄 상자를 키운 줄
  ...
  - **상자에 곱하는 크기는 글자 모양 기본 크기**(`hwp.baseFontSize`)다 — 슬롯 상대 크기를
    50%로 줄여도 밑줄·취소선 자리·두께가 100%와 같다 (2026-09-16 실측 `cm-word-relsz`:
    함초롬 20pt `가나` + Helvetica 40pt `Ag`의 밑줄 −10.3pt·2.64pt, 취소선 첫 글리프 글꼴 ×
    40pt). run 글꼴 크기로 곱하면 슬롯이 갈린 글자 모양 안에서 취소선이 계단이 진다 (PR
    리뷰). 첨자 축소 비율은 취소선에만 곱한다(#179 유추, 호환 문서 첨자 표본 없음).
  - **취소선(가운데 밑줄·삭제선 포함)은 run 단위**: run 상자의 ascent × 0.273, 두께는
    한글 문서와 같은 0.04 × 기본 크기. 한글은 글자 모양 run의 첫 글리프 글꼴을 run 전체에 쓰므로
  ...
  - **줄 상자·베이스라인·전진량도 같은 글꼴 줄 상자다** (#194, 2026-09-20 한글 12.30 실측:
    `targetProgram="MS_WORD"` 합성 HWPX 6종 — 줄 간격 종류 × 값 17문단·글꼴/크기 혼합
    ...
    - 줄 간격 종류별 규칙은 한글 문서와 같다 — 상자만 글꼴 상자로 바뀐다 (Apple SD 산돌고딕
      Neo 10pt 상자 1559: 고정 5·16·30 → 500·1600·3000, 여백만 0·3·10 → +0·300·1000, 최소
      5·16·30 → 1559·1600·3000, 비율 160·200·300·80 → +932·1556·3112·−312). 문단 위/아래
      간격도 같다 (앞 문단 아래 + 뒤 문단 위, 쪽 첫 줄은 상자 상단이 쪽 상단).
    - **비율 여분은 4 HWPUNIT 양자다** — 글자 상자를 4 단위로 내림한 뒤 (p − 100)%를 곱해
      반올림한다 (`HwpLineSpacingRule.percentShare`: 1559 → 389 × 0.6 = 233.4 → 932, Menlo
      1515 → 908, 함초롬 1692 → 1016). 한글 문서의 실물 캐시도 같은 규칙이다(`CCL`·`noori`
      15pt 170% `spacing` 1052 = 4 × 263, 262.5의 반올림; 헌법주석 각주 9pt 130% 6,920줄
      전부 272 = 4 × 68, 67.5의 반올림 — 270이 아니다; 10.5pt 160% 628 — 상자 262.5도
      내림; `noori` 13pt 130% 392). 상자가 4의 배수인 보통 크기에서는 산술과 같다. 이
      규칙으로 헌법주석 각주 줄 피치가 11.70 → 11.72pt, `noori` 15pt 본문이 25.50 →
      25.52pt로 캐시와 같아져 렌더 해시 기준선 23쪽(헌법주석 21·noori 2)을 다시 기록했다.
    - **글자처럼 취급 개체 줄**: 베이스라인 = max(글꼴 상자 베이스라인, 개체 높이), 상자 =
      개체가 **글자 상자보다 높으면** 개체 + 글자 상자의 베이스라인 아래 몫 — 10pt 줄(1559/1104)에
      30pt 표를 넣으면 `vertsize` 3455 = 3000 + 455·`baseline` 3000(개체 바닥이 베이스라인), 셀
      내용으로 18.41pt가 된 표는 2296·1841. 개체가 글자 상자보다 **낮으면** 베이스라인만 내리고 상자는
      글자 상자 그대로다 (#223: Apple SD 20pt 줄 + 25pt 표 3119/2500, 함초롬 10+20pt 줄 + 30pt 표
      3383/3000 — 종전 산식은 34.12·38.53로 부풀렸다). 비율 여분은 한글 문서처럼 글자 상자
      기준(932)이고 고정·여백만·최소는 그 상자에 적용한다 (3455 + 300, max(3455, 1600)). 밑줄은
      이 글꼴 줄 상자의 가장자리에서 잰다 (`underlineReference`가 `msWordLineBox`를 싣는다 — 한글
      문서의 줄 상자 바닥 규칙(#226)은 호환 문서에 걸리지 않는다; 개체만 있는 줄도 같다).
    - **줄 끝 글자와 개체는 글자 상자에 쌓인다** (#223, 한글 12.30 실측 2026-09-25: 합성 HWPX 7종
      ~200줄 — 함초롬돋움·Apple SD·Menlo·Helvetica·Times New Roman과 교차 조합, 10~40pt, 줄 간격
      종류, 한 줄 끝, 개체, 밑줄 — 을 다시 저장한 줄 캐시와 PDF, `ms-word-paragraph-end-box` 쌍).
      `LineMetrics.msWordLineBox`가 글자 상자 T(글자 run들의 축별 최댓값)에 **줄 끝 글자 상자**
      C(문단의 마지막 줄이면 CR 상자 `hwp.msWordParagraphEndBox`, 한 줄 끝으로 끝나는 줄이면 그
      `hwp.lineBreak` run의 상자)와 개체 높이 O를 들인다:
      높이 = T. 다만 **C가 T보다 높으면** C + T의 아래 몫(T.h − T.b), **O가 T보다 높으면** O + T의
      아래 몫과 견줘 큰 것 — 베이스라인 = max(T, C, O). 함초롬돋움 10pt 글 + 12·16·20·40pt CR은
      2456·3132·3809·7192 = CR 상자 + 426, 16pt 글 + 10pt CR은 2706(쌓이지 않음). 가르는 것은
      **상자 높이**다 — 크기도 베이스라인도 아니다: Menlo 10pt 글(1515)에 함초롬 10pt CR(1692)은
      2103으로 쌓이고 함초롬 10pt 글에 Menlo 11pt CR(1666)은 1692 그대로, Apple SD 20pt 글에 Menlo
      20pt CR(베이스라인은 더 깊고 상자는 낮다)은 축별 최댓값 3119/2207, Menlo 10pt 글에 Apple SD
      10pt CR(상자는 높고 베이스라인은 얕다)은 1970/1104. 아래 몫은 **합친 T**의 것이다 (Apple SD
      20pt 한글 + Menlo 20pt 라틴 + 함초롬 30pt CR = 5986 = 한글의 함초롬 30pt 상자 5074 + T의 아래
      몫 912(3119 − 2207), Apple SD 혼자의 959가 아니다; 한글 빈칸이 라틴 슬롯이라 Apple SD/Menlo
      글자 모양의 한글 줄 T는 보통 1559/1104다). 비율 여분은 종전대로 max(T, C) 기준(3132 줄의
      160% `spacing` 1624 = 0.6 × 2706, 200% 2704)이고 고정·여백만·최소는 쌓인 상자에
      걸린다(여백만 5 → 3632, 최소 30 → 3132). 한 줄 끝도 같다 — 10pt 글 + 20pt 한 줄 끝 줄
      `textheight` 3809, MS 워드 호환 문서의 한 줄 끝은 **라틴 슬롯** 글꼴이다
      (`HwpTextRunBuilder.appendLineBreak`: 한글 슬롯 Apple SD·라틴 슬롯 함초롬의 한글 글 뒤 한 줄
      끝 줄 2171 = 1692 + 479, 직전 슬롯이면 1559). 그 줄의 캐시 `vertsize`는 이상치이고
      `textheight`가 배치다(#206의 한 줄 끝 줄과 같은 꼴). 한 줄 끝 뒤 빈 마지막 줄은 **C뿐**이다 —
      빈 줄 앵커(`hwp.emptyLineAnchor`)와 결합 문자열의 문단 구분자
      (`HwpAttributedStringKey.combinedParagraphSeparator`)는 줄 끝 글자의 대역이라 C가 있으면 T에서
      빠진다 (10pt 한 줄 끝 + 16pt CR의 빈 줄 2706/2025, 16pt 한 줄 끝 + 10pt CR은 1692/1266; 구분자를
      글자로 세면 개체로 끝나는 문단이 홀로 잰 상자와 결합 문자열에서 잰 상자가 갈린다). T가 없는
      줄(개체와 CR뿐)은 max(C, O)다(30pt 표 + 10pt CR 3000/3000, 40pt CR 6766/5063). 글자도 줄 끝
      글자도 없이 개체만 있는 줄(넓은 개체가 자동 줄바꿈으로 한 줄에 하나씩 놓인 문단의 앞 줄)은
      O 그대로이고 비율 여분은 개체 마커의 기본 크기 기준이다 — 개체 마커 글꼴은 T로 떨어지지 않는다
      (PR 리뷰 실측: 함초롬돋움 10pt 마커의 30pt 표 줄 3000/3000·160% `spacing` 600, 16pt 마커 960,
      Apple SD/Menlo 마커도 같다; 셀 내용으로 16.92pt가 된 표 1692/1692·600; 그 줄에 구역 첫 문단의
      구역·단 정의나 10·16pt 책갈피가 함께 있어도 3000/3000·600 — 폭·높이 0인 마커의 글꼴도 T가
      되지 않는다. 마커 글꼴 상자를 T로
      삼으면 그 줄만 3426·1016이 되어 같은 문단의 마지막 줄 3000과 규칙이 갈렸다 — 다시 조판한
      뒤 문단이 한글 PDF보다 그런 줄마다 8.3pt 안팎씩 아래에 놓였다). 같은 글꼴·크기의
      C는 T와 정확히 같은 값이라 부동소수 문턱(`LineMetrics.stackingTolerance` 0.001pt)을 넘어야
      쌓는다 — `compat-decorations`의 절반이 그 등호 위에 있다. 이 규칙이 없던 동안(#194의 축별
      최댓값) 끝 글자가 큰 줄은 T의 아래 몫만큼 짧아 뒤 문단이 위로 당겨졌다(함초롬돋움 10pt +
      16pt CR 뒤 4.26pt씩). 히트 판정의 잉크 상한(`msWordVerticalInkReach`)은 개체를 예약한 문자열에서
      descent 전체를 아래 여유로 둔다 — 글자 상자 안쪽 개체가 줄 상자의 베이스라인 아래 몫을 0 가까이
      줄일 수 있다.
      **남은 격차**: MS 워드 호환 문서의 **고정** 줄 간격 줄은 글자를 줄 높이의 0.8 자리에, **최소** 줄
      간격이 상자보다 큰 줄은 줄 바닥에 맞춰 그린다(한글 PDF: 고정 5·16·30pt 줄의 베이스라인이 줄
      상단에서 3.93·12.73·23.97, 최소 30pt 줄은 30 − 상자 아래 몫 — 줄 캐시의 `baseline`과 다르다).
      줄 상자·전진량은 맞고 그 줄의 글자 자리만 다르다 — 이 축 밖(#194 실측은 캐시만 대조했다).
    - **마커 run(run delegate)의 글꼴은 줄 상자에 들지 않는다** — 글자 모양이 함초롬돋움
      10·20pt인 표 마커나 책갈피 컨트롤을 Apple SD/Menlo 10pt 줄에 넣어도 1559·1104
      그대로다. 다만 줄 공간을 **예약한** 마커(글자처럼 취급 개체)의 기본 크기는 비율 여분의
      기준에 든다 (20pt 글자 모양 표 마커가 든 10pt 줄의 160% `spacing` 1200 = 0.6 × 2000,
      20pt 책갈피는 932 그대로 — `LineMetrics.msWordSpacingBase`). 글자 run도 줄 끝 글자도
      개체도 없는 줄만 줄 공간을 **예약하지 않은** 마커(구역·단 정의, 책갈피, 자리 차지 개체
      앵커)의 글꼴로 떨어진다 — 개체가 있는 줄은 상자가 개체다(위 "줄 끝 글자와 개체는 글자
      상자에 쌓인다"). 전진량의 상자와 여분 기준은 독립이다
      (`advance(lineBoxHeight:textBoxHeight:)` — 상자 1559에 기준 2000).
    - **빈 문단·빈 줄 앵커는 라틴 슬롯 글꼴이다** — 그 줄은 문단 끝 글자(CR)뿐이고 한글은
      CR을 라틴 슬롯으로 세운다 (Apple SD/Menlo 10pt 글자 모양의 빈 문단 `vertsize` 1515·
      `baseline` 1104 = Menlo, 20pt 3029·2207; 함초롬돋움 1692·1266). 조판이
      `emptyParagraphAnchor`를 `.english` 슬롯으로 내고 `finishEmptyLastLineAnchor`가 앵커
      글꼴을 `msWordParagraphEndFont`로 바꾼다 (한글 문서는 종전대로 한글 슬롯 — 상자가
      글꼴과 무관하다).
    - 상대 크기는 상자에 들지 않는다 (한글 슬롯 50%·200%도 1559·1104 — 글리프가 상자를
      넘친다). 글꼴 대체 run은 대체 글꼴의 상자로 든다 — 한글은 Times New Roman·Helvetica
      글자 모양의 한글을 함초롬돋움으로 대체해 그 줄이 1692/1266이 되는데, 우리 resolver는
      AppleMyungjo·Apple SD로 대체하므로 그런 줄은 한글과 1.9~3.3pt 어긋난다 (글꼴 대체
      정책의 격차, 이 축 밖).
    - **남은 격차**: ① 한글은 글꼴 상자를 글꼴마다 ±0.1% 다르게 반올림한다 — Apple SD
      10·20·30·80pt 1559·3119·4678·12477(산식 1560·3120·4680·12480), Menlo 1515·3029(1513·
      3027), 함초롬 1692·3383·13531(1690·3380·13520), 크기 7~100pt 스윕 90줄로 어떤 단순
      반올림 모델(ppem 정수·26.6·HWPUNIT ceil/floor)도 전수 일치하지 않았다. 산식 그대로
      ...
      아래 몫 455 (우리 산식은 Menlo 30 상자 4540 + 글자 상자 아래 몫 457(15.60 − 11.03) = 4997 —
      ①의 글꼴 상자 반올림 격차 안).
    ...
    - 이제 호환 문서 픽스처의 베이스라인을 쪽 좌표로 핀한다 (`FixtureBaselineAnchorTests`
      ...
      기기에서만).
  - 덤: 변경 내용 추적 문서의 PDF 내보내기는 쪽 전체를 ~0.8배로 줄인다(왼쪽 변경 막대
    여백) — 글리프가 7.92·31.68pt로 찍혀도 상대 크기가 아니라 쪽 축소이며 em 비율은
    그대로다
- **글자 위치(표 33)는 `glyphBaselineOffset` 한 키로만 나른다** — 조판 문자열에
  ...
  무시한다"는 macOS 27.0에서 거짓이다. 첨자(`superscriptBaselineRatio`·
  `noteReferenceBaselineRatio`)는 처음부터 이 커스텀 키만 쓰므로 영향이 없다. 첨자 몫은
  `hwp.scriptBaselineOffset`에도 따로 실린다 (#179, 아래 장식 선 항목).
- **첨자 글리프의 배율과 이동은 두 규칙이다** (#204, 한글 12.30 PDF 실측 — 함초롬바탕·
  Apple SD 산돌고딕 Neo·함초롬돋움, 8~100pt 스윕, 상대 크기 50·150% 조합):
  - 글자 모양의 위/아래 첨자(표 33, `applySuperscript`·`applySubscript`): 글꼴 크기의
    **0.64배**, 위 첨자는 설정 크기의 **0.44배 위**, 아래 첨자는 **0.12배 아래**
    (`superscriptScale`·`superscriptBaselineRatio`·`subscriptBaselineRatio`; 10pt →
    6.36pt·4.32~4.44 위·1.20 아래, 100pt → 63.96·44.04·12.00). 상대 크기는 축소에는 들고
    이동에는 안 든다 — 20pt 상대 50% 위 첨자는 6.36pt로 8.88pt 위, 150%는 19.20pt로 8.76 위.
  - 각주·미주 참조 번호(본문 마커와 각주 내용의 위 첨자 번호,
    `applyNoteReferenceSuperscript`): 글꼴 크기의 **0.75배**, 설정 크기의 **0.21배 위**
    (`noteReferenceScale`·`noteReferenceBaselineRatio`; 10pt → 7.56pt·2.16 위, 20pt →
    15.00·4.20, 100pt → 75.00·21.00; 상대 50%·150%의 20pt 본문도 7.56·22.56pt에 4.20 위).
    글자 모양 첨자 규칙으로 그리면 10pt 본문의 번호가 1.2pt 작고 2.3pt 높다.
  - 각주 내용의 번호는 **구역 각주 모양의 위 첨자 플래그**(표 134 bit 12)를 따른다 —
    합성 HWPX에서 `hp:autoNum`의 `supscript="1"`만 켜고 구역은 0으로 두면 한글은 번호를
    보통 크기로 그리고 재저장 HWP의 표 143 bit 12도 0으로 적는다(우리 HWPX 매퍼는
    `autoNum`의 값을 그대로 옮기므로 그런 합성 입력에서만 갈린다 — 실물은 둘이 같다).
  - 종전 값(0.67배·0.33em·0.30em, `CharShapeProperty` 실물 육안 추정)은 10pt에서 위 첨자가
    1.1pt 낮고 아래 첨자가 1.8pt 낮았으며, 참조 번호는 첨자 규칙을 그대로 써 0.9pt 작고
    (6.7 vs 7.56) 1.2pt 높았다(3.3 vs 2.16). 픽스처 핀은 `script-decorations`(첨자 취소선·
    가운데 밑줄 절대 좌표)와 `footnote-endnote`(참조 번호 글꼴 7.5pt·베이스라인 105.6pt,
    `FixtureNoteReferenceTests`).
  - **헌법주석(`legacy-common-control-property`)도 같은 규칙이다** — 한글 12.30이 내보낸
    1,030쪽 PDF에서 본문 10.56pt의 참조 번호가 7.92pt(0.75배)·2.16~2.28 위(0.205~0.217em),
    12pt 본문은 9.00pt·2.52 위. 다만 한글은 그 번호를 문서 글꼴(FZSY 대체)로 그려 `4)`가
    1.0em 폭인데 우리 HCRBatang은 0.87em이라(글꼴 대체 정책의 격차, 이 축 밖) 번호가 넓어져
    옮겨진 줄바꿈이 한글과 더 맞아지지는 않는다 — 한글 PDF 줄 시작 31,874개 중 우리 줄
    시작과 일치하는 수가 10,665 → 10,573(33.5 → 33.2%, 110쪽 감소·55쪽 증가). 나머지 2/3의
    불일치는 글꼴 대체와 압축(자간) 미모델 같은 기존 축이다. 렌더 해시는 877/1,030쪽이
    바뀌었고(참조 번호가 있는 875쪽 + 앞 쪽 줄바꿈이 넘어온 p126·p934) 그중 348쪽은 줄바꿈이
    옮겨진 쪽이며, 블록 스냅샷은 조각 경계가 1~2자 옮겨진 p721~723·p894 네 쪽의 블록 다섯
    곳이다(프레임 불변).
- **그 이동은 기하 질의에도 반영한다 — 단 넓히는 것이지 옮기는 것이 아니다** (#197 리뷰).
  ...
  - **하나의 rect로 합치지도 않는다** (리뷰 2·3차): 줄 전체 폭에 최대 오프셋을 걸면 안
    ...
    **`hyperlinkRegions`도 같다**: 스팬 전체 폭에 최대 오프셋을 걸면 같은 URL의 안 옮겨진
    글자 위·아래 빈 자리를 그 링크가 가져가 **뒤에 있는 링크가 진다**. 줄 rect(스팬
    가로 범위 — 세로는 #233부터 클릭 띠)를 그대로 내고, **그 스팬에 속한** 옮겨진 run마다 밴드를 하나씩 더 낸다 —
  ...
  - **밴드는 그 스팬의 run 것만 가져간다 — 가로 클립으로는 못 가른다** (리뷰 4차, 위 항목의
    **회귀 수정**이다: 밴드를 처음 넣은 48d7fa1은 스팬 rect를 **그 스팬 자신의** 오프셋으로
    ...
    당시 스팬 rect는 양끝
    오프셋을 min/max로 정규화해 냈으므로(#1, #201에서 run별 구간으로 교체) **양방향 줄에서 그
    상자가 다른 링크의 글자를 덮었다**.
    ...
    א 위의 탭이 아무것도 안 열었다). 밴드에 run의 **CTLine 문자열 범위**를 실어 자기 스팬의 run만
    고르고, 고른 뒤에는 **자르지 않는다**. 당시에는 run 전부가 스팬에 드는 **포함**으로 물었다 — CT가
    속성 경계마다 run을 끊는다고 봤기 때문이다 (실측: 그 줄의 run이 ct[0,3)·[3,4)·[5,7)·[4,5)로
    정확히 갈린다 — 폰트 폴백이 한 스팬을 여러 run으로 쪼개도 전부 그 스팬 안이다). 그러나 CT는
    자소 묶음·글리프 없는 문자를 속성이 달라도 앞 run에 합치므로(#201 리뷰) 포함은 자기 밴드를
    버렸고, **#201에서 밴드 소속도 "묶음의 첫 run의 첫 글자가 든 스팬"으로 바꿨다** — 줄 상자 구간
    `RunExtent.belongs(to:)`와 같은 규칙이다. 남의 run은 첫 글자가 남의 스팬에 있으니 그 규칙으로도
    **남의 잉크를 안 가져간다**. 단방향 줄은 run이 늘 스팬 상자 안이라 한 자리도 안 바뀐다 (13케이스
    ...
    방출 ≡ 히트 ≡ 칠. 줄 상자 rect 자체의 bidi 오귀속(상자 **안** 탭은 앞 URL이 열린다)은
    밴드와 무관하게 min/max 정규화가 만든 별개의 결함이었고 #201에서 스팬 rect를 run별로 내
    고쳤다 — 바로 아래 항목.
  - **스팬의 줄 상자 rect는 양끝 오프셋이 아니라 run별 진행 폭 범위를 화면에서 잇닿은 구간별로
    합쳐 낸다** (#201, `RunExtent`·`visualSegments`). 양방향 줄에서는 논리 순서와 화면 순서가 달라
    한 스팬이 화면에서 **떨어진 여러 구간**을 차지하고, 방향 경계 인덱스의
    `CTLineGetOffsetForStringIndex`는 반대쪽 run의 가장자리를 가리킨다 (실측 `abc אבג`, Helvetica
    10pt: 인덱스 4 → 18.901·7 → 18.901인데 א는 28.804…35.254). 그래서 종전 상자(0…28.804)는 뒤
    링크 `בג`(18.901…28.804)를 덮고 자기 글자 א를 빠뜨렸으며, **스팬 하나로 전체를 걸어도**
    히브리 글자 셋이 통째로 빠졌다(0…18.901). run은 화면 순서로 놓이고 자기 자리를 안다:
    **기저 글리프** 위치에 run 텍스트 매트릭스를 건 값과 `CTRunGetTypographicBounds` 폭 —
    positions는 매트릭스 **적용 전** 좌표라(장평 50% run: 266.797인데 실제 133.398) 렌더러
    (`drawRun`)처럼 매트릭스를 걸어야 하고, 폭·advances는 적용 후 값이다. **기준 글리프는 방향에
    따라 다르다** (PR 리뷰 실측): run 안 글리프는 시각 순서인데 결합 부호가 LTR run에서는 기저
    뒤, RTL run에서는 기저 **앞**에 오므로, LTR은 첫 글리프 위치가 왼쪽 끝이지만 RTL의 첫 글리프는
    논리 마지막 글자의 부호일 수 있고 그 x는 기저 + GPOS 오프셋이다(GeezaPro `شَّ` 3.71pt·함초롬바탕
    아랍 1.26pt 오른쪽, Times New Roman 홀람 1.59pt **왼쪽**; Lucida Grande 폴백 히브리·함초롬
    히브리는 부호 x = 기저 x라 안 보인다). 첫 글리프로 잡은 첫 형태는 그런 run의 rect가 줄 상자
    밖으로 나가고(잉크 왼쪽이 어느 링크도 아님) 홀람은 이웃 링크 글자 1.6pt를 덮었다 — RTL run은
    마지막 글리프(논리 첫 글자의 기저) 위치 + advance(run 끝까지의 델타)를 오른쪽 끝으로 잡고 폭을
    뺀다(`testRTLRunEndingWithCombiningMarkIsMeasuredFromItsBaseGlyph`). 남은 사각지대는 링크
    경계가 기저|부호 사이에 떨어져 run이 부호로 **시작**하는 경우뿐이다. `CTLineEnumerateCaretOffsets`·양끝 오프셋의 secondary 값도 검토했으나 클러스터 짝짓기·
    경계 쪽 선택이 문서화되지 않은 규칙에 기대 run이 낫다. 인접 run의 끝과 다음 run의 시작은
    CT가 같은 누적값을 줘 비트 단위로 같고(프레임 안 줄의 개행 run은 폭 0) 잇닿음 판정 여유는
    0.001pt다(실측 글꼴에서는 비트 동일이지만 폴백 글꼴에 따라 1e-15 수준 오차가 날 수 있다 —
    리뷰 실측). **스팬 소속은 run의 첫 글자로 정한다** — 글자 위치 밴드(`GlyphOffsetBand.belongs`)
    도 같은 규칙이다(#197 리뷰 4차의 포함 판정을 함께 바꿨다): CT는 속성 경계마다 run을 끊지만
    **자소 묶음은 예외**라 첫가끝 자모(U+1112 U+1161 U+11AB)·ZWJ 이모지
    열·아랍 lam-alef는 링크 경계가 묶음 안에 떨어져도 한 run으로 나오고(결합 부호 U+0301·서로게이트
    쌍은 갈린다) 그 run의 속성 사전은 첫 글자 것이다. 포함이면 그 run이 양쪽 스팬에서 버려져 클릭
    구멍이 나고(리뷰 지적), 렌더러는 그 run을 첫 글자의 링크로 그리므로 소속도 첫 글자여야 방출 ≡
    칠이다(종전 산식은 캐럿이 묶음 안에서 0폭이라 **뒤** 스팬이 통째로 가져갔다 — 그려진 링크와
    어긋났다). 잇닿은 run을 합친 구간마다 rect 하나 — 단방향 줄에서는 구간이 늘 하나라 자간이 없으면
    종전 상자와 같다(프로브 대조 21케이스 비트 동일: 단일·다중 스팬·글꼴 폴백·꼬리 공백·장평·기울임
    근사·줄 넘김·양쪽 정렬·글자 위치 밴드·한 줄 끝 표식·여러 문단·탭·폭 0 마커; 저장소 테스트
    `testUnidirectionalSpansKeepTheLegacySpanBox`는 그중 6케이스를 종전 산식 복제본과 0.001pt로
    대조한다). **자간(`kCTKern`)이 있으면 다르다**: 종전 캐럿은 글자 사이 kern을 반씩 나누고 줄
    끝에서 kern을 통째로 뺐는데(실측 kern +2: 경계 14.123 vs run 끝 15.123, 줄 끝 27.685 vs 줄 폭
    29.685; kern −1은 줄 끝 캐럿 18.685가 줄 상자 17.685 **밖**), 새 구간은 줄 상자
    ...
    HWP 자간 −30%로 닿는다) run 끝이 시작보다 왼쪽이고, 그대로 `ClosedRange`를 만들면 프로세스가
    종료된다 — `RunExtent`가 `minX ≤ maxX`로 정규화한다. 다만 접힌 진행 폭 [시작 + 폭, 시작]만 담으면
    원점 **오른쪽**에 그려지는 글리프(잉크 0.3…2.5)가 어느 링크도 아니게 되므로(리뷰 2차; 종전 캐럿
    산식은 줄 끝에서 kern을 빼 [0, 2.78]을 냈다) 진행 폭이 **0 이하**인 run은 **잉크 경계**
    (`CTRunGetImageBounds`)까지 넓힌다 — 단일 글리프 `i`·`.`의 kern은 CT가 폭 0으로 클램프하지만
    잉크(0.65…1.54)는 그대로 그리므로 폭 0도 같은 경우다(리뷰 3차). 잉크 없는 폭 0 run(공백·U+200B·
    폭 0 개체 마커·프레임 안 개행 run — 실측 전부 잉크 empty)은 그대로 버려지고, 렌더러가 글리프를
    안 그리는 run(개체 마커 run delegate·한 줄 끝 표식)은 잉크가 보고돼도 넓히지 않는다
    (`drawsGlyphs`, `glyphOffsetBands`와 같은 술어). 양수 폭 run은 넓히지 않는다(기울임 오버행이
    이웃 링크 상자에 들어 단방향 줄이 종전과 달라진다) (`testNegativeRunWidthKeepsTheVisibleGlyphHittable`). run 범위는 줄 캐시 옆
    ...
    3.81 → 3.28ms, 양방향 40스팬 2.08 → 1.72ms·120스팬 6.95 → 6.27ms, 글자 위치 혼합 120스팬
    3.87 → 3.73ms, 표 셀 스팬 1개 0.029 → 0.023ms; 링크 하나뿐인 89줄 문단은 6.40 → 6.30ms로
    같다 — 조판이 다다). **디버그 빌드로 재면 반대로 20% 느리게 보인다** — 프로브는 `-O`인데
    라이브러리가 `-Onone`이라 비교가 기울어서다. 벤치는 `swift build -c release`의
    `.build/out/Products/Release/*.o`에 링크할 것.
    **한글 12.30 실측** (합성 HWPX → 한글 저장 HWP·PDF, 함초롬바탕 24pt): 한글도 히브리·아랍
    글자를 CT와 같은 시각 순서로 놓고 글리프 x가 0.1pt 안에서 같다(ג 138.2 vs 138.27 — 합성
    HWPX 생성기·한글 저장본·PDF·클릭 도구는 `~/.claude/projects/-Users-sboh-Repos-hwp-swift/probes/201/`,
    픽스처 쌍으로는 안 올렸다: HWPX 매퍼가 `fieldBegin type="HYPERLINK"`를 아직 강등하므로 등가
    스윕이 갈린다). 그러나
    **한글의 PDF 링크 주석은 종전 우리 산식과 같은 결함**이다 — `abc א`(A)+`בג`(B)에서 A 주석이
    `abc ` 폭(85.08…138.24)뿐이고 B·א 주석은 없으며, 아랍 `x مر`(P)+`حبا y`(Q)는 P 주석이 Q의 글자
    위에 서고, 순수 RTL 두 링크는 주석이 하나도 없다. 그래서 링크 영역의 정답은 한글 PDF가 아니라
    **글리프 소속**(run의 링크 속성)이며, 한글 PDF 글리프 중심 25곳 전부가 우리 히트에서 자기 링크로
    열린다. **한글 편집 화면의 클릭은 글자마다 자기 링크를 연다** (2026-09-21 실측, 한글 저장
    HWP를 다시 열어 CGEvent 클릭 → 보안 대화상자 "한 번 허용" → 로컬 HTTP 서버 로그): `abc א`(A)
    +`בג`(B)에서 abc → A, ג → B, ב → B, א → A이고 스팬 하나로 전체를 건 줄은 abc·ג·א 모두 그
    링크다 — 수정 후 우리 판정과 같다. 다만 커서 스윕(`screencapture -C`로 손/I빔 판독, 2pt
    간격)에서 א의 **왼쪽 약 4pt는 I빔**(어느 링크도 아님)이고 그 오른쪽부터 손이다 — 한글
    자체의 작은 사각지대라 따르지 않는다(우리는 א 전체가 A). 한글 12.30 macOS는 링크를 누르면
    "보안 위험으로 인해 확인되지 않은 외부 개체 또는 링크 실행이 기본적으로 차단" 대화상자
    (`window "한컴오피스 한글"`의 `button "한 번 허용"`)가 먼저 뜬다 — 대화상자가 떠 있으면 다음
    클릭이 전부 먹히지 않으니 클릭마다 AX로 확인한다. 클릭은 `CGEventSetIntegerValueField(…,
    kCGMouseEventClickState, 1)`을 실은 CGEvent(venv pyobjc)로, 창은 두 번째 디스플레이라 전역
    좌표(x ≈ 2000+)를 쓴다.
  ...
  - **자격의 세로 여유는 줄 높이 + 최대 |오프셋|이다** (#200 리뷰, `textBounds`): 밴드는
    ...
    **합**이 상위집합이다. 줄 높이만 주면 오프셋이 그보다 큰 글리프 위의 탭이 rect 판정에
    닿기도 전에 블록 단계에서 기각된다 (#200 리뷰 실측, 당시 첨자 상수 0.67배·0.33em 기준:
    Helvetica 10pt·줄 높이 10에서 오프셋 12부터 `hit`이 nil; 6.7pt 글꼴에 13.3이면 밴드 y
    87.2…93.9가 자격 93.3…116.7 밖 — #204 뒤의 같은 조합은 6.4pt 글꼴에 14.4). 파싱 문서에서
    닿는 조합이다 — 첨자(`applySuperscript`, 0.64배 글꼴·+0.44em)는 기존
  ...
  - **프레임 밖 칠해진 글자는 링크가 없어도 그 블록이 claim한다** (R53의 텍스트 축, #200 리뷰
    ...
    각주 참조 번호만 있는 10pt 본문은 위 2.1pt)을 더한다(`verticalInkReach`); **옆**은
...
- 장식 선은 글리프가 아니라 **줄 원점** 기준이다 — `glyphBaselineOffset`(글자 위치·
  첨자 합산)이 글리프만 옮겨도 선은 제자리다. 글자 위치는 한글도 같다(2026-09-09 실측:
  `hh:offset` 50으로 글리프가 5pt 내려가도 세 선 모두 같은 y). **첨자만 예외이고
  선마다 다르다** (#179, 2026-09-15 한글 12.30 PDF 실측 — 함초롬바탕·Apple SD 두 글꼴,
  10·20pt, 위/아래 첨자 × 취소선·가운데·아래·위 밑줄 × 글자 위치 50 동반):
  - 취소선·글자 가운데 밑줄은 **첨자로 옮겨진 베이스라인 + 줄어든 크기 × 0.35**
    (10pt 위 첨자: 4.32 위 + 0.35 × 6.36 = 6.60 위 — 장치 0.12pt 양자화, 첨자 단독 표본은
    4.44 — 아래 첨자: 1.20 아래 + 2.28 = 1.08 위).
    글자 위치 몫은 따라가지 않는다 — 첨자 + 위치 50 run에서도 첨자만 준 자리 그대로다.
  - 글자 아래 밑줄·글자 위 밑줄은 첨자에도 **원래 베이스라인**에, 크기는 **첨자 축소 전
    값**(−0.17 × 10 / +0.87 × 10)으로 그린다 — 밑줄은 줄 단위(#226)라 첨자 run의 기본 크기가
    줄 상자·줄 글자 크기에 드는 것으로 성립한다 (기본 40pt 위 첨자 무장식 run이 든 줄은 40pt
    상자·1.56pt).
  - **선 두께는 네 선 모두 축소 전 크기 × 0.04**다 (첨자 run의 선도 0.36pt = 본문).
  그래서 첨자 이동량은 합산 키와 별도로 `hwp.scriptBaselineOffset`(첨자 몫만, 양수 = 위)
  에도 실린다 (`applySuperscript`·`applySubscript`가 둘 다 누적) — 합산 키에서 두 몫을
  되돌려 가를 수 없어서다. 렌더러는 취소선 경로만 이 키를 더하고, 크기는 선마다 글자 모양
  기본 크기(`hwp.baseFontSize` — 상대 크기·첨자 축소 전, #226)에서 잰다 (종전 `spaceTargetSize`
  는 슬롯 상대 크기를 반영해 기본 40pt·50% run의 선을 20pt 자리에 그렸다). 글리프·기하 질의·서식 복사는
  종전대로 합산 키만 본다(`HwpSelectionRTF`는 `hwp.*` 일괄 제거로 첨자 키를 버리므로
  이중 적용이 없다). 실물 근거는 `script-decorations` 픽스처 쌍 + PDF 좌표 핀
  (`FixtureDecorationLineRenderTests+Script` — 첨자 취소선·가운데 밑줄도 한글 좌표
  117.12·138.72·149.16·170.76·261.12에 0.3pt 안으로 핀, #204에서 첨자 글리프 자체를
  0.64배·0.44em·0.12em으로 맞춘 뒤).
- 키 큰 인라인 개체 줄에서 글자 아래 밑줄이 개체 하단(= 줄 상자 바닥)에 남는 실물(공공누리)은
  별도 보정이 아니라 **줄 단위 밑줄 규칙의 한 경우**다 (#226) — 개체가 줄 상자를 정한 줄의 상자
  바닥이 곧 개체 하단이다. 종전에는 원점을 `0.15 × 개체 높이`만큼 되돌리는 보정
  (`underlineReturnDrop`, 삭제됨)을 두고 그 원점에서 다시 run 크기 × 0.17을 내려 상자 바닥 몫을
  두 번 셌다 (10pt 글자 + 40pt 개체 줄: 한글 −6.12~−6.24pt, 종전 −7.70pt). 개체가 상자를 정한
  줄의 상자는 개체 **바깥 상자**(개체 + 바깥 여백)라 바깥 여백이 있으면 밑줄은 개체 아래로 그
  여백만큼 떨어진다 (위 7·아래 3pt의 20pt 그림: 밑줄 −4.68·위 밑줄 +25.68pt). 글자 위 밑줄은 반대쪽
  가장자리(상자 상단)에 붙는다 — 한글 12.30: 10pt 위 밑줄 + 40pt 그림·표 줄 +34.20pt(종전
  +8.70pt). 옛 보정에서 얻은 교훈 둘은 규칙에 그대로 녹아 있다: 아래쪽 보정을
  위쪽 밑줄에 걸면 글자 아래로 떨어지고(PR #175 리뷰 — 이제 두 선이 각자 상자의 반대
  가장자리를 쓴다), 개체가 글자보다 낮은 줄(10pt 글자 + 4·8·9·9.9pt 개체)은 상자가 글자 크기라
  밑줄이 글자 줄 자리다(PR #197 리뷰 — 상자 높이 `max(기본 크기, 개체 높이)`가 그대로 판정이다).
  가드는 `HwpBaselineAnchorTests.testUnderlineReferenceIsTheAnchorLineBox`·
  `HwpUnderlineReferenceTests`·`HwpDecorationLineGeometryTests+LineWide`.
...
- 쪽 번호 위치 (표 148)의 상/하 밴드 프레임이 없으면 (여백 0) 아래·위 여백의 가운데에
  붙인다 (#193, 위 쪽 번호 방출 항목) — 위로 제책의 제본 여백은 기하가 위 인셋 안으로
  클램프한 값(`HwpPageGeometry.topGutter`)이라 제본 여백이 쪽 높이를 넘는 잘못된
  문서에서도 그 가운데가 쪽 안이다 (PR 리뷰; 머리말 자리의 위 여백·꼬리말 여백은 아직
  안 접는다). 감추기 (표 145 pghd·표 130 구역 정의)의 머리말/꼬리말/쪽 번호
...
- 단 종류 (일반/배분/평행)와 맞쪽 방향은 미세분화 — 밴드가 닫힐 때 항상 배분,
  방향은 왼쪽/오른쪽만; 다단 캐시 run(`placeCachedColumnRuns`)으로 단에 걸친 문단의
  각주는 여전히 마지막 조각의 쪽에 귀속된다 (흐름 분할은 #207부터 조각 단위 — 위
  "페이지/단 경계로 분할된 문단의 컨트롤" 항목; 줄 앵커가 있는 treatAsChar 개체는 세
  경로 모두 조각과 함께 간다, #164). 쪽 나눔 보호 규칙(`HwpParagraphSplitPolicy`)은
  1단 쪽 경계 실측이고 다단의 단 경계에도 같은 규칙을 적용하지만(미실측), 밴드를
  닫으며 첫 단 텍스트를 균등 재배치하는 `rebalanceColumnBand`는 규칙을 보지 않는다.
