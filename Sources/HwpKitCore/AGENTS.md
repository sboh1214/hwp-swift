...
- **흐름 배치 문단의 자리 차지 표는 글줄 앞에 흘린다** (#190, `appendTablesPrecedingText`).
  ...
  **제목 행 반복(표 76 bit 2)의 판정도 쪽마다다** — 진입 때는 빈 단 용량
  ...
  `refitContinuation`이 넘긴 쪽의 예약을 뺀 용량으로 여백 → 제목 행 순으로 다시 재어 안
  들어가는 것만 그 조각에서 뺀다. 진입 쪽의 `effectiveContentHeight`로 표 전체의 반복을 끄면
  앞 문단의 큰 각주가 남은 쪽에서 시작한 표는 첫 조각이 바로 넘어가는데도 뒤 쪽의 제목이
  전부 사라진다(PR #212 리뷰 2차).
