...
- **절대 캐시 모드의 각주는 본문 아래 자리에 한글의 이어짐 규칙으로 싣는다** (#165,
  ...
  마지막 줄의 **상자** 아래가 본문 하단에 닿는다 (줄 간격은 세지 않는다). ② 자리 =
  ...
  진행 보장으로 놓인 미주를 덮는다, 재현: 미주 하단 104.67 위 구분선 92.6pt) − 구분선
  ...
  **이월의 비용은 쪽 수에 비례해야 한다** (PR 리뷰, `HwpFootnoteLayout.SourceLayout`):
  ...
  0.7s·32,000줄 1,599쪽 10.6s (선형). 가드: `HwpFootnoteContinuationSourceLayoutTests`
  (누적표·분할 탐색 동치, 같은 폭에서만 원본 재사용, 높이가 문자열 없이 나옴).
