...
- **MS 워드 호환 문서(호환 문서 대상 프로그램 2, `HwpCompatibleDocumentTarget.msWord`)
  ...
  - **줄 상자·베이스라인·전진량도 같은 글꼴 줄 상자다** (#194, 2026-09-20 한글 12.30 실측:
    ...
    - **글자처럼 취급 개체 줄**: 베이스라인 = max(글꼴 상자 베이스라인, 개체 높이), 상자 =
      베이스라인 + 글꼴 상자의 베이스라인 아래 몫 — 10pt 줄(1559/1104)에 30pt 표를 넣으면
      `vertsize` 3455 = 3000 + 455·`baseline` 3000(개체 바닥이 베이스라인), 셀 내용으로
      18.41pt가 된 표는 2296·1841. 비율 여분은 한글 문서처럼 글자 상자 기준(932)이고
      고정·여백만·최소는 그 상자에 적용한다 (3455 + 300, max(3455, 1600)). 밑줄 되돌림
      (`underlineReturnDrop`)은 호환 문서에서 걸리지 않는다 — 베이스라인이 곧 개체 바닥이다.
    ...
    - **남은 격차**: ① 한글은 글꼴 상자를 글꼴마다 ±0.1% 다르게 반올림한다 — Apple SD
      ...
      키운다 (Apple SD 10pt 160%: 2496 vs 2491). ② 마지막 run이 글자보다 큰 글자 모양의
      표 마커인 줄(10pt 글자 + 30pt 글자 모양 마커, 셀 10.62pt)은 한글이 `vertsize` 4999·
