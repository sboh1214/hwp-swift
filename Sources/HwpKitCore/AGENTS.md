...
- **절대 캐시 모드의 각주는 본문 아래 자리에 한글의 이어짐 규칙으로 싣는다** (#165,
  ...
  일련번호)다: 쪽마다 번호를 새로 시작하면 (표 134 모드 2) 이월된 각주와 그 쪽의 새
  ...
  비례 환산 결과가 곧 그 위치라 값이 같다. 경계가 줄 중간에 떨어지면 그 줄의 앞부분을
  ...
  **개체를 담은 각주는 나누지 않는다** (`NoteMeasurement.carriesObjects`): 개체는 앵커
  ...
  겹친다 (실측: 블록 44.16pt에 개체 하단 79.35pt). 여러 문단짜리 각주의 뒤 문단이 0에서 다시 시작하면 그
