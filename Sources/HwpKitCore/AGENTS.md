...
- **흐름 배치 문단의 자리 차지 표는 글줄 앞에 흘린다** (#190, `appendTablesPrecedingText`).
  ...
  쪽의 쪽 번호·각주 카운터로 다시 재어진다. **그 재시도는 새 번호 지정(`applyNewNumbers`)도
  되풀이하지 않는다**(`isRetryingParagraphWithPlacedTables`) — 첫 시도에서 이미 적용됐고 표
  셀 각주가 그 번호를 썼으므로 다시 적용하면 본문 각주가 같은 번호를 받는다(PR 리뷰: 새 번호
  7 + 셀 각주 + 본문 각주 → 7)·7)). 표를 놓지 않은 문단의 재시도는 종전대로 되풀이한다(아직
  아무 번호도 소비되지 않아 무해하고, 쪽마다 새로 시작하는 문서는 `cacheCurrentPage`의 리셋
  뒤 다시 적용돼야 한다). **표보다 앞선 쪽 장식(머리말·꼬리말·쪽 번호 위치·감추기)은 표를
  ...
  표 뒤의 장식은 종전대로 글줄 뒤 방출이 등록한다. **여백이 빈 쪽마저 소진하면 여백을
  버린다**(`fittingTableMargins`: 첫 행 + 위·아래 여백 > 본문 높이) — 제목 행 반복을 끄는
  #13과 같은 폴백으로, 그대로 두면 가용 높이가 음수가 돼 splitter가 0 높이 조각을 상한까지
  낸다(PR 리뷰: 본문 200.8pt·행 30pt·여백 110 + 110pt → 4,098쪽).
