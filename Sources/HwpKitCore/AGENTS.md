...
- **입력 쪽 일도 유계다.** 형식 분해는 문단마다 하되 **출력 천장까지만** 한다
  ...
  — 천장까지만 분해하면 일도 저장도 형식 길이와 무관하다. 순회는
  `HwpPaginator.init`의 동기 경로라 조판의 쪽 단위 지연·문단 단위 취소 관찰
  밖이다 — 그래서 걷는 문단 수에 `maximumVisitedParagraphs`(500,000 — 항목 상한은
  번호 문단에서만 줄어들어 번호 없는 문단 수백만 개는 막지 못한다)를 두고,
  256문단마다 `Task.isCancelled`를 봐 취소된 로드는 걷다 만다(둘 다
