...
- **자리 차지 표는 문단 글줄 앞 띠가 우선이다** (#161, `appendBandedTableIfNeeded`).
  ...
  **위**에 두고 문단 줄을 표 높이 + 위·아래 바깥 여백만큼 내리는데, 그 결정이 줄
  캐시에 남는다 — **절대 캐시 모드에서** 문단 진입 시점의 흐름 위치와
  `paragraphAnchorTop` 사이가 표 높이 + 여백 이상이면 그 띠가 표 자리다. 기준점은
  `paragraphAnchorTop`이 **아니다** (절대 모드에선 캐시 y라 이미 띠 뒤 자리다) —
  `processParagraph`가 진입 시점 흐름 위치를 `paragraphEntryFlow`에 잡아 둔다.
