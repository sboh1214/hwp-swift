...
`so222c`·`so222d`, 줄 캐시 없는 합성 HWPX를 한글이 다시 저장한 `linesegarray`와 PDF, 우리 재조판과
사례 22개 전부 일치): 남은 17.62에 16pt·160% 한 줄(상자 16, 전진량 25.6)은 남고 18pt는 넘어간다;
...
- **절대 캐시 모드의 각주는 본문 아래 자리에 한글의 이어짐 규칙으로 싣는다** (#165,
  ...
  예약 `measuredFootnoteHeight(isNoteEnd:)`도 같은 산식). 줄 캐시가 **없는** 각주(흐름 조판
  문서·캐시를 버린 HWPX)도 같다 — CT 프레임의 마지막 줄 상자 아래 몫
  (`HwpParagraphMeasurer.Result.trailingGap`)을 뺀다(#222, 한글 12.30 실측 `probes/222` FN: 10pt·
  160% 각주 한 줄의 상자 바닥이 본문 하단에 닿고 영역은 8.5 + 5.67 + 10 = 24.17). 종전에는 0을 빼
  영역이 6pt 커졌고 구분선이 그만큼 위였다. ④ 통째로 안 들어가는 각주는
