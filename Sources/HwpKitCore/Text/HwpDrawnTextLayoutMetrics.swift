...
extension HwpDrawnTextLayout {
    ...
    struct LineMetrics {
        ...
        /// 호환 문서는 글꼴 줄 상자 높이와 `msWordSpacingBase` 가운데 큰 것 (한글 12.30 실측
        /// 2026-09-20: Apple SD 산돌고딕 Neo 10pt 줄 `vertsize` 1559에 160%의 `spacing` 932,
        /// 30pt 표가 든 같은 줄도 932).
        var textBoxHeight: CGFloat {
            guard let msWordTextBox else { return max(baseFontSize, objectMarkerBaseFontSize) }
