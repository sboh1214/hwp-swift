...
public actor HwpPaginator {
    ...
    private struct ParagraphEntryFlow {
        ...
        /// 직전에 배치한 문단의 저작 아래 간격 (pt) — 띠에서 뺀다. 절대 캐시 배치는 이
        /// 몫을 커서로 소비하지 않고 캐시 줄 위치에 이미 담아 두며, 절대 캐시 블록 높이
        /// (`absoluteRunBlockHeight`)는 줄 위치·높이·줄 간격만으로 만들어져 이 몫을 담지
        /// 않으므로, 그 여백이 `entry.top`과 캐시 줄 사이 간격에 남는다 (한글 12.30 실측:
        /// 앞 문단 아래 간격 20pt는 표 **앞**에 그대로 들어간다). 이 문단 자신의 위 간격은
        /// 빼지 않는다 — 한글은 자리 차지 표를 품은 문단의 위 간격을 표 앞에도 표와 글줄
        /// 사이에도 쓰지 않는다 (#190 실측, 1단·2단 모두).
    ...
    /// 본문 텍스트 블록의 마지막 줄 **상자 아래** 몫 (`currentBlocks` 인덱스별) — 각주
    /// 이어짐 판정의 본문 하한은 마지막 줄 상자 아래다 (#165 한글 실측). 블록 프레임은
    /// 전진량(줄 간격, 문단 끝이면 아래 간격까지)이라 그 차를 따로 둔다. 절대 캐시 run 블록
    /// (#165)과 흐름 배치 블록(#222 — 쪽 끝 적합이 줄 상자까지라 여분이 본문 하단을 넘을 수
    /// 있다)이 기록한다. 쪽이 확정되면 비운다.
    private var trailingSpacesBelowLineBox: [Int: CGFloat] = [:]
    ...
    /// 이번 문단에서 글줄 **앞**에 흐름으로 놓은 자리 차지 표의 서수 → 첫 조각의 1-기반 쪽
    /// (#190). 문단 단위 방출(`appendControlBlocks`)이 이 서수를 건너뛰어야 같은 표가 글줄
    /// 뒤에 한 번 더 놓이지 않는다. 열쇠는 `tablesPlacedBeforeTextParagraph`가 가리키는
    /// 문단의 것이다 — 글줄이 쪽에 안 들어가 문단을 다음 쪽에서 다시 처리해도 표는 이미
    /// 놓였으므로 그 문단이 끝날 때까지 남긴다.
    private var tablesPlacedBeforeText: [Int: Int] = [:]
    /// `tablesPlacedBeforeText`가 속한 문단 (구역·문단 서수). 다른 문단에 들어서면 비운다.
    private var tablesPlacedBeforeTextParagraph: HwpParagraphPath?
    /// 글줄 앞 표(#190)보다 **앞선** 쪽 장식 컨트롤(머리말·꼬리말·쪽 번호 위치·감추기)의
    /// 서수 — 표를 놓기 전에 등록해 표가 넘긴 쪽부터 장식이 실린다 (종전 순서: 글줄 뒤 방출이
    ...
    private var controlsPlacedBeforeParagraphEnd: [Int: Int] {
        inlineControlsPlacedPerFragment.merging(tablesPlacedBeforeText) { fragment, _ in fragment }
    }
    ...
    /// — 저장은 band
    private var bandTextBlocks: [HwpColumnBandController.BandTextBlock] {
...
private extension HwpPaginator {
    ...
    func emitColumnDividers() {
        ...
        currentBlocks += band.columnDividerBlocks(currentBlocks: currentBlocks) { offset, block in
    ...
    func rebalanceColumnBand() {
        ...
        {
            if let spacing = trailingSpacesBelowLineBox[offset] {
    ...
    func processParagraph(
        ...
        // 각주가 번호를 먼저 가져가므로 본문 각주 참조 미리보기도 그 뒤 카운터로 구워야
        // 참조와 각주가 같은 번호를 쓴다 (PR 리뷰).
        appendTablesPrecedingText(from: paragraph)
        ...
        appendControlBlocks(
            ...
            skipping: controlsPlacedBeforeParagraphEnd
        )
    ...
    func placeParagraphText(
        ...
        return if let runs = absoluteCacheRuns(for: paragraph) {
    ...
    /// 이 문단이 절대 캐시 경로(`placeAbsoluteCachedParagraph`)로 배치될 때 그 run들 —
    /// 아니면 nil (흐름 배치). 절대 캐시 모드의 **1단** 밴드에서 줄 캐시가 있는 문단만이다;
    /// 다단 밴드는 절대 캐시 모드라도 흐름 배치(`appendParagraphAcrossColumns`)를 탄다.
    /// 배치(`placeParagraphText`)와 글줄 앞 표 판정(`appendTablesPrecedingText`)이 **같은
    /// 술어**를 봐야 표가 띠와 흐름 어느 쪽으로 갈지 갈리지 않는다 (#190).
    private func absoluteCacheRuns(
        for paragraph: CoreHwp.HwpParagraph
    ) -> [[CoreHwp.HwpParaLineSegInternal]]? {
        guard absoluteCacheMode, columnFrames.count <= 1 else { return nil }
        return HwpAbsoluteCachePlacer.cacheRuns(for: paragraph)
    }
    ...
    private func placeFlowParagraph(
        ...
        // 글줄 앞에 자리 차지 표를 놓은 문단은 저작 문단 위 간격을 **쓰지 않는다** (#190,
        // 한글 12.30 실측: 위 간격 20pt를 준 문단의 표는 표 앞에도, 표와 글줄 사이에도
        // 그 간격 없이 놓인다 — 같은 간격을 준 보통 문단은 20pt 내려간다). 문단 높이와
        // 측정 프레임에서 그 몫을 함께 걷어 높이 출처 판정(`heightIsMeasured`)이 그대로다.
        let suppressedGap = tablesPlacedBeforeText.isEmpty ? 0 : authoredBeforeGap(of: paragraph)
        let paragraphFrame = suppressedGap > 0
            ? HwpParagraphFrame(
                totalHeight: max(1, paragraphFrame.totalHeight - suppressedGap),
                lines: paragraphFrame.lines
            )
            : paragraphFrame
        var paragraphHeight = suppressedGap > 0
        ...
        let split = FlowSplitInput(
            ...
            fitLines: paragraphFrame.lines.isEmpty
                ? emptyParagraphLines(paragraph, attributedString: attributedString)
                : paragraphFrame.lines,
            cachedLineBoxBottom: cacheHeightUsed ? cachedLineBoxExtent(paragraph) : nil
        )
        if columnFrames.count > 1 {
            placeMultiColumnParagraph(split)
            return true
        }
        // 쪽 끝 적합은 문단 높이(전진량 합 + 아래 간격)가 아니라 마지막 줄 **상자 하단**까지로
        // 판정한다 (#222 — 한글 12.30 실측: 남은 17.62pt에 16pt·160% 한 줄 문단을 남긴다). 줄
        // 간격 여분과 아래 간격은 쪽 아래로 넘쳐도 되고, 그 몫은 다음 쪽으로 이월하지 않는다
        // (다음 문단이 새 쪽 상단에서 시작한다). 높이와 같은 출처로 잰다
        // (`paragraphFitHeight`).
        let fitHeight = paragraphFitHeight(split)
        if contentHeightUsed > 0,
           !HwpPageEndFit.fits(
               contentHeightUsed + fitHeight, in: effectiveContentHeight - anticipatedFootnotes
           )
        {
            ...
            if canSplitAtEntry(split, cacheHeightUsed: cacheHeightUsed),
        ...
        recordFlowTrailingSpace(
            blockHeight: paragraphHeight - beforeGap, fitHeight: fitHeight - beforeGap
        )
    ...
    private struct FlowSplitInput {
        ...
        /// 쪽 끝 적합(#222)에 쓸 줄 프레임 — `paragraphFrame.lines`이고, 빈 문단은 그것이 비어
        /// 있어 한 번 더 조판한 앵커 줄이다 (`emptyParagraphLines`).
        let fitLines: [HwpLineFrame]
        /// 문단 높이를 저장본 줄 캐시로 잡았을 때 그 캐시의 줄 상자 바닥 (`cachedLineBoxExtent`) —
        /// 아니면 nil.
    ...
    ///
    /// 문단 단위로 마지막 조각 뒤에 나오는 것이 문단 머리에 묶여 있으면 나누지 않는다 — 자리
    /// 차지·글 앞뒤 개체는 `paragraphAnchorTop`(문단 상단) 기준인데 뒤 조각 쪽에서 방출되면 그
    /// 쪽 머리를 기준으로 그려지고, 자동 쪽 번호 필드는 조판 문자열에 진입 쪽 번호로 구워져
    ...
    ///
    /// 다단 밴드도 만들지 않는다 (PR 리뷰): 각주 영역은 **쪽 폭** 하단이라 뒤 단 조각이 걷은 각주가
    /// 이미 끝까지 찬 앞 단의 아랫줄을 덮는다 — 밴드의 모든 단이 문단 전체 예약을 미리 빼는
    /// 종전 회계를 유지한다.
    private func flowFragmentFootnotes(
        for paragraph: CoreHwp.HwpParagraph,
        attributedString: NSAttributedString
    ) -> HwpFlowFragmentFootnotes? {
        guard columnFrames.count <= 1,
              currentSectionDef?.footNoteShape.numberingModeRawValue != 2
        else { return nil }
    ...
    /// 문단 전체를 한 번에 남길 때의 쪽 끝 적합 높이 (#222) — 문단 위 간격 + 첫 줄 상자 상단부터
    /// 가장 아래 줄 **상자 하단**까지. 줄 간격 여분과 문단 아래 간격은 넣지 않는다 (쪽 아래로
    /// 넘쳐도 된다). 문단 높이(`paragraphHeight`)와 **같은 출처**로 잰다 (#166의 높이 출처 원칙):
    /// 저장본 줄 캐시 높이를 쓴 문단의 마지막 줄은 캐시의 줄 상자 바닥(`vertsize`)으로, 그 밖의
    /// 줄과 측정 높이 문단은 CT 줄 프레임(`HwpFragmentLineAdvances.fitHeight`)으로. 줄이 없으면
    /// 캐시 상자 바닥, 그것도 없으면 문단 높이 전체다.
    private func paragraphFitHeight(_ split: FlowSplitInput) -> CGFloat {
        ...
        // 판정과 루프가 모든 줄을 남긴다는 판정이 같아야 한다. 줄 수가 같으면 캐시 높이 문단도 앞
        // 줄들은 CT 상자로 잰다: 렌더러는 캐시 높이 블록 안에서도 줄을 CT 전진량으로 그리므로
        // (`HwpDrawnTextLayout.lines`), CT 줄 상자가 캐시보다 큰 문단(MS 워드 호환 문서의 글꼴 대체,
        // 글자보다 낮은 낡은 캐시)을 캐시 바닥만으로 통째로 두면 그 뒤 줄이 본문 아래·쪽 밖에
        // 그려진다 (#222 PR 리뷰 — 캐시 바닥만 쓰자는 제안을 이 이유로 두지 않았다). 고정 줄 간격이 상자보다 작은 줄은
        // 상자가 전진량보다 커서 이 값이 문단 높이를 넘을 수 있다 — 한글도 그 상자로 판정한다
        // (남은 9pt에 상자 10·전진량 8인 줄을 넘긴다).
        let advances = HwpFragmentLineAdvances(
            lines: lines, textHeight: split.paragraphHeight - split.beforeGap,
            cachedLastLineBoxBottom: split.cachedLineBoxBottom
        )
        return split.beforeGap + advances.fitHeight(from: 0, through: lines.count - 1)
    }

    /// 빈 문단의 줄 프레임 — 페이지네이터의 `layout`은 빈 문단 앵커(#145)의 줄 프레임을 비우므로
    /// (높이만 남긴다) 쪽 끝 적합에 쓸 줄 상자를 한 번 더 조판해 얻는다 (#222). 빈 줄도 한글은
    /// 상자로 판정한다 — 쪽 끝의 빈 문단은 흔하다. 앵커가 아니면(줄이 정말 없는 문단) 빈 배열.
    private func emptyParagraphLines(
        _ paragraph: CoreHwp.HwpParagraph, attributedString: NSAttributedString
    ) -> [HwpLineFrame] {
        guard HwpTextRunBuilder.isEmptyParagraphAnchor(attributedString) else { return [] }
    ...
    /// 방금 놓은 흐름 텍스트 블록의 마지막 줄 상자 아래 몫을 기록한다 (#222) — 블록 높이
    /// (전진량 합)와 그 블록 줄들의 적합 높이(상자 하단)의 차. 쪽 끝 적합이 줄 상자까지라
    ...
    private func recordAbsoluteRunTrailingSpacing(
        ...
        guard !isStaleAdjusted else {
            trailingSpacesBelowLineBox[currentBlocks.count - 1] = 0
            return
        }
        ...
        trailingSpacesBelowLineBox[currentBlocks.count - 1] = max(0, blockHeight - inkHeight)
    }
    ...
    private func footnoteBodyBottom() -> CGFloat? {
        currentBlocks.enumerated().compactMap { offset, block -> CGFloat? in
            ...
            return HwpHitTester.paintedObjectBounds(of: block).maxY
                - (trailingSpacesBelowLineBox[offset] ?? 0)
        }.max()
    }
    ...
    /// `paraShape`는 조각의 줄 앵커를 목적 단 폭으로 다시 조판할 때, 그리고 측정 높이 문단의
    ...
    func appendParagraphAcrossColumns(
        ...
        // 넘어가면 그 단 폭으로 다시 잰다 (`HwpFragmentRemainder`, PR 리뷰).
        ...
        while !remainder.isExhausted {
            ...
            let available = max(1, effectiveContentHeight - reservedFootnoteHeight)
                - contentHeightUsed
            let columnIsEmpty = contentHeightUsed <= 0 || columnHoldsOnlyThisParagraph
            ...
            // 분할 규칙이 빈 단에서 0줄을 내는 경우(두 줄도 안 들어가는 단의 외톨이줄 보호)도
            // 같은 진행 보장이다.
            let onEmptyStartColumn = columnHoldsOnlyThisParagraph && isAtParagraphStart
            if columnIsEmpty, takeCount == 0 {
            ...
            appendLineSliceBlock(
                remainder.lines[remainder.lineIndex ..< remainder.lineIndex + takeCount],
                of: remainder,
    ...
    /// 한 줄 문단(또는 텍스트 몫 0)의 배치 — 나눌 줄이 없으니 들어가면 놓고 아니면 통째로 다음
    /// 단으로 옮긴다. 빈 단에서 gap+text가 안 맞는 초과 문단이면 gap을 무르고 현재 단 top에
    ...
    private func placeSingleLineParagraph(
        ...
        // 적합은 줄 상자 하단까지로 잰다 (#222) — 줄 간격 여분·아래 간격은 단 아래로 넘쳐도 된다.
        let fitHeight = remainder.remainingFitHeight
        let usableHeight = max(1, effectiveContentHeight - placement.reservedFootnoteHeight)
        ...
        if columnHoldsOnlyThisParagraph {
            if !HwpPageEndFit.fits(
                placement.beforeGap + fitHeight + noteReservation, in: usableHeight
            ) {
                contentHeightUsed = 0
                paragraphAnchorTop = currentColumnFrame.minY
            }
        } else if !HwpPageEndFit.fits(
            contentHeightUsed + fitHeight + noteReservation, in: usableHeight
        ) {
        ...
        appendBlock(
            ...
            attributedString: placedFragment(attributedString, reservedWidth: placement.reservedWidth),
            hyperlinkURL: placement.hyperlinkURL,
        ...
        recordFlowTrailingSpace(
            blockHeight: remainder.advances.textHeight, fitHeight: remainder.remainingFitHeight
        )
        collectFlowFragmentFootnotes(
            through: max(0, remainder.lines.count - 1), of: remainder, placement.fragmentFootnotes
        )
        return true
    }
    ...
    private func advancePastUnplacedFragment(
        ...
        if isAtParagraphStart,
           HwpPageEndFit.fits(
               placement.beforeGap + remainder.firstLineFitHeight, in: usableAfterAdvance
           )
        {
    ...
    private func moveWholeParagraphToNextColumn(
        ...
        if HwpPageEndFit.fits(
            placement.beforeGap + remainder.firstLineFitHeight,
            in: max(1, effectiveContentHeight - placement.reservedFootnoteHeight)
        ) {
            contentHeightUsed += placement.beforeGap
        }
        paragraphAnchorTop = currentColumnFrame.minY + contentHeightUsed
    ...
    /// `HwpFragmentRemainder`). 측정 줄 조각 표식은 접힘만 막아, 넓은 단에서는 CT가 줄을
    /// 합쳐 잰 높이의 상자 아래가 비고 좁은 단에서는 줄이 늘어 상자를 넘쳤다 — 한글처럼
    /// 옮겨진 단 폭으로 다시 줄바꿈해 조각 높이·줄 수·앵커를 그 줄바꿈으로 잡는다. 저장본
    /// 줄 캐시 높이를 따르는 문단은 다시 재지 않는다 (그 높이는 한글의 것이다).
    ///
    /// 다시 재는 문자열은 조각을 그릴 문자열과 같아야 한다 — 이어지는 조각의 문단 스타일
    /// (`continuationFragment`)과 목적 단으로 다시 푼 개체 예약(`placedFragment`)까지. 문단
    /// 머리가 아닌 나머지는 표식을 단 채로 재어 문단 단위 한 줄 넘침 규칙을 건너뛴다
    /// (`markedAsMeasuredLineFragment` — 조각을 접으면 한글의 줄바꿈과 갈린다); 문단 머리부터
    /// 통째로 옮긴 나머지는 문단이라 그 규칙을 따른다. 텍스트 몫은 `layout`의 높이에서 문단 위
    /// 간격(첫 조각 앞에서 커서로 이미 소비)을 뺀 것이다.
    private func remeasureRemainderIfNeeded(
        _ remainder: inout HwpFragmentRemainder,
        attributedString: NSAttributedString,
        placement: HwpFragmentPlacement
    ) {
        guard remainder.heightIsMeasured, remainder.canRemeasure,
              currentColumnFrame.width != remainder.measuredWidth,
              let range = remainder.remainingRange(in: attributedString), range.length > 0
        else { return }
        ...
        // 문단 지표(마지막 줄 높이·줄 뒤 간격)는 문단 전체 문자열로 — 조각만 보면 큰 글자가
        // 앞 조각에만 있는 문단의 나머지 마지막 줄이 짧아진다 (PR 리뷰).
        let frame = HwpParagraphLayout().layout(
            attributedString: range.location > 0
                ? HwpParagraphLayout.markedAsMeasuredLineFragment(fragment) : fragment,
            paraShape: placement.paraShape,
            columnWidth: currentColumnFrame.width,
            maxLineFrames: remainder.lineBudget,
            metricsReference: attributedString
        )
        guard !frame.lines.isEmpty else { return }
        // 문단 위 간격은 첫 조각 앞에서 커서로 소비했다 — `authoredBeforeGap`과 같이 0 하한.
        let spacingBefore = max(0, HwpParagraphLayout.ParagraphMetrics(
            paraShape: placement.paraShape, attributedString: attributedString
        ).paragraphSpacingBefore)
        remainder.replace(
            with: frame, textHeight: max(1, frame.totalHeight - spacingBefore),
            width: currentColumnFrame.width, range: range
        )
    }

    /// `appendParagraphAcrossColumns`가 조각마다 같은 값으로 쓰는 문단 문맥 — 개체 예약이
    /// 묶인 단 폭은 진입 시의 현재 단 폭이고, 높이 출처는 문단 높이가 측정값
    /// (`paragraphFrame.totalHeight`)과 같은지로 가른다 (아니면 저장본 줄 캐시라 마지막 줄의
    /// 전진량은 캐시 잔여다, #166).
    ...
    /// 단 폭이 줄을 잰 폭(`remainder.measuredWidth`)과 다르면 앵커 문맥은 목적 단 폭으로
    /// 다시 조판한 줄이다 — 측정 높이 문단은 나머지를 목적 단 폭으로 다시 재므로
    ...
    /// 비등폭 단 이월) 렌더러가 이 단 폭으로 다시 줄바꿈하므로 그 판정은 폭 허용 오차 없이
    /// 실제 줄바꿈으로 한다. 앵커 문맥(`fragmentAnchorLines`)과 개체 예약(`placedFragment`)의
    /// "같은 폭"도 같은 정확 비교다 (PR 리뷰) — 표식이 붙어 렌더러가 이 단 폭의 줄바꿈으로
    /// 그리는 조각의 앵커를 잰 폭의 줄로 잡으면 개체가 다른 줄·다른 x에 놓인다.
    private func appendLineSliceBlock(
        ...
        appendBlock(
            ...
            paragraphId: placement.paragraphId,
            lines: isWholeParagraph ? remainder.lines : [],
            heightIsMeasured: heightIsMeasured,
            paraShape: placement.paraShape,
        ...
        recordFlowTrailingSpace(
            blockHeight: height,
            fitHeight: advances.fitHeight(from: slice.startIndex, through: slice.endIndex - 1)
        )
    }
    ...
    func collectUnsupported(from paragraph: CoreHwp.HwpParagraph, firstPage: Int) {
        ...
        walkUnsupported(
            ...
            pages: UnsupportedPageContext(
                ...
                placedPages: controlsPlacedBeforeParagraphEnd
            ),
            numbering: currentParagraphScope
        )
    }
    ...
    func appendControlBlocks(
        ...
        for (ctrlIndex, ctrl) in ctrls.enumerated() {
            // 글줄 앞 표보다 앞선 쪽 장식은 표를 놓기 전에 등록됐다 (#190) — 되풀이하면
            // 쪽 감추기 마스크가 두 번 쌓인다.
            if depth == 0, chromeRegisteredBeforeText.contains(ctrlIndex) {
                continue
            }
    ...
    /// 자리 차지(위·아래 배치) 표를 문단 글줄 **앞**에 흐름으로 놓는다 (#190).
    ///
    /// 한컴오피스 한글 12.30은 세로 기준이 '문단'인 자리 차지 표를 그 문단의 글줄 위에
    /// 두고 글줄을 표 아래로 내린다 — 표가 단·쪽을 넘으면 뒤 조각이 다음 단 첫머리에
    /// 서고 글줄은 마지막 조각 아래를 따른다 (`line-shapes` 2단 실측: 1단 7행 / 2단
    /// 10행 뒤 `table anchor` 줄 = 캐시 `vertpos` 13386 = 위 여백 283 + 10행 12820 +
    /// 아래 여백 283). 절대 캐시 배치 문단은 캐시 간격이 곧 그 자리라 띠 판정
    /// (`appendBandedTableIfNeeded`, #161)이 맡고, 여기는 **흐름 배치** 문단만 본다 —
    /// 다단 밴드(절대 캐시 모드라도 흐름 배치)와 캐시 없는 문단이 그것이다. 두 경로의
    /// 갈림은 `absoluteCacheRuns(for:)` 하나가 소유한다.
    ///
    /// 표는 조각마다 위 바깥 여백 뒤에서 시작하고 아래 바깥 여백을 남긴다 (한글 실측 —
    /// 이어지는 단의 조각도 단 상단에서 위 여백만큼 내려 시작한다). 놓은 서수는
    /// `tablesPlacedBeforeText`에 남겨 문단 단위 방출이 건너뛰고, 그 뒤 표는 띠로
    /// 되감지 않는다 (`bandClosed`). 레이아웃에 실패한 표(자리표시자)는 종전대로 글줄
    /// 뒤 방출에 맡긴다. 컨테이너 안 문단은 이 경로를 지나지 않는다 — `processParagraph`
    /// 가 최상위 문단마다 한 번 부른다.
    private func appendTablesPrecedingText(from paragraph: CoreHwp.HwpParagraph) {
        let numbering = currentParagraphScope
        // 글줄이 쪽에 안 들어가 같은 문단을 다음 쪽에서 다시 처리하는 중이면 표는 이미
        // 놓였다 — 서수 기록은 그대로 두고 건너뛴다 (다시 놓으면 표가 두 번 그려진다).
        if tablesPlacedBeforeTextParagraph == numbering.path {
            paragraphEntryFlow?.bandClosed = !tablesPlacedBeforeText.isEmpty
            return
        }
    ...
    /// 이번 `processParagraph` 호출이 글줄 앞 표(#190)를 이미 놓은 같은 문단의 재시도인지 —
    /// 글줄이 쪽에 안 들어가 `placeParagraphText`가 false를 돌려 다음 쪽에서 다시 처리하는
    /// 경우다. 표를 놓지 않은 문단의 재시도는 종전대로 (새 번호 지정 반복이 무해하다 —
    /// 아직 아무 번호도 소비되지 않았다).
    private var isRetryingParagraphWithPlacedTables: Bool {
        !tablesPlacedBeforeText.isEmpty
            && tablesPlacedBeforeTextParagraph == currentParagraphScope.path
    }

    /// `ordinal`보다 앞선 쪽 장식 컨트롤 가운데 아직 등록하지 않은 것을 서수 순으로 등록한다
    /// (#190). 등록한 서수는 `chromeRegisteredBeforeText`에 남아 글줄 뒤 방출이 건너뛴다.
    private func registerPageChromePreceding(
        ...
        for index in start ..< ordinal {
            ...
            case .header, .footer, .pageNumberPosition, .pageHide:
                ...
                chromeRegisteredBeforeText.insert(index)
            default:
                continue
            }
        }
    ...
    /// 문단 글줄 **앞**에 놓이는 자리 차지 표인지 — 글자처럼 취급이 아니고, 배치 방식이
    /// 자리 차지(위·아래)이며, 세로 기준이 '문단'이고 세로 오프셋이 0인 표 (#161·#190).
    /// 실측한 것은 이 조합뿐이다 — 어울림(`square`)은 한글이 표 **옆**으로 글을 흘리고
    /// (`consumesFlow`는 둘과 미지 값(nil)을 함께 담으므로 그것으로 게이트하면 안 된다),
    /// 오프셋이 있으면 저작이 자리를 직접 지정한 것이다. 바깥 여백 넷이 있어야 위·아래
    /// 몫을 읽을 수 있다. 띠 판정(`floatingTableBand`)과 흐름 배치
    /// (`appendTablesPrecedingText`)가 **같은 술어**를 본다.
    static func precedesParagraphText(_ property: CoreHwp.HwpCommonCtrlProperty) -> Bool {
    ...
    func beginParagraphEntryFlow(for _: CoreHwp.HwpParagraph) {
    ...
    /// 앞에서 커서로 소비하고(`placeFlowParagraph` — 글줄 앞에 자리 차지 표를 놓은 문단은
    /// 쓰지 않는다, #190), 절대 캐시 배치는 한글이 준 줄 위치에 이미 담겨 있다.
    ...
    /// 흐름으로 흘리는 표 조각의 위·아래 바깥 여백 (pt, 표 70 `marginArray`의 위·아래 몫).
    /// 조각마다 위 여백 뒤에서 시작하고 아래 여백을 남긴다 — 이어지는 단·쪽의 조각도
    /// 단 상단에서 위 여백만큼 내려 시작하며, 아래 여백은 적합 판정에 든다 (한글 12.30
    /// 실측 #190: 아래 여백 6pt에서 7행째가 남은 공간 5.05pt에 들어가지 않아 다음 단으로
    /// 갔다). 글줄 뒤 흐름 배치(종전 경로)는 여백을 실측하지 않아 `none`으로 부른다.
    struct TableFlowMargins {
        let top: CGFloat
        let bottom: CGFloat

        static let none = TableFlowMargins(top: 0, bottom: 0)
        ...
        /// 표 70 바깥 여백 (왼쪽·오른쪽·위쪽·아래쪽) — 넷이 아니면 여백 없음.
        init(outerMargins: [CoreHwp.HWPUNIT16]) {
            guard outerMargins.count == 4 else {
                self = .none
                return
            }
            top = max(0, HwpUnits.points(fromHwpUnit16: outerMargins[2]))
            bottom = max(0, HwpUnits.points(fromHwpUnit16: outerMargins[3]))
        }
    ...
    private func floatingTableBand(
        ...
        // 대상 표의 술어는 `precedesParagraphText` 하나다 (#190과 공유).
        guard Self.precedesParagraphText(property),
        ...
        let margins = TableFlowMargins(outerMargins: property.marginArray)
        ...
        // 띠는 **앞 문단의 아래 간격** 뒤에서 시작한다 — 그 여백은 앞 문단이 의도한 것이고
        // 캐시 줄 위치에 이미 들어 있다. 이 문단의 위 간격은 빼지 않는다 (한글은 자리 차지
        // 표를 품은 문단의 위 간격을 쓰지 않는다 — #190 실측; 빼면 그런 문단의 띠가 좁다고
        // 오판돼 표가 글줄 뒤로 밀린다). 같은 문단의 앞 표가 쓴 자리가 있으면 그 아래부터다.
        let bandTop = entry.bandUsed.map { currentColumnFrame.minY + $0 }
            ?? (entry.top + entry.precedingSpacingBottom)
        let gap = paragraphAnchorTop - bandTop
        guard gap + Self.floatingBandTolerance >= height + margins.total else {
            return nil
        }
        return FloatingTableBand(
            contentTop: bandTop - currentColumnFrame.minY + margins.top,
    ...
    func appendTableSegments(
        ...
        let (repeatedRows, repeatedHeight) = repeatedHeaderRows(
            of: frame, headerRowCount: headerRowCount, margins: margins
        )
              ...
              !Task.isCancelled
        {
            ...
            if isFirstSegment {
                firstSegmentPage = cachedPages.count + 1
            }
            appendTableSegmentBlock(
                rows: segmentRows,
                original: frame,
        ...
        return firstSegmentPage
    }

    /// 쪽 경계 나눔이 없는 표(표 76 bits 0-1 == 0)를 통째로 놓는다 — 남은 공간에 여백까지
    /// 안 들어가면 새 쪽으로 넘긴다. 반환값은 놓인 1-기반 쪽. `cellsByRow`가 있으면 셀
    /// 각주를 이 쪽에 담는다 (표 모델이 없는 호출은 nil).
    private func appendWholeTable(
        _ frame: HwpTableFrame,
        instanceId: UInt32,
    ...
    /// 시작하고, 밴드 하단도 그만큼 내려간다. 여백이 0이면 무동작이다.
    private func consumeTableBottomMargin(_ margins: TableFlowMargins) {
        guard margins.bottom > 0 else { return }
        contentHeightUsed += margins.bottom
        markBandUsage()
    }
    ...
    private func repeatedHeaderRows(
        ...
        headerRowCount: Int,
        margins: TableFlowMargins
    ) -> (rows: [HwpTableRowFrame], height: CGFloat) {
        let candidateRows = headerRowCount > 0 && headerRowCount < frame.rows.count
            ? Array(frame.rows.prefix(headerRowCount))
            : []
        guard let firstRow = candidateRows.first else { return ([], 0) }
        let candidateHeight = candidateRows.reduce(CGFloat(0)) { partial, row in
    ...
    /// 줄 캐시가 주는 문단의 줄 상자 범위 (pt) — 첫 줄 상단부터 가장 아래 줄 상자(`vertsize`,
    /// `lineHeight`) 하단까지. 쪽 끝 적합 판정(#222)의 캐시 출처다 — `cachedParagraphHeight`와
    /// 달리 줄 간격(`lineSpacing`)과 문단 위·아래 간격을 넣지 않는다. 캐시가 유효하지 않으면 nil.
    func cachedLineBoxExtent(_ paragraph: CoreHwp.HwpParagraph) -> CGFloat? {
        let segments = paragraph.paraLineSeg.paraLineSegInternalArray
        guard isValidLineSegmentCache(segments) else { return nil }
        let top = Int(segments[0].lineLocation)
        var bottom = top
        for segment in segments {
            bottom = max(bottom, Int(segment.lineLocation) + Int(max(0, segment.lineHeight)))
        }
        return max(0, HwpUnits.points(fromHwpUnit: Int32(clamping: bottom - top)))
    }
