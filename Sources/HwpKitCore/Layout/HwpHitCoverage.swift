...
extension HwpHitTester {
    ...
    ///   몫(`glyphOffsetReach`)을 더한다. Helvetica 10/10은 위 0·아래 0.8pt라 각주 참조 번호
    ///   (+2.1pt)만 있는 본문은 위 2.1pt 띠에서만 조판한다 — 자격 띠(줄 높이 + 오프셋)를 그대로
    ...
    static func textBounds(_ rect: CGRect, of attributed: NSAttributedString) -> CGRect {
        ...
        // 더해야 상위집합이다. 줄 높이만 보면 오프셋이 그보다 큰 글리프 — 첨자(0.64배
        // 글꼴·+0.44em, #204)와 글자 위치(±1.0×크기)가 **누적**되는 글자 모양이나, 공개
        // 키로 실린 큰 값 — 위의 탭이 rect 판정에 닿기도 전에 블록 단계에서 기각된다
        // (#200 리뷰 실측, 당시 첨자 상수 0.67배·0.33em 기준: Helvetica 10pt·줄 높이 10에서
        // 오프셋 12부터 `hit`이 nil, 6.7pt 글꼴에 13.3이면 밴드 y 87.2…93.9가 자격
        // 93.3…116.7 밖 — 지금 상수로는 6.4pt 글꼴에 14.4, `HwpLargeGlyphOffsetEligibilityTests`).
