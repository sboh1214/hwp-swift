    ...
    private func layoutBandBlocks(
        ...
        for paragraph in paragraphs {
            ...
            guard !HwpTextRunBuilder.isEmptyParagraphAnchor(attributed) else {
                ...
                // (#137). 조판 문자열이 빈 문단 앵커(#145)라 높이는 측정 계층
                // (`HwpParagraphMeasurer`·`HwpPaginator.layout`)과 같은 `layout`으로
                // 잰다. 크롬은 선택·복사 단위가 아니라 앵커 블록을 둘 이유가 없다.
                cursorY += max(1, paragraphLayout.layout(
