...
- **`FixturePreview.renderImage`가 이 API에 위임한다** — 그래야 렌더 가드 다섯 층이 테스트 전용 사본이 아니라 출하되는 코드를 검사한다. 하네스에 남는 것은 `zoom`과 `.fail` 정책 선택뿐이다
...
- **장식선의 기준은 선마다 다르다** (#136·#176·#179·#226, 네 장식선은 한글 12.30 PDF 실측).
  네 선(취소선·글자 가운데·아래·위 밑줄)의 공통은 줄 원점(글자 위치로 옮겨진 글리프를
  따라가지 않는다)과 크기의 기준인 **글자 모양 기본 크기**(`hwp.baseFontSize` — 슬롯 상대
...
- **MS 워드 호환 문서(`hwp.compatibleDocumentTarget` == `msWord`)의 장식선은 글꼴 지표
  ...
  두 값을 미리 푼다: ① `HwpDrawnTextLayout.underlineReference(of:endsParagraph:)`가 싣는
  `msWordLineBox`(`msWordLineBox(of:endsParagraph:)`와 같은 값) — 줄의
  ...
  **세로 배치가 쓰는 줄 상자와 같은 값**(`LineMetrics.msWordLineBox`, #194·#223)
  ...
  글자 크기 비례 경로(밑줄은 줄 상자 가장자리, #226)이고, **한글 2007 호환(`hwp200X`)은 같은
  자리에 크기와 무관한 고정 0.36pt 선을 얹는 제3의 갈래**다 (#210, 갈래 판정과 세 산식은
...
- **복사는 평문 + RTF 두 표현형을 한 항목에 싣는다** (#118). 조립은 HwpKitCore(`HwpSelectionController.selectedAttributedText()` — 평문과 `.string` 파리티). **평문도 그 `.string`에서 얻는다** — `selectedText()`를 따로 부르면 같은 값을 얻으려고 선택 전체(페이지 순회·dedup·조각 substring·마커 제거)를 한 번 더 돌고, 복사는 `@MainActor`에서 돈다 (합성 40,000문단 전체 선택 실측: 0.345 → 0.249초). 정규화·직렬화는 `HwpSelectionRTF` — CT·`hwp.*` 키를 표준 키로 바꾸는 변환 표가 그 파일 하나에 있고, 변환 뒤 남은 `hwp.*`는 접두사 일괄 제거라 미래 키도 새지 않는다. **줄 간격은 `hwp.lineSpacing` 규칙 표식에서 옮긴다** (#180) — 비율 → `lineHeightMultiple`, 고정 → min = max, 최소 → `minimumLineHeight`, 여백만 → `lineSpacing`; CT 스타일의 줄 높이 지정은 규칙이 없는 문자열의 폴백이다 (CT 힌트는 고정과 최소를 가르지 못하고 Cocoa RTF 작성기는 네 속성을 전부 왕복시킨다 — `\slmult1`·`\sl-N`·`\slN`·`\slleadingN`). RTF 직렬화 실패는 평문 복사를 막지 않는다 (조건 추가). 페이스트보드 주입 지점은 양 플랫폼 대칭이다 — macOS `HwpDocumentNSView.pasteboard`, iOS `HwpDocumentUIView.pasteboard` (둘 다 테스트 주입용, 왕복 테스트가 실제 기록을 단언한다).
