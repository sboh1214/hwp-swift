...
- 탐색(`onCurrentMatchChanged`)은 오버레이를 **한 번만** 다시 만든다 — `scrollToMatch` 가 끝에서 `updateVisiblePages` 를 돌려 이미 칠했으면 (반환값 `true`) 호출부가 건너뛴다 (#75 리뷰). 무조건 부르면 페이지별 매치 전량 필터와 CGPath 재구성을 탐색마다 두 번 문다. 스크롤이 없었던 경우(매치 없음·범위 밖·iOS 첫 레이아웃 전)에만 직접 부른다.
