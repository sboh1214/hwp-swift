...
- 확정됐다가 바이트 예산으로 **축출된** 변형은 재시도하지 않고 nil이다. 재요청하면 동시 해석자끼리 서로를 밀어내는 라이브락이 된다 — 그 페이지를 온전히 그려야 하는 경로는 `evictsPinnedOverBudget: false` provider를 쓴다 (아래 PDF 절)
...
- 고정 변형을 **예산 초과여도 축출하지 않는다** (`evictsPinnedOverBudget: false`). 뷰의 하드 상한은 축출된 pin을 다음 재드로우가 되살린다는 전제 위에 있는데, 여기는 draw가 한 번뿐이라 되살릴 기회가 없어 그 이미지가 회색 사각형으로 **PDF에 박힌다** (한 페이지가 256MB를 넘길 때 — 4096² 이미지 4장). 상주량은 한 페이지 작업셋이고 페이지가 바뀌면 unpin이 예산 안으로 되돌린다
