...
extension HwpPageLayer {
    ...
    /// 실측 #179, 함초롬바탕·Apple SD 동일: 10pt 위 첨자 6.36pt 글리프가 4.44pt 올라가면
    /// 선은 원래 베이스라인 위 6.60pt = 4.44 + 0.35 × 6.36, 아래 첨자가 1.20pt 내려가면
    /// 1.08pt = −1.20 + 2.28; 글자 가운데 밑줄도 같은 자리). 글자 위치(`hh:offset`)로
    /// 옮겨진 몫은 따라가지 않는다 — 첨자와 글자 위치를 함께 준 run에서도 선은 첨자
    /// 몫만 옮겨진 자리(위 첨자 + 위치 50에서 6.72pt)에 남는다. 종전에는 이동 없이
    /// 원래 베이스라인에 그려 첨자 글리프 아래로 떨어졌다.
    ///
    ...
    /// 1.14pt 위·0.27pt 두께로 첨자 글리프에 붙었다.
