...
`.swiftlint.yml`의 `included`에 적힌 경로(`Sources`·`Tests`·`Sample`·`Package.swift`)를
봅니다. 샘플 앱(`Sample/`)도 그 목록에 있으므로 함께 검사되며, 오류(error) 수준
위반은 CI와 pre-commit 훅을 모두 실패시킵니다(경고는 실패시키지 않습니다).
